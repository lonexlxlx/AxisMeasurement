...
program_21::program_21(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
...
bool program_21::stepLengthCompensationCaculation_axis5()
{
...
void program_21::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
void program_21::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime = 50)
{
	//初始化变量
...
void program_21::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_21::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_21::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_21::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_21::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_21::clearRoundoutData()
{
...
//***********************************************************************************************************************************************************************************************************************************************
string program_21::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_21::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_21::cam2_Measure_prepare(int positionNumber)
{
...
void program_21::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_21::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_21::creatDatabaseTable()
{
...
void program_21::writeToDatabase()
{
	/*
...
void program_21::zeroMeasureNub()
{
	allFeatureFlag = "--";
	ngFeatureNum = 0;//NG特征数量
	measurePartsNum_all = 0;//该型号轴测量总数
...
void program_21::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
//远心相机图像检测算法
void program_21::cam0Picture_algorithm()//远心相机1图像算法
{
...
//螺纹中径计算函数
double program_21::cam0Picture1_algorithmLuowen(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
	...
	hv_ModelFile = "./programParmeter/WJ10H.13.1035A-LUOWEN/cam0_LUOWENmuban.sbm";//读取螺纹匹配模板，需要改动
	...
	AddMetrologyObjectGeneric(hv_MetrologyHandle, "line", ((((hv_Line1.TupleConcat(hv_Line2)).TupleConcat(hv_Line3)).TupleConcat(hv_Line4)).TupleConcat(hv_Line5)),
	...
	double D = 53.745;
	double Pix_luowen = D / (2 * Calik) + x00;
	cout << "D:" << D << endl;
...
double program_21::cam0Picture1_L_K_b(HObject ho_Image, double* hv_Box)
{
	...
	hv_ModelFile = "./programParmeter/WJ10H.13.1035A-LUOWEN/cam0_ZHIXIANmuban.sbm3";//参考直线匹配模板，需要改动
	ReadShapeModel(hv_ModelFile, &hv_ReusedModelID);
	//创建测量模型	
	CreateMetrologyModel(&hv_MetrologyHandle);
...
HObject program_21::imgAug(string imgPath) {
...
//模板匹配找特征点
void program_21::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
...
//根据起始点与终止点求倒角
double program_21::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
...
double program_21::TLineFP_Chamfer_H2(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
double program_21::TLineFP_Chamfer_H2_117up(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
void program_21::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_21::Straight_TLineFP_P137(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour;
...
void program_21::Straight_TLineFP_P116(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_21::Straight_TLineFP_P170(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_21::Straight_TLineFP_P102(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_21::cam0_roundoutMeasure_prepare(int positionNumber)
{
	//trap5轴
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_21::clearScrewRoundOutData()
{
...
double program_21::cam1Picture_holeAlgorithm0(string imgPath)//H0孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_21::cam1Picture_holeAlgorithm1(string imgPath)//H1孔图像算法
{
	//在下方贴入孔径图像处理代码
...
double program_21::cam1Picture_holeAlgorithm2(string imgPath)//H2孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_21::cam1Picture_holeAlgorithm3(string imgPath)//H3孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_21::cam1Picture_holeAlgorithm4(string imgPath)//H4孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
void program_21::run()
{
	...
	emit programTips("1，下顶尖换成平头顶尖； 2，零件大端朝下装夹； 3，使用鸡心夹夹紧零件大端任意外圆； 4，确保拨叉位于相机侧后，点击'程序确认按钮'进行检测！", 1);
	do
	{
		msleep(500);
	...
	clearScrewRoundOutData();//测螺纹需要进入该函数并修改该函数的具体内容不需要测量螺纹则注释
	...
	defaultPath = defaultPath + "\\measureData\\WJ10H.13.1035A";//！需要将文件后夹后缀名改为对应的零件号
	...
	*/
	...
	HTuple PatternlUOWENpath = "./programParmeter/WJ10H.13.1035A-LUOWEN/cam0_ZHIXIANmuban.sbm";//参考直线模板路径，需要改动
	...
	if (looptime_luowen == cam0_roundoutImgNumber)
	{
		...
		double sumValue1 = accumulate(screwResult.begin(), screwResult.end(), 0.0);
		 ...
		 double MaxValue2= *max_element(screwResult.begin() + 1, (screwResult.begin() + 4));
		 double MiValue3 = *min_element(screwResult.begin() + 1, (screwResult.begin() + 4));
		...
		//pushback_vectors(resultVectorList, 11, 4, 52.943, -0.1, +0);

		//以下两行是对螺纹跳动进行计算，如不需要需要将下面两行注释掉
		resultVectorList.push_back(screwResultMax/2- screwResultMin/2);
		pushback_vectors(resultVectorList, 5, 2, 0, +0.015, 0);//第一个参数resultVectorList不变，第二个参数5不变，第三个参数为特征编号，第四个参数为公称值，第五个参数为公差下限，第六个参数为公差上限
		//resultVectorList.push_back(sumValue2);
		//pushback_vectors(resultVectorList, 11, 77, 38.771, -0.094, +0);
		//resultVectorList.push_back(screwResult.1);

		//resultVectorList.push_back(MaxValue2);
		//pushback_vectors(resultVectorList, 11, 1, 0, 0, +0.015);
		//resultVectorList.push_back(MiValue3);
		//pushback_vectors(resultVectorList, 11, 2, 0, 0, +0.015);
