...
program_14::program_14(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
	...
	//跳动数据结构体初始化
	for (int i = 0; i < 6; i++)//如有跳动测量需求将i的循环上限修改为需要进行跳动采集的轴段数量
	{
		roundoutDataPtr[i]= new roundoutData();
	};
	clearRoundoutData();//需要修改本函数中的循环次数为需要进行跳动采集的轴段数量
...
bool program_14::stepLengthCompensationCaculation_axis5()
{
...
void program_14::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
void program_14::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime = 50)
{
	//初始化变量
...
void program_14::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_14::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_14::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_14::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_14::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_14::clearRoundoutData()
{
	for (int i = 0; i < 6; i++)//需改改为跳动采集的轴段总数
	{
...
//***********************************************************************************************************************************************************************************************************************************************
string program_14::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_14::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_14::cam2_Measure_prepare(int positionNumber)
{
...
void program_14::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_14::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_14::creatDatabaseTable()
{
...
void program_14::writeToDatabase()
{
	/*
...
void program_14::zeroMeasureNub()
{
	allFeatureFlag = "--";
	ngFeatureNum = 0;//NG特征数量
	measurePartsNum_all = 0;//该型号轴测量总数
...
void program_14::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
//远心相机图像检测算法
void program_14::cam0Picture_algorithm()//远心相机1图像算法
{
	//大远心相机采集图像路径
	/*大远心相机采集图像路径--调用示例
	...
	*/
	string Imgh4path = "C:/Users/123/Desktop/XD/H0.bmp";
	...
	*/
	...
	//2.cam0000直线模板匹配找直线特征点--模板调用示例
	HTuple Pattern1path = "./programParmeter/WZ10451080-370-3/cam0_xd-1.sbm";
	double cam0_1_84_XD[4];
	TemplateMatching_TLineFP_P(cam0_1_84_XD, Img1Aug,
		Pattern1path,
		100, -1420, 100, -220,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_176_XD[4];
	TemplateMatching_TLineFP_P(cam0_1_176_XD, Img1Aug,
		Pattern1path,
		-120, 75, 80, -87,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	...
	HTuple Pattern2path = "./programParmeter/WZ10451080-370-3/cam0_xd-2.sbm";
	double cam0_1_84_XD2[4];
	TemplateMatching_TLineFP_P(cam0_1_84_XD2, Img2Aug,
		Pattern2path,
		0, 40, 130, 40,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	...
	//double AxialD_105_XD=cam0_1_84_XD2[3] - cam0_1_84_XD[3];
	...
	//计算拍照之间的距离（需要，多次测量，多次声明，且复制修改下行代码）
	...
	double pic2To3_moveDistance;
	pic2To3_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[3]) - axis5_compensation(cam0_arriveOrgEncode_axis5[2]);//计算拍照位置0和拍照位置1之间的距离(减法代表拍照位置2-拍照位置1)
	...
	double AxialD_176_xd = atan((cam0_1_176_XD[0] - cam0_1_176_XD[2]) / (cam0_1_176_XD[3] - cam0_1_176_XD[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_176_xd);
...
double program_14::cam0Picture1_algorithmLuowenDown(HObject ho_Image,  HObject ho_Image_L,double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
...
double program_14::cam0Picture1_algorithmLuowenUp(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
	// Local iconic variables
	HObject  ho_ModelContours, ho_LineContours;
	HObject  ho_LineContour;
...
double program_14::cam0Picture1_L_K_b_Down(HObject ho_Image, double* hv_Box)
{
...
double program_14::cam0Picture1_L_K_b_Up(HObject ho_Image, double* hv_Box)
{
...
HObject program_14::imgAug(string imgPath) {
...
//模板匹配找特征点
void program_14::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
...
//根据起始点与终止点求倒角
double program_14::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
...
double program_14::TLineFP_Chamfer_H2(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
double program_14::TLineFP_Chamfer_H2_117up(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
void program_14::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_14::Straight_TLineFP_P137(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour;
...
void program_14::Straight_TLineFP_P116(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_14::Straight_TLineFP_P170(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_14::Straight_TLineFP_P102(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_14::cam0_roundoutMeasure_prepare(int positionNumber)
{
	//trap5轴
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_14::clearScrewRoundOutData()
{
	for (int i = 0; i < 2; i++)
	{
...
//远心相机1测孔 //需要从sample 里粘贴进来
double program_14::cam1Picture_holeAlgorithm0(string imgPath)//远心相机1测孔图像算法
{//需要从sample 里粘贴进来
	HTuple halconPath = imgPath.c_str();
	...
	double cam0_11_460_152LeftChamferBegin_TLineFP[4];
	{
		...
		CreateMetrologyModel(&hv_MetrologyHandle);//需要
		hv_Line1.Clear();
		hv_Line1[0] = 327;
		hv_Line1[1] = 1176;
		hv_Line1[2] = 327;
		...
		return diameter1;//粘贴至此
	//	resultVectorList.push_back(diameter1);
		//pushback_vectors(resultVectorList, 8, 111, 8.9, -0, +0.2);
	}
	

};
double program_14::cam1Picture_holeAlgorithm1(string imgPath)//远心相机1测孔图像算法
{
	return 0;
};
double program_14::cam1Picture_holeAlgorithm2(string imgPath)//远心相机1测孔图像算法
{
	return 0;
};
...
void program_14::run()
{
	...
	defaultPath = defaultPath + "\\measureData\\XD-WZ10451080-370";//需要将文件后夹后缀名改为对应的零件号
	...
	*/
	...
	for (int i = 7; i >= 0; i--)//第二次测量（向下移动）（将i修改为直径采集的总次数-1）
	{
	...
	pushback_vectors(resultVectorList, 1, lsDiameterResult[3][3], lsDiameterResult[3][4], lsDiameterResult[3][6], lsDiameterResult[3][5]);//需要sDiameterResult[i=3][3]
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();

	
	cam2_radius[0][1] = lsDiameterResult[0][9];//传递旋转半径d4给粗糙度0号采集位置（如有新增复制本句代码）进行相应修改即可lsDiameterResult[i] ,i为光幕测直径的序号
	...
	*/
	...
	pic0To2_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[2]) - axis5_compensation(cam0_arriveOrgEncode_axis5[0]);//计算拍照位置0和拍照位置1之间的距离(减法代表拍照位置2-拍照位置1)
	...
	emit programProcess(QString("正在对 （圆柱度） 进行数据采集！"), 81 );//需要，多次测量，多次复制修改
	lsSensorMeasure_cylindricity(0);//需要,圆柱度测量函数，此处表明对为圆柱度测量位置C0进行圆柱度测量,如有多个位置有圆柱度测量要求可以参照下面代码新加
	lsSensorMeasure_cylindricity(1);//需要,圆柱度测量函数，此处表明对为圆柱度测量位置C0进行圆柱度测量,如有多个位置有圆柱度测量要求可以参照下面代码新加
	/*
	...
	*/
	...
	cam1_Measure_prepare(0);//需要，多次测量，多次复制修改，对孔径拍摄位置H0进行测量（这个函数参数为0）
	int looptime = 0;
	for (int i = 0; i < cam1PathList.size(); i++)
	{
		try {
			double diameter = cam1Picture_holeAlgorithm0(cam1PathList[i]);
	...
	{
		try {
			double diameter = cam1Picture_holeAlgorithm1(cam1PathList[i]);
			resultVectorList.push_back(diameter);
			looptime++;
	...
	*/
	...
	double referencePoint[6][3];//可能需要，修改数组第一个长度=基准数量*3，如A-B基准则为6，单基准则为3
	int loopTime = 0;
	for (int j = 0; j < 3; j++)
	{
		copy(begin(roundoutDataPtr[0]->axisPoint[j]), end(roundoutDataPtr[0]->axisPoint[j]), begin(referencePoint[loopTime]));//需要，oundoutDataPtr[13]中 i=跳动段数，提取第二个基准截面上的圆心数据，提取第一个基准上三个数据采集截面的圆心数据
		loopTime++;
		copy(begin(roundoutDataPtr[5]->axisPoint[j]), end(roundoutDataPtr[5]->axisPoint[j]), begin(referencePoint[loopTime]));//可能需要，A-B则2两处，A基准则删行，
		loopTime++;
	}
