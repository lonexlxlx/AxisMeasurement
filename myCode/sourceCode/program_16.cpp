...
program_16::program_16(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
	...
	//跳动数据结构体初始化
	for (int i = 0; i < 2; i++)//如有跳动测量需求将i的循环上限修改为需要进行跳动采集的轴号（!!!!只是为了测试新建的子程序是否添加到主程序中时可以暂时不用修改）
	{
...
bool program_16::stepLengthCompensationCaculation_axis5()
{
...
void program_16::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
void program_16::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime = 50)
{
	//初始化变量
...
void program_16::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_16::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_16::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_16::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_16::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_16::clearRoundoutData()
{
	for (int i = 0; i < 2; i++)//需要将循环体的i变量修改为跳动采集的轴段总数
	{
...
//***********************************************************************************************************************************************************************************************************************************************
string program_16::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_16::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_16::cam2_Measure_prepare(int positionNumber)
{
...
void program_16::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_16::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_16::creatDatabaseTable()
{
...
void program_16::writeToDatabase()
{
	/*
...
void program_16::zeroMeasureNub()
{
	allFeatureFlag = "--";
	ngFeatureNum = 0;//NG特征数量
	measurePartsNum_all = 0;//该型号轴测量总数
...
void program_16::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
//远心相机图像检测算法
void program_16::cam0Picture_algorithm()//远心相机1图像算法
{
	...
	//大远心相机采集图像路径
	/*大远心相机采集图像路径--调用示例
	string Img1path = "C:/Users/123/Desktop/XD/0.bmp";//需要，复制进RUN后，修改string Img1path = imgSavePath[0];
	*/
	...
	*/
	//	string ImghpathH0 = "C:/Users/123/Desktop/XD/12.7//H0.bmp";
	...
	HTuple Pattern1path = "D:/temporary_sun/AxisMeasurement/programParmeter/WZ10151107-160-1/cam0_1.sbm";
	double cam0_1_51_1_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_51_1_TLineFP, Img1_raw,
		Pattern1path,
		-670, 170, -670, 300,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_93_TLineFP[4];
	Straight_TLineFP_P(cam0_1_93_TLineFP, Img1_raw,
		3334, 1724, 3831, 1724,
		50, 12, 1, 70);
	double Chamfer_73 = TLineFP_Chamfer_P(Img1_raw,
		cam0_1_93_TLineFP[0] - 100, cam0_1_93_TLineFP[1] - 50, cam0_1_93_TLineFP[0] + 50, cam0_1_93_TLineFP[1] + 15000,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
	...
	pushback_vectors(resultVectorList, 2, 93, 1.5, -0.2, +0.2);

	double cam0_1_92_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_92_TLineFP, Img1_raw,
		Pattern1path,
		-80, 60, -115, 150,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	...
	double cam0_1_99_1_TLineFP[4];
	Straight_TLineFP_P(cam0_1_99_1_TLineFP, Img2_raw,
		2043, 1815, 2395, 1812,
		50, 12, 1, 70);


	double cam0_1_99_2_TLineFP[4];
	Straight_TLineFP_P(cam0_1_99_2_TLineFP, Img2_raw,
		2412, 1747, 2321, 1834,
		50, 12, 1, 50);
	HTuple Pattern2path = "D:/temporary_sun/AxisMeasurement/programParmeter/WZ10151107-160-1/cam0_2.sbm";
	double cam0_1_51_1_2_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_51_1_2_TLineFP, Img2_raw,
		Pattern2path,
		-600, 120, -300, 120,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	...
	double AxialD_99 = cam0_1_99_2_TLineFP[0] - cam0_1_99_1_TLineFP[0];
	resultVectorList.push_back(AxialD_99 * Calik);
	pushback_vectors(resultVectorList, 4, 99, 3.84, -0.05, +0.05);
	double AxialD_51 = cam0_1_51_1_2_TLineFP[0] - cam0_1_51_1_TLineFP[0];
	resultVectorList.push_back(AxialD_51 * Calik + pic0To1_moveDistance);
	pushback_vectors(resultVectorList, 4, 84, 69.51, -0.1, +0.1);
	double AxialD_92_1 = atan((cam0_1_92_TLineFP[0] - cam0_1_92_TLineFP[2]) / (cam0_1_92_TLineFP[3] - cam0_1_92_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_92_1);
	pushback_vectors(resultVectorList, 3, 92, 11, -1, +1);
...
double program_16::cam0Picture1_algorithmLuowenDown(HObject ho_Image,  HObject ho_Image_L,double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
...
double program_16::cam0Picture1_algorithmLuowenUp(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
	// Local iconic variables
	HObject  ho_ModelContours, ho_LineContours;
	HObject  ho_LineContour;
...
double program_16::cam0Picture1_L_K_b_Down(HObject ho_Image, double* hv_Box)
{
...
double program_16::cam0Picture1_L_K_b_Up(HObject ho_Image, double* hv_Box)
{
...
HObject program_16::imgAug(string imgPath) {
...
//模板匹配找特征点
void program_16::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
...
//根据起始点与终止点求倒角
double program_16::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
...
double program_16::TLineFP_Chamfer_H2(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
double program_16::TLineFP_Chamfer_H2_117up(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
void program_16::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_16::Straight_TLineFP_P137(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour;
...
void program_16::Straight_TLineFP_P116(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_16::Straight_TLineFP_P170(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_16::Straight_TLineFP_P102(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_16::cam0_roundoutMeasure_prepare(int positionNumber)
{
	//trap5轴
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_16::clearScrewRoundOutData()
{
	for (int i = 0; i < 2; i++)
	{
...
double program_16::cam1Picture_holeAlgorithm0(string imgPath)//H0孔图像处理算法
{
	//在下方贴入孔径图像处理代码
	HTuple halconPath = imgPath.c_str();
	...
	double cam0_11_460_152LeftChamferBegin_TLineFP[4];
	{
		...
		CreateMetrologyModel(&hv_MetrologyHandle);//需要修改HAC调试坐标
		hv_Line1.Clear();
		hv_Line1[0] = 703;
		hv_Line1[1] = 1173;
		hv_Line1[2] = 703;
...
double program_16::cam1Picture_holeAlgorithm1(string imgPath)//H1孔图像算法
{
	//在下方贴入孔径图像处理代码
...
double program_16::cam1Picture_holeAlgorithm2(string imgPath)//H2孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_16::cam1Picture_holeAlgorithm3(string imgPath)//H3孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_16::cam1Picture_holeAlgorithm4(string imgPath)//H4孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
void program_16::run()
{
	...
	defaultPath = defaultPath + "\\measureData\\WZ10151107-160-1";//！需要将文件后夹后缀名改为对应的零件号
	...
	*/
	...
	cam2_radius[0][1] = lsDiameterResult[3][9];//传递旋转半径d4给粗糙度0号采集位置（如有新增复制本句代码）进行相应修改即可
	cam2_radius[1][1] = lsDiameterResult[6][9];
	/*
	新增参考代码
	...
	*/
	...
	camPtrList[0]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[0]->capturedImg);
	for (int i = 0; i < 2; i++)//远心0-1号采集位置采集图像（将i修改为远心相机的拍照位置总数，循环中的其他语句不需要修改）
	{
		camPtrList[0]->setExposeTime(cam0_exposeTime);
		camPtrList[0]->m_captureMode = "continuous";
		originalImgPtr = &(camPtrList[0]->capturedImg);
	...
	*/
	...
	//跳动测量*****************************************************************
	//第一步采各跳动测量轴段的基本数据
	for (int i = 1; i >= 0; i--)//只需要修改i值为跳动数据采集的轴段的总数-1（该循环中的其他部分不需要修改）
	{
	...
	for (int i = 0; i <2; i++)//（只需要修改i值为跳动数据采集的轴段的总数）
	{
		for (int j = 0; j < 3; j++)
		{
			centerCosFit(i, j);
	...
	//double referencePoint[0][3];//需要数组第一个长度=基准数量*3
	//int loopTime = 0;
	//for (int j = 0; j <3; j++)
	//{
	...
	for (int i = 0; i <2; i++)//需要修改i值为跳动数据采集的轴段的总数
	{
		//对于基准采集位置不进行跳动计算，所以使用case跳过
		switch (i) {
		case 0://跳动计算轴段RT2（以顶尖为基准）
	...
	{
		pushback_vectors(resultVectorList, 8, 81, 4.62, -0.25, +0.15);//含义为将多次孔径测量的结果显示到软件上，需要修改pushback_vector参数的特征号，公称值等信息
