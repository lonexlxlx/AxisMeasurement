...
program_9::program_9(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
...
bool program_9::stepLengthCompensationCaculation_axis5()
{
...
void program_9::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
void program_9::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime = 50)
{
	//初始化变量
...
void program_9::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_9::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_9::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_9::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_9::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_9::clearRoundoutData()
{
	for (int i = 0; i < 2; i++)
	{
...
//***********************************************************************************************************************************************************************************************************************************************
string program_9::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_9::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_9::cam2_Measure_prepare(int positionNumber)
{
...
void program_9::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_9::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_9::creatDatabaseTable()
{
...
void program_9::writeToDatabase()
{
	/*
...
void program_9::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
//远心相机图像检测算法
void program_9::cam0Picture_algorithm()//远心相机1图像算法
{
	...
	int fault_detect = 0;
	fault_detect = 1;
	HTuple Pattern1path = "./programParmeter/WZ10-11-2006-430/cam0_1-430-8.sbm";
	

	HTuple Pattern2path = "./programParmeter/WZ10-11-2006-430/cam0_1-430-5.sbm";
	double cam0_1_8left_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_8left_TLineFP, Img1_raw,
		Pattern2path,
		-1580, -140, -1580, -10,
		30, 12, 1, 1,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_6right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_6right_TLineFP, Img1Aug,
		Pattern2path,
		30, 50, 250, 50,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_5_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_5_TLineFP, Img1Aug,
		Pattern2path,
		-45, -45, 30, 30,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);
	...
	TemplateMatching_TLineFP_P(cam0_1_7right_TLineFP, Img1Aug,
		Pattern2path,
		630, 50, 880, 50,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_11right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_11right_TLineFP, Img1Aug,
		Pattern2path,
		1030, 40, 1280, 40,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_8right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_8right_TLineFP, Img1Aug,
		Pattern2path,
		1260, 30, 1520, 30,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_10right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_10right_TLineFP, Img1Aug,
		Pattern2path,
		1680, 30, 1960, 30,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	...
	fault_detect = 2;
	HTuple Pattern3path = "./programParmeter/WZ10-11-2006-430/cam0_2-430-59.sbm";
	double cam0_2_59left_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_59left_TLineFP, Img2Aug,
		Pattern3path,
		-530, 30, -330, 30,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_59right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_59right_TLineFP, Img2Aug,
		Pattern3path,
		-160, 30, 20, 30,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	...
	double cam0_2_64left_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_64left_TLineFP, Img2Aug,
		Pattern3path,
		810, 30, 990, 30,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);
	fault_detect = 3;
	HTuple Pattern4path = "./programParmeter/WZ10-11-2006-430/cam0_3-430-60.sbm";
	double cam0_3_60right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_60right_TLineFP, Img3Aug,
		Pattern4path,
		-610,1020,-610,1140,
		50, 12, 1, 1,
		0.5, 1, 0.3, 0, 0.7);
	...
	double cam0_3_109up_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_109up_TLineFP, Img3Aug,
		Pattern4path,
		-940, 830, -740, 830,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_109down_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_109down_TLineFP, Img3Aug,
		Pattern4path,
		-700, 800, -650, 800,
		50, 12, 1, 1,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_67_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_67_TLineFP, Img3Aug,
		Pattern4path,
		-180, 1010, -330, 1170,
		50, 12, 1, 60,
		0.5, 1, 0.3, 0, 0.7);
	...
	double cam0_3_87_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_87_TLineFP, Img3Aug,
		Pattern4path,
		70, 20, 150, 100,
		50, 12, 1, 60,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_88left_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_88left_TLineFP, Img3Aug,
		Pattern4path,
		220, 180, 310, 180,
		50, 12, 1, 60,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_88right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_88right_TLineFP, Img3Aug,
		Pattern4path,
		340, 40, 340, 140,
		50, 12, 1, 60,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_91left_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_91left_TLineFP, Img3Aug,
		Pattern4path,
		870, 40, 870, 140,
		50, 12, 1, 10,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_91right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_91right_TLineFP, Img3Aug,
		Pattern4path,
		1000, 80, 1000, 160,
		50, 12, 1, 60,
		0.5, 1, 0.3, 0, 0.7);
	...
	double cam0_3_65right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_65right_TLineFP, Img3Aug,
		Pattern4path,
		-200, 800, -200, 930,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_79right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_79right_TLineFP, Img3Aug,
		Pattern4path,
		1000, 80, 1000, 150,
		50, 12, 1, 20,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_122right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_122right_TLineFP, Img3Aug,
		Pattern4path,
		1300, 40, 1468, 40,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_124_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_124_TLineFP, Img3Aug,
		Pattern4path,
		1750, 10, 1990, -10,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);
	...
	TemplateMatching_TLineFP_P(cam0_3_125_TLineFP, Img3Aug,
		Pattern4path,
		3100, -20, 3400, 0,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);
	...
	fault_detect = 4;
	HTuple Pattern5path = "./programParmeter/WZ10-11-2006-430/cam0_4-430-131new.sbm";
	double cam0_4_127left_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_4_127left_TLineFP, Img4Aug,
		Pattern5path,
		-1190, 120, -1100, 120,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_4_128left_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_4_128left_TLineFP, Img4Aug,
		Pattern5path,
		-1030, 190, -920, 190,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_4_128right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_4_128right_TLineFP, Img4Aug,
		Pattern5path,
		-870, 100, -870, 170,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_4_129right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_4_129right_TLineFP, Img4Aug,
		Pattern5path,
		-670, 50, -500, 50,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_4_132_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_4_132_TLineFP, Img4Aug,
		Pattern5path,
		25, -10, -15, 40,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_4_61right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_4_61right_TLineFP, Img4Aug,
		Pattern5path,
		1110, -90, 1110, -40,
		30, 12, 1, 1,
		0.5, 1, 0.3, 0, 0.7);
	...
	double AxialD_430_6 = cam0_1_6right_TLineFP[0] - (cam0_1_8left_TLineFP[0] + cam0_1_8left_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_430_6* Calik);
	pushback_vectors(resultVectorList, 4, 6, 19.8, -0.26, +0.26);

	double AxialD_430_5 = atan((cam0_1_5_TLineFP[2] - cam0_1_5_TLineFP[0]) / (cam0_1_5_TLineFP[3] - cam0_1_5_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_430_5);
	pushback_vectors(resultVectorList, 3, 5, 45, -1, +1);

	double Chamfer_430_4 = TLineFP_Chamfer_P(Img1Aug,
		cam0_1_5_TLineFP[0] - 75, 1800, cam0_1_5_TLineFP[0], cam0_1_5_TLineFP[1],
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_4* Calik);
	pushback_vectors(resultVectorList, 2, 4, 1, -0.3, +0.3);


	double AxialD_430_7 = cam0_1_7right_TLineFP[2] - (cam0_1_8left_TLineFP[0] + cam0_1_8left_TLineFP[2]) / 2 + 3;
	resultVectorList.push_back(AxialD_430_7* Calik);
	pushback_vectors(resultVectorList, 4, 7, 29.7, -0.26, +0.26);

	double Chamfer_430_9_1 = TLineFP_Chamfer_P(Img1Aug,
		cam0_1_7right_TLineFP[2] + 12, 1800, cam0_1_7right_TLineFP[2] + 100, cam0_1_7right_TLineFP[1] - 15,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_9_1* Calik);
	pushback_vectors(resultVectorList, 2, 9.1, 1, -0.3, +0.3);

	double Chamfer_430_9_2 = TLineFP_Chamfer_P(Img1Aug,
		cam0_1_11right_TLineFP[0] - 100, 1800, cam0_1_11right_TLineFP[0] - 25, cam0_1_11right_TLineFP[1] - 17,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_9_2* Calik);
	pushback_vectors(resultVectorList, 2, 9.2, 1, -0.3, +0.3);


	double AxialD_430_11 = cam0_1_11right_TLineFP[0] - cam0_1_7right_TLineFP[2] - 9;
	//resultVectorList.push_back(AxialD_430_11* Calik);
	//pushback_vectors(resultVectorList, 4, 11, 2.5, -0.1, +0.1);

	double AxialD_430_8 = cam0_1_8right_TLineFP[2] - (cam0_1_8left_TLineFP[0] + cam0_1_8left_TLineFP[2]) / 2 + 7;
	resultVectorList.push_back(AxialD_430_8* Calik);
	pushback_vectors(resultVectorList, 4, 8, 37.5, -0.31, +0.31);



	double AxialD_430_10 = cam0_1_10right_TLineFP[0] - cam0_1_8right_TLineFP[2] - 8;
	//resultVectorList.push_back(AxialD_430_10* Calik);
	//pushback_vectors(resultVectorList, 4, 10, 2.5, -0.1, +0.1);

	double Chamfer_430_12_2 = TLineFP_Chamfer_P(Img1Aug,
		cam0_1_10right_TLineFP[0] - 110, 1800, cam0_1_10right_TLineFP[0] - 30, cam0_1_10right_TLineFP[1] - 15,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_12_2* Calik);
	pushback_vectors(resultVectorList, 2, 12.2, 1, -0.3, +0.3);

	double Chamfer_430_34 = TLineFP_Chamfer_P(Img2Aug,
		cam0_2_59left_TLineFP[0] - 64, 1800, cam0_2_59left_TLineFP[0] - 6, cam0_2_59left_TLineFP[1],
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_34* Calik);
	pushback_vectors(resultVectorList, 2, 34, 1, -0.3, +0.3);

	double AxialD_430_59 = cam0_2_59right_TLineFP[2] - cam0_2_59left_TLineFP[0] + 3;
	resultVectorList.push_back(AxialD_430_59* Calik);
	pushback_vectors(resultVectorList, 4, 59, 6, -0.15, +0.15);

	double Chamfer_430_35_1 = TLineFP_Chamfer_P(Img2Aug,
		cam0_2_59right_TLineFP[2] + 10, 1800, cam0_2_59right_TLineFP[2] + 70, cam0_2_59right_TLineFP[3],
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_35_1* Calik);

	double Chamfer_430_35_2 = TLineFP_Chamfer_P(Img2Aug,
		cam0_2_64left_TLineFP[0] - 70, 1800, cam0_2_64left_TLineFP[0] - 10, cam0_2_64left_TLineFP[1],
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_35_2* Calik);
	pushback_vectors(resultVectorList, 2, 35, 1, -0.3, +0.3);

	double AxialD_430_60 = (cam0_3_60right_TLineFP[0] + cam0_3_60right_TLineFP[2]) / 2 - (cam0_1_8left_TLineFP[0] + cam0_1_8left_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_430_60* Calik + pic1To2_moveDistance+ pic2To3_moveDistance);
	pushback_vectors(resultVectorList, 4, 60, 135.6, -0, +0.12);

	double AxialD_430_63 = (cam0_3_60right_TLineFP[0] + cam0_3_60right_TLineFP[2]) / 2 - cam0_2_59right_TLineFP[2];
	resultVectorList.push_back(AxialD_430_63* Calik + pic2To3_moveDistance);
	pushback_vectors(resultVectorList, 4, 63, 51.2, -0.3, +0.3);

	double AxialD_430_64 = (cam0_3_60right_TLineFP[0] + cam0_3_60right_TLineFP[2]) / 2 - cam0_2_64left_TLineFP[0];
	resultVectorList.push_back(AxialD_430_64* Calik + pic2To3_moveDistance);
	pushback_vectors(resultVectorList, 4, 64, 41, -0.3, +0.3);


	double AxialD_430_109 = (cam0_3_109up_TLineFP[1] + cam0_3_109up_TLineFP[3]) / 2 - (cam0_3_109down_TLineFP[1] + cam0_3_109down_TLineFP[3]) / 2;
	resultVectorList.push_back(AxialD_430_109* Calik);
	pushback_vectors(resultVectorList, 4, 109, 0.4, -0.1, +0.1);


	double AxialD_430_67 = ((cam0_3_67_TLineFP[0] - cam0_3_67_TLineFP[2]) + (cam0_3_67_TLineFP[3] - cam0_3_67_TLineFP[1])) / 2;
	resultVectorList.push_back(AxialD_430_67* Calik);
	pushback_vectors(resultVectorList, 4, 67, 1.5, -0.3, +0.3);

	double AxialD_430_87 = atan((cam0_3_87_TLineFP[2] - cam0_3_87_TLineFP[0]) / (cam0_3_87_TLineFP[3] - cam0_3_87_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_430_87);
	pushback_vectors(resultVectorList, 3, 87, 45, -1, +1);

	double AxialD_430_88 = (cam0_3_88right_TLineFP[0] + cam0_3_88right_TLineFP[2]) / 2 - cam0_3_88left_TLineFP[0];
	resultVectorList.push_back(AxialD_430_88* Calik);
	pushback_vectors(resultVectorList, 4, 88, 1.5, -0.12, +0.12);

	double Chamfer_430_89_1 = TLineFP_Chamfer_P(Img3Aug,
		cam0_3_88right_TLineFP[0], 1500, cam0_3_88right_TLineFP[0] + 100, cam0_3_88right_TLineFP[1] - 20,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_89_1* Calik);
	pushback_vectors(resultVectorList, 2, 89.1, 1, -0.3, +0.3);

	double Chamfer_430_89_2 = TLineFP_Chamfer_P(Img3Aug,
		cam0_3_91left_TLineFP[0] - 100, 1500, cam0_3_91left_TLineFP[0], cam0_3_91left_TLineFP[1] - 20,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_89_2* Calik);
	pushback_vectors(resultVectorList, 2, 89.2, 1, -0.3, +0.3);

	double AxialD_430_91 = (cam0_3_91right_TLineFP[0] + cam0_3_91right_TLineFP[2]) / 2 - (cam0_3_91left_TLineFP[0] + cam0_3_91left_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_430_91* Calik);
	pushback_vectors(resultVectorList, 4, 91, 1.5, -0.12, +0.12);

	double AxialD_430_90 = (cam0_3_91left_TLineFP[0] + cam0_3_91left_TLineFP[2]) / 2 - (cam0_3_88right_TLineFP[0] + cam0_3_88right_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_430_90* Calik);
	pushback_vectors(resultVectorList, 4, 90, 6.4, -0.18, +0.18);
	...
	double AxialD_430_65 = (cam0_3_65right_TLineFP[0] + cam0_3_65right_TLineFP[2]) / 2 - (cam0_3_60right_TLineFP[0] + cam0_3_60right_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_430_65* Calik);
	pushback_vectors(resultVectorList, 4, 65, 5, -0.15, +0.15);

	double AxialD_430_79 = (cam0_3_79right_TLineFP[0] + cam0_3_79right_TLineFP[2]) / 2 - (cam0_3_60right_TLineFP[0] + cam0_3_60right_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_430_79* Calik);
	pushback_vectors(resultVectorList, 4, 79, 19.6, -0.1, +0.1);

	

	double AxialD_430_122 = cam0_3_122right_TLineFP[2] - (cam0_3_79right_TLineFP[0] + cam0_3_79right_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_430_122* Calik);
	pushback_vectors(resultVectorList, 4, 122, 5.5, -0.1, +0.1);

	double AxialD_430_124 = atan((cam0_3_124_TLineFP[1] - cam0_3_124_TLineFP[3]) / (cam0_3_124_TLineFP[2] - cam0_3_124_TLineFP[0])) * 180 / PI;
	resultVectorList.push_back(AxialD_430_124);
	pushback_vectors(resultVectorList, 3, 124, 5, -0.25, +0.25);

	double AxialD_430_125 = atan((cam0_3_125_TLineFP[3] - cam0_3_125_TLineFP[1]) / (cam0_3_125_TLineFP[2] - cam0_3_125_TLineFP[0])) * 180 / PI;
	resultVectorList.push_back(AxialD_430_125);
	pushback_vectors(resultVectorList, 3, 125, 5, -0.25, +0.25);

	double Chamfer_430_123_1 = TLineFP_Chamfer_P(Img3Aug,
		cam0_3_79right_TLineFP[0], 1600, cam0_3_79right_TLineFP[0] + 55, cam0_3_79right_TLineFP[1],
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_123_1* Calik);

	double Chamfer_430_123_2 = TLineFP_Chamfer_P(Img4Aug,
		cam0_4_128left_TLineFP[0] - 60, 1600, cam0_4_128left_TLineFP[0] - 20, cam0_4_128left_TLineFP[1] - 25,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_123_2* Calik);
	pushback_vectors(resultVectorList, 2, 123, 0.6, -0.2, +0.2);

	double AxialD_430_127 = (cam0_4_128right_TLineFP[0] + cam0_4_128right_TLineFP[2]) / 2 - cam0_4_127left_TLineFP[0];
	resultVectorList.push_back(AxialD_430_127* Calik);
	pushback_vectors(resultVectorList, 4, 127, 3.8, -0.15, +0.15);

	double AxialD_430_128 = (cam0_4_128right_TLineFP[0] + cam0_4_128right_TLineFP[2]) / 2 - cam0_4_128left_TLineFP[0] + 16;
	resultVectorList.push_back(AxialD_430_128* Calik);
	pushback_vectors(resultVectorList, 4, 128, 1.8, -0.12, +0.12);



	double Chamfer_430_126_1 = TLineFP_Chamfer_P(Img4Aug,
		cam0_4_128right_TLineFP[0], 1500, cam0_4_128right_TLineFP[0] + 40, cam0_4_128right_TLineFP[1] - 60,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_126_1* Calik);

	double Chamfer_430_126_2 = TLineFP_Chamfer_P(Img4Aug,
		cam0_4_129right_TLineFP[0] - 40, 1500, cam0_4_129right_TLineFP[0], cam0_4_129right_TLineFP[1] - 10,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_126_2* Calik);
	pushback_vectors(resultVectorList, 2, 126, 0.5, -0.2, +0.2);

	double AxialD_430_129 = cam0_4_129right_TLineFP[0] - (cam0_4_128right_TLineFP[0] + cam0_4_128right_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_430_129* Calik);
	pushback_vectors(resultVectorList, 4, 129, 2.5, -0.12, +0.12);

	double AxialD_430_132 = atan((cam0_4_132_TLineFP[0] - cam0_4_132_TLineFP[2]) / (cam0_4_132_TLineFP[3] - cam0_4_132_TLineFP[1])) * 180 / PI;
	//resultVectorList.push_back(AxialD_430_132);
	//pushback_vectors(resultVectorList, 3, 132, 45, -1, +1);

	double Chamfer_430_131 = TLineFP_Chamfer_P(Img4Aug,
		cam0_4_132_TLineFP[0] + 10, 1500, cam0_4_132_TLineFP[0] + 70, cam0_4_132_TLineFP[1] - 10,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_430_131* Calik);
	pushback_vectors(resultVectorList, 2, 131, 0.5, -0.2, +0.2);

	double AxialD_430_68 = (cam0_4_128right_TLineFP[0] + cam0_4_128right_TLineFP[2]) / 2 - (cam0_3_60right_TLineFP[0] + cam0_3_60right_TLineFP[2]) / 2;
	//resultVectorList.push_back(AxialD_430_68* Calik + pic3To4_moveDistance);
	//pushback_vectors(resultVectorList, 4, 68, 57.2, -0, +0.05);

	double AxialD_430_61 = (cam0_4_61right_TLineFP[0] + cam0_4_61right_TLineFP[2]) / 2 - (cam0_1_8left_TLineFP[0] + cam0_1_8left_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_430_61* Calik + pic1To2_moveDistance+ pic2To3_moveDistance + pic3To4_moveDistance);
	pushback_vectors(resultVectorList, 4, 61, 217, -0.2, +0.2);

};


double program_9::cam0Picture1_algorithmLuowenDown(HObject ho_Image,  HObject ho_Image_L,double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
...
double program_9::cam0Picture1_algorithmLuowenUp(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
	// Local iconic variables
	HObject  ho_ModelContours, ho_LineContours;
	HObject  ho_LineContour;
...
double program_9::cam0Picture1_L_K_b_Down(HObject ho_Image, double* hv_Box)
{
...
double program_9::cam0Picture1_L_K_b_Up(HObject ho_Image, double* hv_Box)
{
...
HObject program_9::imgAug(string imgPath) {
...
//模板匹配找特征点
void program_9::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
...
//根据起始点与终止点求倒角
double program_9::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
...
double program_9::TLineFP_Chamfer_H2(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
double program_9::TLineFP_Chamfer_H2_117up(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
void program_9::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_9::Straight_TLineFP_P137(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour;
...
void program_9::Straight_TLineFP_P116(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_9::Straight_TLineFP_P170(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_9::Straight_TLineFP_P102(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_9::cam0_roundoutMeasure_prepare(int positionNumber)
{
	//trap5轴
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_9::clearScrewRoundOutData()
{
	for (int i = 0; i < 2; i++)
	{
...
double program_9::cam1Picture_holeAlgorithm1(string imgPath)//远心相机1测孔图像算法
{
	return 0;
};

void program_9::cam1Picture_holeAlgorithm4(string imgPath,int n,vector <double> &container_117, vector <double>& container_118,  vector <double> &container_170)//远心相机2测孔图像算法
{
	string Imgh2path = imgPath;
	HTuple halconPath = Imgh2path.c_str();
	HObject Imgh2_raw;
	ReadImage(&Imgh2_raw, halconPath);
	Straight_TLineFP_P116(cam1_h2_118down, Imgh2_raw,
		1380, 750, 1380, 900,
		30, 12, 1, 30);
	

	double cam1_h2_118up[4];
	Straight_TLineFP_P116(cam1_h2_118up, Imgh2_raw,
		1355, 1171, 1355, 1345,
		30, 12, 1, 30);
	cam1_h2_118down_0[n] = cam1_h2_118up[0];
	cam1_h2_118down_2[n] = cam1_h2_118up[2];

	double cam1_h2_170left[4];
	Straight_TLineFP_P170(cam1_h2_170left, Imgh2_raw,
		1500, 495, 1586, 495,
		30, 12, 1, 1);

	double cam1_h2_170right[4];
	Straight_TLineFP_P170(cam1_h2_170right, Imgh2_raw,
		1500, 1985, 1586, 1985,
		30, 12, 1, 1);

	//以下程序对特征集中处理，不涉及特征点检测
	double Chamfer_430_117down_left = TLineFP_Chamfer_H2(Imgh2_raw,
		940, 250, 1380, 624,
		3, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	container_117.push_back(Chamfer_430_117down_left * CalikKong);

	double Chamfer_430_117down_right = TLineFP_Chamfer_H2(Imgh2_raw,
		926, 1615, 1300, 2040,
		3, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	container_117.push_back(Chamfer_430_117down_right * CalikKong);
	double AxialD_430_118 = (cam1_h2_118down[0] + cam1_h2_118down[2]) / 2 - (cam1_h2_118up[0] + cam1_h2_118up[2]) / 2;
	container_118.push_back(AxialD_430_118 * CalikKong);
	
	
	double AxialD_430_170 = (cam1_h2_170right[1] + cam1_h2_170right[3]) / 2 - (cam1_h2_170left[1] + cam1_h2_170left[3]) / 2 - 30;
	container_170.push_back(AxialD_430_170 * CalikKong);
	
};
void program_9::cam1Picture_holeAlgorithm3(string imgPath)//远心相机4测孔图像算法
{
	string Imgh3path = imgPath;
	HTuple halconPath = Imgh3path.c_str();
	HObject Imgh3_raw;
	ReadImage(&Imgh3_raw, halconPath);
	double cam1_h3_102left[4];
	Straight_TLineFP_P102(cam1_h3_102left, Imgh3_raw,
		1255, 1003, 1073, 1003,
		30, 12, 1, 10);

	double cam1_h3_102right[4];
	Straight_TLineFP_P102(cam1_h3_102right, Imgh3_raw,
		1255, 1355, 1073, 1355,
		30, 12, 1, 10);
	double AxialD_430_102 = (cam1_h3_102right[1] + cam1_h3_102right[3]) / 2 - (cam1_h3_102left[1] + cam1_h3_102left[3]) / 2;
	resultVectorList.push_back(AxialD_430_102 * CalikKong);
};
void program_9::cam1Picture_holeAlgorithm2(string imgPath, int n, vector <double>& container_116,vector <double>& container_117, vector <double>& container_119)
{
	string Imgh4path = imgPath;
	HTuple halconPath = Imgh4path.c_str();
	HObject Imgh4_raw;
	ReadImage(&Imgh4_raw, halconPath);
	double cam1_h4_116up_1[4];
	Straight_TLineFP_P116(cam1_h4_116up_1, Imgh4_raw,
		1190, 1625, 1190, 1751,
		50, 12, 1, 30);

	double cam1_h4_116up_2[4];
	Straight_TLineFP_P116(cam1_h4_116up_2, Imgh4_raw,
		1190, 708, 1190, 860,
		50, 12, 1, 30);

	double cam1_h4_116down[4];
	Straight_TLineFP_P116(cam1_h4_116down, Imgh4_raw,
		1202, 1171, 1202, 1345,
		50, 12, 1, 30);
	...
	//以下程序对特征集中处理，不涉及特征点检测
	double Chamfer_430_117up_left = TLineFP_Chamfer_H2_117up(Imgh4_raw,
		1241, 426, 1600, 870,
		3, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	container_117.push_back(Chamfer_430_117up_left * CalikKong);
	

	double Chamfer_430_117up_right = TLineFP_Chamfer_H2_117up(Imgh4_raw,
		1262, 1438, 1754, 2059,
		3, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	container_117.push_back(Chamfer_430_117up_right * CalikKong);

	double AxialD_430_116 = (cam1_h4_116down[0] + cam1_h4_116down[2]) / 2 - (cam1_h4_116up_2[0] + cam1_h4_116up_2[2]) / 2;
	container_116.push_back(AxialD_430_116 * CalikKong);
	



	double AxialD_430_119 = (cam1_h2_118down_0[n] + cam1_h2_118down_0[n]) / 2 - (cam1_h4_116down[0] + cam1_h4_116down[2]) / 2;
	container_119.push_back(AxialD_430_119 * CalikKong + pichole2to4_moveDistance);
	
};

double program_9::cam1Picture_holeAlgorithm5(string imgPath)//远心相机5测孔图像算法
{
	return 0;
};
...
void program_9::run()
{
	...
	defaultPath = defaultPath + "\\measureData\\WZ10-11-2006-430";
	...
	dataScreenOrg.push_back(lsDiameterResult[0][0]);//30号直径数据处理
	dataScreenOrg.push_back(lsDiameterResult[0][1]);
	dataScreenOrg.push_back(lsDiameterResult[1][0]);
	dataScreenOrg.push_back(lsDiameterResult[1][1]);
	dataScreenOrg.push_back(lsDiameterResult[2][0]);
	...
	dataScreenOrg.push_back(lsDiameterResult[4][0]);//18.1号直径数据处理
	...
	dataScreening_minTwo(dataScreenOrg, resultVectorList);
	dataScreenOrg.push_back(lsDiameterResult[8][0]);//18.2号直径数据处理
	...
	dataScreenOrg.push_back(lsDiameterResult[17][0]);//94.1号直径数据处理
	...
	dataScreenOrg.push_back(lsDiameterResult[19][0]);
	dataScreenOrg.push_back(lsDiameterResult[19][1]);
	dataScreening_maxTwo(dataScreenOrg, resultVectorList);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();
	dataScreenOrg.push_back(lsDiameterResult[20][0]);//94.2号直径数据处理
	...
	dataScreening_maxTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[22][3], lsDiameterResult[22][4], lsDiameterResult[22][6], lsDiameterResult[22][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();

	dataScreenOrg.push_back(lsDiameterResult[24][0]);//168号直径数据处理
	...
	dataScreenOrg.push_back(lsDiameterResult[26][0]);
	dataScreenOrg.push_back(lsDiameterResult[26][1]);
	dataScreening_maxTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[26][3], lsDiameterResult[26][4], lsDiameterResult[26][6], lsDiameterResult[26][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();

	dataScreenOrg.push_back(lsDiameterResult[27][0]);//155号直径数据处理
	...
	dataScreenOrg.push_back(lsDiameterResult[29][0]);
	dataScreenOrg.push_back(lsDiameterResult[29][1]);
	dataScreening_minTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[29][3], lsDiameterResult[29][4], lsDiameterResult[29][6], lsDiameterResult[29][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();

	dataScreenOrg.push_back(lsDiameterResult[31][0]);//147号直径数据处理
	...
	dataScreening_minTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[33][3], lsDiameterResult[33][4], lsDiameterResult[33][6], lsDiameterResult[33][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();

	cam2_radius[0][1] = lsDiameterResult[3][9];//传递旋转半径给粗糙度测量参数
	cam2_radius[1][1] = lsDiameterResult[11][9];
	cam2_radius[2][1] = lsDiameterResult[14][9];
	...
	for (int i = 0; i < 3; i++)
	{
		emit programProcess(QString("正在对 %1 号特征（粗糙度） 进行测量 ！").arg(roughnessResult[i][0]), 19 + i * 2);
	...
	camPtrList[2]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[2]->capturedImg);
    emit programProcess(QString("正在对 %1 号特征（粗糙度） 进行测量 ！").arg(roughnessResult[3][0]), 19 + 3 * 2);
    cam2_Measure_prepare(3);
	...
	camPtrList[0]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[0]->capturedImg);
	for (int i = 0; i < 4; i++)//远心1-4号采集位置采集图像
	{
		camPtrList[0]->setExposeTime(cam0_exposeTime);
		camPtrList[0]->m_captureMode = "continuous";
		originalImgPtr = &(camPtrList[0]->capturedImg);
	...
	//跳动&圆柱度&孔径的测量（再次下行）***************************************************************************************
	emit programTips("即将进行跳动、圆柱度以及孔径检测，请安装机心夹！", 3);
	//暂停部分
	programConfirm = "doNotContinue";
	do
	{
	...
	//圆柱度测量采集测量
	emit programProcess(QString("正在对 %1 号特征（圆柱度） 进行数据采集！").arg(lsCylindricityResult[0][0]), 81 );
	...
	axisLineCalculate(referencePoint, loopTime, directionVector, aixsReferancePoint);
	for (int i = 0; i < 2; i++)//跳动计算
	{
		switch (i) {
		case 0:
			break;
		default:
			for (int j = 0; j < 3; j++)
			{
				axisLoaction(i, j, directionVector, aixsReferancePoint);
	...
	*/
	//螺纹中径******************************************************************************
	emit updateDeviceInf(QString("正在进行上部螺纹测量"));//螺纹测量位置1
	cam0_exposeTime = 14;
	camPtrList[0]->setExposeTime(cam0_exposeTime);
	camPtrList[0]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[0]->capturedImg);
	cam0_roundoutMeasure_prepare(1);
	cout << "螺纹中径数据收集完了" << endl;
	
	int cam0_roundoutImgNumber = myScrewRoundoutDataPtr[1]->screwImgPath.size();
	int looptime = 0;

	HTuple PatternlUOWENpath = "./programParmeter/WZ10-11-2006-430/cam0_4-430-131.sbm";
	double cam0_4_luowenchuanru_TLineFP[4];
	for (int i = 0; i < cam0_roundoutImgNumber; i++)
	{
		try {
			//cout << "L圆心图像路径：" << myScrewRoundoutDataPtr[0]->screwImgPath[i] << endl;
			TemplateMatching_TLineFP_P(cam0_4_luowenchuanru_TLineFP, imgAug(myScrewRoundoutDataPtr[1]->screwImgPath[i]),
				PatternlUOWENpath,
				-175, 50, -55, 50,
				50, 12, 1, 50,
				0.5, 1, 0.3, 0, 0.7);
			double luowen_guangmu_chuanru = cam0_4_luowenchuanru_TLineFP[3];
			double luowen_guangmu_chuanruYYYY = cam0_4_luowenchuanru_TLineFP[2];
			//cout << "图像序号i" << i << endl;
			...
			cout << "光幕测得直径: " << myScrewRoundoutDataPtr[1]->diameter[i] << endl;
			double d = cam0Picture1_algorithmLuowenUp(imgAug(myScrewRoundoutDataPtr[1]->screwImgPath[i]), imgAug(myScrewRoundoutDataPtr[1]->screwImgPath[i]), myScrewRoundoutDataPtr[1]->diameter[i], myScrewRoundoutDataPtr[1]->out2[i], i, luowen_guangmu_chuanru);
	...
	emit updateDeviceInf(QString("正在进行下部螺纹测量"));
	cam0_exposeTime = 14;
	camPtrList[0]->setExposeTime(cam0_exposeTime);
	camPtrList[0]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[0]->capturedImg);
	...
	cout << "螺纹中径数据收集完了" << endl;
	cam0_roundoutImgNumber = myScrewRoundoutDataPtr[0]->screwImgPath.size();
	looptime = 0;
	PatternlUOWENpath = "./programParmeter/WZ10-11-2006-430/cam0_1-430-5.sbm";
	...
	for (int i = 0; i < cam0_roundoutImgNumber; i++)
	{
		try {
			...
			double d = cam0Picture1_algorithmLuowenDown(imgAug(myScrewRoundoutDataPtr[0]->screwImgPath[i]), imgAug(myScrewRoundoutDataPtr[0]->screwImgPath[i]), myScrewRoundoutDataPtr[0]->diameter[i], myScrewRoundoutDataPtr[0]->out2[i], i, luowen_guangmu_chuanru);
			//double d = cam0Picture1_algorithmLuowen(imgAug(myScrewRoundoutDataPtr[0]->screwImgPath[i]), imgAug(myScrewRoundoutDataPtr[0]->referanceImgPath[i]), myScrewRoundoutDataPtr[0]->diameter[i], myScrewRoundoutDataPtr[0]->out2[i], i, luowen_guangmu_chuanru);
			//double d = cam0Picture1_algorithmLuowen( imgAug(myScrewRoundoutDataPtr[0]->referanceImgPath[i]), imgAug(myScrewRoundoutDataPtr[0]->screwImgPath[i]), myScrewRoundoutDataPtr[0]->diameter[i], myScrewRoundoutDataPtr[0]->out2[i], i, luowen_guangmu_chuanru);
	...
	if (looptime == cam0_roundoutImgNumber)
	{
		...
		cout << "中径均值" << meanValue << "  中径最大值" << screwResultMax << "  中径最小值" << screwResultMin << endl;
		resultVectorList.push_back(meanValue);
		pushback_vectors(resultVectorList, 11, 31, 59.234, -0.08, +0);
		screwResult.clear();
		screwResult.shrink_to_fit();
	}
	else {
	...
	//第一个孔径不测
	cam1_exposeTime = 700;
	camPtrList[1]->setExposeTime(cam1_exposeTime);//孔径
	camPtrList[1]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[1]->capturedImg);
	emit programProcess("正在对 孔径 进行测量！", 68);
	cam1_Measure_prepare(4);
	for (int i = 0; i < cam1PathList.size(); i++)
	{
		try {
			//cam1Picture_holeAlgorithm(cam1PathList[i]);
		}
		catch (...)
		{
			emit updateDeviceInf("孔图像处理执行错误");
		};
	}
	*/


	cam1_exposeTime = 1000;
	camPtrList[1]->setExposeTime(cam1_exposeTime);//槽宽
	camPtrList[1]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[1]->capturedImg);
	emit programProcess("正在对 槽宽 进行测量！", 68);
	cam1_Measure_prepare(3);
	vector <double> result_117;//用于暂时存储117号特征结果（该特征后面还会用到）
	vector <double> result_118;//用于暂时存储118号特征结果（）
	vector <double> result_170;//用于暂时存储170号特征结果（）
	for (int i = 0; i < cam1PathList.size(); i++)
	{
		
		try {
			cam1Picture_holeAlgorithm4(cam1PathList[i],i, result_117, result_118, result_170 );
		}
		catch (...)
		{
			emit updateDeviceInf("镂空补槽宽处理执行错误");
		};
	}
	//pushback_vectors(result_118, 4, 118, 0.2, -0.1, +0.1);
	pushback_vectors(result_170, 4, 170, 10.2, -0.2, +0.2);
	

	cam1_exposeTime = 550;
	camPtrList[1]->setExposeTime(cam1_exposeTime);//镂空部分棱宽
	camPtrList[1]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[1]->capturedImg);
	emit programProcess("正在对 镂空部分棱宽 进行测量！", 68);
	cam1_Measure_prepare(2);
	for (int i = 0; i < cam1PathList.size(); i++)
	{
		try {
			cam1Picture_holeAlgorithm3(cam1PathList[i]);
		}
		catch (...)
		{
			emit updateDeviceInf("镂空部分棱宽执行错误");
		};
	}
	pushback_vectors(resultVectorList, 4, 102, 2.4, -0.08, +0);

	cam1_exposeTime = 1500;
	camPtrList[1]->setExposeTime(cam1_exposeTime);//镂空部分倒角
	camPtrList[1]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[1]->capturedImg);
	vector <double> result_116;//用于暂时存储118号特征结果
	vector <double> result_119;//用于暂时存储118号特征结果（）
	emit programProcess("正在对 镂空部分倒角 进行测量！", 68);
	cam1_Measure_prepare(1);
	pichole2to4_moveDistance = cam1_axis5Place[3] - cam1_axis5Place[1];
	cout << "2-4图像距离" << pichole2to4_moveDistance << endl;
	for (int i = 0; i < cam1PathList.size(); i++)
	{
		try {
			cam1Picture_holeAlgorithm2(cam1PathList[i],i, result_116,result_117, result_119);
		}
		catch (...)
		{
			emit updateDeviceInf("镂空部分倒角图像处理计算错误");
		};
	}
	//pushback_vectors(result_116, 4, 116, 0.2, -0.1, +0.1);
	pushback_vectors(result_117, 2, 117, 3, -0.5, +0.5);
	//pushback_vectors(result_119, 4, 119, 29, -0, +0.2);
	/*
	//该特征不测
	cam1_exposeTime = 700;
	camPtrList[1]->setExposeTime(cam1_exposeTime);//凹槽部分棱宽
	camPtrList[1]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[1]->capturedImg);
	emit programProcess("正在对 凹槽部分棱宽 进行测量！", 68);
	...
	for (int i = 0; i < cam1PathList.size(); i++)
	{
		...
		{
			emit updateDeviceInf("凹槽部分棱宽执行错误");
		};
	}
	*/
