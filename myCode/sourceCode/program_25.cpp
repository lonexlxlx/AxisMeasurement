...
program_25::program_25(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
...
bool program_25::stepLengthCompensationCaculation_axis5()
{
...
void program_25::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
void program_25::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime = 50)
{
	//初始化变量
...
void program_25::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_25::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_25::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_25::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_25::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_25::clearRoundoutData()
{
...
//***********************************************************************************************************************************************************************************************************************************************
string program_25::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_25::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_25::cam2_Measure_prepare(int positionNumber)
{
...
void program_25::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_25::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_25::creatDatabaseTable()
{
...
void program_25::writeToDatabase()
{
	/*
...
void program_25::zeroMeasureNub()
{
	allFeatureFlag = "--";
	ngFeatureNum = 0;//NG特征数量
	measurePartsNum_all = 0;//该型号轴测量总数
...
void program_25::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
//远心相机图像检测算法
void program_25::cam0Picture_algorithm()//远心相机1图像算法
{
...
//螺纹中径计算函数
double program_25::cam0Picture1_algorithmLuowen(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru, vector <double> luoju_P, vector <double> yadi_R, vector <double> yaxingbanjiao, vector <double> xiaojing_d)//测螺纹
{
	...
	AddMetrologyObjectGeneric(hv_MetrologyHandle, "line", ((((((hv_Line1.TupleConcat(hv_Line2)).TupleConcat(hv_Line3)).TupleConcat(hv_Line4)).TupleConcat(hv_Line5)).TupleConcat(hv_Line6)).TupleConcat(hv_Line7)).TupleConcat(hv_Line8),
		20, 12, 1, 18, HTuple(), HTuple(), &hv_LineIndices);//不需要改动
	...
	FindShapeModel(ho_Image, hv_ReusedModelID, hv_AngleStart, hv_AngleExtent, 0.5,
		1, 0.3, "least_squares", 0, 0.5, &hv_Row3, &hv_Column3, &hv_Angle, &hv_Score);//不需要改动
	//显示形状特征模板匹配结果
	...
	GenRectangle1(&ho_Rectangle, 0, 0, 60, 60);
	HomMat2dIdentity(&hv_HomMat2DIdentity);
	HomMat2dTranslate(hv_HomMat2DIdentity, hv_Column3 + 1180, hv_Row3 - 1310, &hv_HomMat2DTranslate);
	...
	SelectContoursXld(ho_ContoursSplit, &ho_SelectedContours, "contour_length", 20,hv_Width / 2, -0.5, 0.5);
	UnionAdjacentContoursXld(ho_SelectedContours, &ho_UnionContours, 40, 1, "attr_keep");
	FitCircleContourXld(ho_Edges, "algebraic", 20, 0, 0, 3, 2, &hv_Row, &hv_Column,&hv_Radius, &hv_StartPhi, &hv_EndPhi, &hv_PointOrder);
	TupleMax(hv_Radius, &hv_Max);*/
	//*******************************///////底角圆检测函数，对应模板匹配找倒角函数 END
	// 
	//对测量模型执行变换
	...
	{
		cout <<"hv_RowBegin.Length():"<< t << endl;
		if (t < 5) {
	...
	//--------------计算中径x轴的纵坐标------------------------//
	double X_jiexian = (Pix_luowen - cam0_1_460_luowenP_left_x_TLineFP[0]) / 2 + cam0_1_460_luowenP_left_x_TLineFP[0];
	double Y_jie1 = k1 * X_jiexian + b1;
	...
	/*****************************/
	double hv_Box[3];
	//到这一步之后，CTRL+左键点击该函数名（cam0Picture1_L_K_b）进入函数体
	...
	////牙型半角
	//duoble yaxingjiao=0;
	//yaxingjiao = (atan2(hv_RowBegin[1].D() - hv_RowEnd[1].D()) / (hv_ColEnd[1].D() - hv_ColBegin[1].D()) + atan2((hv_RowEnd[4].D()- hv_RowBegin[4].D())/(hv_ColEnd[4].D() - hv_ColBegin[4].D()))) / 2;
	//yaxingbanjiao.push_back(yaxingjiao);
	//牙底角
	//double dijiao = 0;
	//dijiao = hv_Radius.D();
	//yadi_R.push_back(dijiao);
	//小径
	//double xiaojing=0;
	//xiaojing=L-(Pix_zhoujing_chuanru-hv_Column..D())*Calik*2;
	//xiaojing_d.push_back(xiaojing);
...
double program_25::cam0Picture1_L_K_b(HObject ho_Image, double* hv_Box)
{
	...
	hv_ModelFile = "./programParmeter/WZ10-11-2006-430/cam0_4-430-131.sbm";//参考直线匹配模板，需要改动
	ReadShapeModel(hv_ModelFile, &hv_ReusedModelID);
	//创建测量模型	
	CreateMetrologyModel(&hv_MetrologyHandle);
	//检测参考直线上的两个点（中等长度即可），需要改动
	hv_Line.Clear();
	hv_Line[0] = -175;
	hv_Line[1] = 50;
	hv_Line[2] = -55;
...
HObject program_25::imgAug(string imgPath) {
...
//模板匹配找特征点
void program_25::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
...
//根据起始点与终止点求倒角
double program_25::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
...
double program_25::TLineFP_Chamfer_H2(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
double program_25::TLineFP_Chamfer_H2_117up(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
void program_25::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_25::Straight_TLineFP_P137(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour;
...
void program_25::Straight_TLineFP_P116(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_25::Straight_TLineFP_P170(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_25::Straight_TLineFP_P102(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_25::cam0_roundoutMeasure_prepare(int positionNumber)
{
	//trap5轴
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_25::clearScrewRoundOutData()
{
	//需要将for循环中的边界条件修改为进行螺纹测量的位置总数
	for (int i = 0; i < 1; i++)
	{
...
double program_25::cam1Picture_holeAlgorithm0(string imgPath)//H0孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_25::cam1Picture_holeAlgorithm1(string imgPath)//H1孔图像算法
{
	//在下方贴入孔径图像处理代码
...
double program_25::cam1Picture_holeAlgorithm2(string imgPath)//H2孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_25::cam1Picture_holeAlgorithm3(string imgPath)//H3孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_25::cam1Picture_holeAlgorithm4(string imgPath)//H4孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
void program_25::run()
{
	...
	clearScrewRoundOutData();//需要进入该函数并修改该函数的具体内容
	...
	defaultPath = defaultPath + "\\measureData\\ceship11";//！需要将文件后夹后缀名改为对应的零件号
	...
	*/
	...
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
	for (int i = 0; i < 13; i++)//第一次测量（向上移动）（需要将i修改为直径采集的总次数）
	{
	...
	for (int i = 12; i >= 0; i--)//第二次测量（向下移动）（需要将i修改为直径采集的总次数-1）
	{
		...
		case 3://10号直径特征（后续取最小值）
			break;
		case 4:
			break;
		case 5:
			break;
		case 7://10号直径特征（后续取最小值）
			break;
		case 8:
			break;
		case 9:
	...
	//10号直径数据处理取最小值输出
	dataScreenOrg.push_back(lsDiameterResult[3][0]);//10.1号直径数据处理
	...
	//10号直径选最小值输出
	dataScreenOrg.push_back(lsDiameterResult[7][0]);//10.2号直径数据处理
	...
	dataScreening_minTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[7][3], lsDiameterResult[7][4], lsDiameterResult[7][6], lsDiameterResult[7][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();


	//需要将直径实际测量结果传递给粗糙度用于补偿粗糙度轴移动位置
	cam2_radius[0][1] = lsDiameterResult[4][9];//传递旋转半径d4给粗糙度0号采集位置（如有新增复制本句代码）进行相应修改即可
	/*
	新增参考代码
	...
	*/
	...
	//第一步采各跳动测量轴段的基本数据
	for (int i = 4; i >= 0; i--)//只需要修改i值为跳动数据采集的轴段的总数-1（该循环中的其他部分不需要修改）
	{
	...
	for (int i = 0; i < 5; i++)//（只需要修改i值为跳动数据采集的轴段的总数）
	{
		for (int j = 0; j < 3; j++)
		{
			centerCosFit(i, j);
	...
	for (int i = 0; i < 5; i++)//需要修改i值为跳动数据采集的轴段的总数
	{
	...
	*/
	...
	//emit updateDeviceInf(QString("正在进行螺纹测量"));//螺纹测量位置1
	//cam0_exposeTime = 14;
	//camPtrList[0]->setExposeTime(cam0_exposeTime);
	//camPtrList[0]->m_captureMode = "continuous";
	//originalImgPtr = &(camPtrList[0]->capturedImg);

	////函数cam0_roundoutMeasure_prepare是用来自动控制设备对零件螺纹进行数据采集的函数，该函数参数需要修改，其值等于螺纹测量位置序号-1，示例如下
	////cam0_roundoutMeasure_prepare(0);表明对第一个螺纹测量位置进行原始数据采集
	//cam0_roundoutMeasure_prepare(0);
	//cout << "螺纹中径数据收集完了" << endl;

	//int cam0_roundoutImgNumber = myScrewRoundoutDataPtr[0]->screwImgPath.size();
	//int looptime_luowen = 0;

	//HTuple PatternlUOWENpath = "./programParmeter/luowenjiaoxue/cam0_CANKAOZHIXIAN.sbm";//参考直线模板路径，需要改动
	//double cam0_4_luowenchuanru_TLineFP[4];
	//for (int i = 0; i < cam0_roundoutImgNumber; i++)
	//{
	//	try {
	//		// myScrewRoundoutDataPtr[0]->screwImgPath[i] 为参考直线图像路径，不需要改动
	//		TemplateMatching_TLineFP_P(cam0_4_luowenchuanru_TLineFP, imgAug(myScrewRoundoutDataPtr[0]->screwImgPath[i]),//该函数检测参考直线上的两个点，后续用于计算直线方程，需要改动
	//			PatternlUOWENpath,
	//			180, 80, 1250, 80,
	//			50, 12, 1, 50,
	//			0.5, 1, 0.3, 0, 0.7);
	...
	//		//cam0Picture1_algorithmLuowen函数为检测螺纹中径的主函数，到这一步后，CTRL+左键点击函数名，进入函数体中
	//		double d = cam0Picture1_algorithmLuowen(imgAug(myScrewRoundoutDataPtr[0]->screwImgPath[i]), imgAug(myScrewRoundoutDataPtr[0]->screwImgPath[i]), myScrewRoundoutDataPtr[0]->diameter[i], myScrewRoundoutDataPtr[0]->out2[i], i, luowen_guangmu_chuanru, luoju_P, yadi_R, yaxingbanjiao, xiaojing_d);
	//		/// <summary>
	//		/// cam0Picture1_algorithmLuowen函数中最后四个vector传出来需要修改
	//		/// </summary>
	//		cout << "螺纹中径d: " << d << endl;
	//		looptime_luowen++;
	//		cout << "螺纹中径：" << d << "当前测量次数" << looptime_luowen << endl;
	...
	//	}
	//};
	//cout << "螺纹测量次数" << looptime_luowen << endl;
	//if (looptime_luowen == cam0_roundoutImgNumber)
	//{
	//	cout << "中径数据处理1" << "screwResult.size()" << screwResult.size() << endl;
	...
	//	//需要将pushback_vector函数中的参数修改为工艺卡中的数值，具体修改方式参照入如下
	//	//pushback_vectors(resultVectorList, 11, 77, 38.771, -0.094, +0);第一个参数resultVectorList不变，第二个参数11不变，第三个参数为特征编号，第四个参数为公称值，第五个参数为公差下限，第六个参数为公差上限
	//	pushback_vectors(resultVectorList, 11, 77, 38.771, -0.094, +0);

	//	//以下两行是对螺纹跳动进行计算，如不需要需要将下面两行注释掉
	//	//pushback_vectors(resultVectorList, 5, 66, 0.05, -0.01, +0);第一个参数resultVectorList不变，第二个参数5不变，第三个参数为特征编号，第四个参数为公称值，第五个参数为公差下限，第六个参数为公差上限
	//	resultVectorList.push_back((screwResultMax - screwResultMin) / 2);
	//	pushback_vectors(resultVectorList, 5, 66, 0.05, -0.01, +0);
