...
program_11::program_11(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
	...
	//跳动数据结构体初始化
	for (int i = 0; i < 6; i++)//如有跳动测量需求将i的循环上限修改为需要进行跳动采集的轴号
	{
...
bool program_11::stepLengthCompensationCaculation_axis5()
{
...
void program_11::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
void program_11::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime = 50)
{
	//初始化变量
...
void program_11::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_11::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_11::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_11::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_11::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_11::clearRoundoutData()
{
	for (int i = 0; i < 4; i++)
	{
...
//***********************************************************************************************************************************************************************************************************************************************
string program_11::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_11::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_11::cam2_Measure_prepare(int positionNumber)
{
...
void program_11::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_11::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_11::creatDatabaseTable()
{
...
void program_11::writeToDatabase()
{
	/*
...
void program_11::zeroMeasureNub()
{
	allFeatureFlag = "--";
	ngFeatureNum = 0;//NG特征数量
	measurePartsNum_all = 0;//该型号轴测量总数
...
void program_11::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
//远心相机图像检测算法
void program_11::cam0Picture_algorithm()//远心相机1图像算法
{
...
double program_11::cam0Picture1_algorithmLuowenDown(HObject ho_Image,  HObject ho_Image_L,double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
...
double program_11::cam0Picture1_algorithmLuowenUp(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
	// Local iconic variables
	HObject  ho_ModelContours, ho_LineContours;
	HObject  ho_LineContour;
...
double program_11::cam0Picture1_L_K_b_Down(HObject ho_Image, double* hv_Box)
{
...
double program_11::cam0Picture1_L_K_b_Up(HObject ho_Image, double* hv_Box)
{
...
HObject program_11::imgAug(string imgPath) {
...
//模板匹配找特征点
void program_11::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
...
//根据起始点与终止点求倒角
double program_11::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
...
double program_11::TLineFP_Chamfer_H2(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
double program_11::TLineFP_Chamfer_H2_117up(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
void program_11::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_11::Straight_TLineFP_P137(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour;
...
void program_11::Straight_TLineFP_P116(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_11::Straight_TLineFP_P170(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_11::Straight_TLineFP_P102(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_11::cam0_roundoutMeasure_prepare(int positionNumber)
{
	//trap5轴
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_11::clearScrewRoundOutData()
{
	for (int i = 0; i < 2; i++)
	{
...
double program_11::cam1Picture_holeAlgorithm1(string imgPath)//远心相机1测孔图像算法
{
	return 0;
};


double program_11::cam1Picture_holeAlgorithm2(string imgPath, int n, vector <double>& container_116,vector <double>& container_117, vector <double>& container_119)
{
	return 0;
};
...
void program_11::run()
{
	...
	defaultPath = defaultPath + "\\measureData\\WZ10451080-370-1";//将文件后夹后缀名改为对应的零件号
	...
	*/
	...
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
	for (int i = 0; i < 6; i++)//第一次测量（向上移动）（将i修改为直径采集的总次数）
	{
	...
	for (int i = 5; i >= 0; i--)//第二次测量（向下移动）（将i修改为直径采集的总次数-1）
	{
		...
		/*case 0://88号直径特征（后续取最大值）
			break;
		case 1:
			break;
		case 2:
			break;*/
		case 4://89号直径特征（后续取最小值）
			break;
		case 5:
			break;
		case 3:
			break;
		default://其余的直径按照正常方式输出
	...
	pushback_vectors(resultVectorList, 1, lsDiameterResult[5][3], lsDiameterResult[5][4], lsDiameterResult[5][6], lsDiameterResult[5][5]);
	dataScreenOrg.clear();
	...
	*/
	...
	cam2_radius[0][1] = lsDiameterResult[1][9];
	cam2_radius[1][1] = lsDiameterResult[2][9];//传递旋转半径d4给粗糙度0号采集位置（如有新增复制本句代码）进行相应修改即可
	/*
	新增参考代码
	...
	*/
	...
	//粗糙度测量部分
	camPtrList[2]->setExposeTime(cam2_exposeTime);
	camPtrList[2]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[2]->capturedImg);
	for (int i = 0; i < 2; i++)//修改i为粗糙度测量点位的总数量
	{
		switch (i)
		{
		case 1://设置粗糙度测量点位Ra0的曝光
			cam2_exposeTime = 500;
			break;
		//如需对其他点位如Ra1进行曝光设置，请参照如下代码
		/*
		//Ra1参考代码
		case 1:
			cam2_exposeTime = 550;
	...
	camPtrList[0]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[0]->capturedImg);
	for (int i = 0; i < 3; i++)//远心0-1号采集位置采集图像（将i修改为远心相机的拍照位置总数，循环中的其他语句不需要修改）
	{
		camPtrList[0]->setExposeTime(cam0_exposeTime);
		camPtrList[0]->m_captureMode = "continuous";
		originalImgPtr = &(camPtrList[0]->capturedImg);
	...
	//计算拍照之间的距离
	double pic0To1_moveDistance;
	pic0To1_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[1]) - axis5_compensation(cam0_arriveOrgEncode_axis5[0]);//计算拍照位置0和拍照位置1之间的距离(减法代表拍照位置2-拍照位置1)
	double pic0To2_moveDistance;
	pic0To2_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[2]) - axis5_compensation(cam0_arriveOrgEncode_axis5[0]);
																																 /*
	...
	*/
	try {
		cam0Picture_algorithm();//远心相机的图像处理函数
	}
	catch (...) {
	...
	emit programProcess(QString("正在对 （圆柱度） 进行数据采集！"), 81 );
	lsSensorMeasure_cylindricity(0);//圆柱度测量函数，此处表明对为圆柱度测量位置C0进行圆柱度测量,如有多个位置有圆柱度测量要求可以参照下面代码新加
	lsSensorMeasure_cylindricity(1);
									/*
	lsSensorMeasure_cylindricity(0);//对圆柱度测量位置C1进行圆柱度测量
	*/

	//孔径相机测量
	cam1_exposeTime = 550;//孔径相机测量曝光设置（按照需要进行修改）其他没有注释的语句不需要修改
	...
	cam1_Measure_prepare(0);//对孔径拍摄位置H0进行测量（这个函数参数为0）
	for (int i = 0; i < cam1PathList.size(); i++)
	{
		try {
			//cam1Picture_holeAlgorithm0(cam1PathList[i]);//孔径相机拍摄位置H0的图像处理函数（注意函数名字）
		}
		catch (...)
		{
	...
	/*
	如需要添加其他孔径位置如H1测量可参考此部分代码
	cam1_exposeTime =470;//孔径相机测量曝光设置（按照需要进行修改）其他没有注释的语句不需要修改
	camPtrList[1]->setExposeTime(cam1_exposeTime);
	camPtrList[1]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[1]->capturedImg);
	emit programProcess("正在对 孔径 进行测量！", 68);
	cam1_Measure_prepare(1);//对孔径拍摄位置H1进行测量（这个函数参数为0）
	for (int i = 0; i < cam1PathList.size(); i++)
	{
		try {
			//cam1Picture_holeAlgorithm1(cam1PathList[i]);//孔径相机拍摄位置H1的图像处理函数
		}
		catch (...)
		{
			emit updateDeviceInf("孔图像处理执行错误");
	...
	*/
	...
	//第三步选择跳动基准面的圆心数据并计算拟合的基准轴线（如不需要拟合基准轴线可以直接注释掉这部分代码）
	double referencePoint[6][3];//修改数组第一个长度=基准数量*3
	int loopTime = 0;
	for (int j = 0; j < 3; j++)
	{
		copy(begin(roundoutDataPtr[0]->axisPoint[j]), end(roundoutDataPtr[0]->axisPoint[j]), begin(referencePoint[loopTime]));//提取第一个基准上三个数据采集截面的圆心数据
		loopTime++;
		copy(begin(roundoutDataPtr[5]->axisPoint[j]), end(roundoutDataPtr[5]->axisPoint[j]), begin(referencePoint[loopTime]));//提取第二个基准截面上的圆心数据
		loopTime++;
	}
	double directionVector[3];
	double aixsReferancePoint[3];
	axisLineCalculate(referencePoint, loopTime, directionVector, aixsReferancePoint);//拟合基准轴线

	//第4步跳动的计算
	vector <double> lsRoundout_forRank;
	for (int i = 0; i < 6; i++)//需要修改i值为跳动数据采集的轴段的总数
	{
		//对于基准采集位置不进行跳动计算，所以使用case跳过
		switch (i) {
		case 0://跳动计算轴段0（需要跳过的基准轴段）
			break;
		case 1://跳动计算轴段1（以A-B为基准）
			for (int j = 0; j < 3; j++)//依次对三个截面进行跳动计算(case中的其余位置不需要修改)
			{
		...
		case 2://跳动计算轴段2（以A-B为基准）
			...
			pushback_vectors(resultVectorList, 5, lsRoundoutResult[i][0], 0, 0, lsRoundoutResult[i][2]);
			break;
		case 3://跳动计算轴段3（以顶尖为基准）
			for (int j = 0; j < 3; j++)//依次对三个截面进行跳动计算(case中的其余位置不需要修改)
			{
				axisLoaction(i, j, directionVector, aixsReferancePoint);
				roundoutCalculate(i, j,1);
			};
			lsRoundout_forRank.push_back(roundoutDataPtr[i]->roundoutResult[0]);
			lsRoundout_forRank.push_back(roundoutDataPtr[i]->roundoutResult[1]);
			lsRoundout_forRank.push_back(roundoutDataPtr[i]->roundoutResult[2]);
