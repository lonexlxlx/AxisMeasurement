...
program_3::program_3(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
...
void program_3::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
/*
void program_3::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime=100)
{
	//初始化变量
	...
	for (int i = 0; i < 3; i++)
	{
		...
		moveControlPtr->setTrapPrm(moveControlPtr->axisCore[moveControlPtr->currentAxisIndex], moveControlPtr->currentAxisNumber, moveControlPtr->trapAuto[moveControlPtr->currentAxisIndex], lsCylindricitySteplength_axis5[positionNumber][i], moveControlPtr->trapHighVelAuto[moveControlPtr->currentAxisIndex]);
		moveControlPtr->startTrap();
		do
		{
			moveControlPtr->updateAxisStatus(moveControlPtr->currentAxisNumber);
	...
	pushback_vectors(lsCylindricityResult[positionNumber][1], 6, lsCylindricityResult[positionNumber][0], 0, 0, lsCylindricityResult[positionNumber][2]);
	//便于调试观察(该部分代码已删除，如需调试查看数据请从其他程序复制粘贴)
...
void program_3::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_3::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_3::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_3::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_3::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_3::clearRoundoutData()
{
	for (int i = 0; i < 4; i++)
	{
	...
	fill(&directionVector_N[0], &directionVector_N[0] + sizeof(directionVector_N) / sizeof(double), 0);
	fill(&aixsReferancePoint_N[0], &aixsReferancePoint_N[0] + sizeof(aixsReferancePoint_N) / sizeof(double), 0);
};
...
string program_3::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_3::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_3::cam2_Measure_prepare(int positionNumber)
{
	...
	if (abs(axis1_steplength - cam2_parpare[positionNumber][1]) >= 10000)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			replace(roughnessPicPath[i].begin(), roughnessPicPath[i].end(), '\\', '/');
		};
		*/
		do
		{
...
void program_3::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_3::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_3::creatDatabaseTable()
{
...
void program_3::writeToDatabase()
{
	/*
...
void program_3::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
函数作用：模板匹配找特征点
*/
void program_3::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
...
//模板匹配找特征点
void program_3::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4,
...
//模板匹配求倒角
double program_3::TemplateMatching_Chamfer_P(HObject ho_Image, HTuple hv_ModelFile, double ShapeModel1, double ShapeModel2, double ShapeModel3, double ShapeModel4, double ShapeModel5,
...
//根据起始点与终止点求倒角
double program_3::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
HObject program_3::imgAug(string imgPath) {
...
函数作用：远心相机视觉算法
*/
void program_3::cam0Picture_algorithm()
{
	...
	HObject Img1_61right;
	ReadImage(&Img1_61right, halconPath);

	halconPath = Img2path.c_str();
	HObject Img2_35;
	ReadImage(&Img2_35, halconPath);

	//cout << "图像地址1" << Img1Aug << endl;
	//cout << "图像地址2" << Img1Aug << endl;
	//cout << "图像地址3" << Img1Aug << endl;
	int fault_detect = 0;

	fault_detect = 1;
	HTuple Pattern1path = "./programParmeter/WZ10-15-1107-141/cam0_1-141-99.sbm";
	double cam0_1_64_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_64_TLineFP, Img1Aug,
		Pattern1path,
		-1320, -105, -1270, -50,
		50, 12, 1, 40,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_61right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_61right_TLineFP, Img1_61right,
		Pattern1path,
		-1320, -160, -1320, -70,
		30, 12, 1, 20,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_61left_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_61left_TLineFP, Img1Aug,
		Pattern1path,
		-1030, 40, -830, 40,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_63_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_63_TLineFP, Img1Aug,
		Pattern1path,
		-1080, -60, -980, 50,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_102_100right_chamfer101_2begin_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_102_100right_chamfer101_2begin_TLineFP, Img1Aug,
		Pattern1path,
		-410, 40, -330, -30,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	...
	double cam0_1_chamfer101_2end_chamfer101_1begin_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_chamfer101_2end_chamfer101_1begin_TLineFP, Img1Aug,
		Pattern1path,
		-300, -60, -110, -60,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_97_100left_chamfer101_1end_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_97_100left_chamfer101_1end_TLineFP, Img1Aug,
		Pattern1path,
		-50, 00, 15, 77,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_99left_chamfer98_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_99left_chamfer98_TLineFP, Img1Aug,
		Pattern1path,
		170, 60, 290, 60,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	...
	fault_detect = 2;
	HTuple Pattern2path = "./programParmeter/WZ10-15-1107-141/cam0_2-141-52.sbm";
	double cam0_2_52right_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_52right_TLineFP, Img2Aug,
		Pattern2path,
		-600, 140, -410, 140,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_52left_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_52left_TLineFP, Img2Aug,
		Pattern2path,
		400, 140, 580, 140,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_53right_chamfer54_2begin_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_53right_chamfer54_2begin_TLineFP, Img2Aug,
		Pattern2path,
		560, 120, 620, 30,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_chamfer54_2end_chamfer54_1begin_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_chamfer54_2end_chamfer54_1begin_TLineFP, Img2Aug,
		Pattern2path,
		700, -30, 1090, -30,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_47right_53left_chamfer54_1end_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_47right_53left_chamfer54_1end_TLineFP, Img2Aug,
		Pattern2path,
		1170, -10, 1190, 40,
		50, 12, 1, 20,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_53_2_chamfer_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_53_2_chamfer_TLineFP, Img2Aug,
		Pattern2path,
		-645, 30, -600, 130,
		50, 12, 1, 20,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_47left_50_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_47left_50_TLineFP, Img2Aug,
		Pattern2path,
		1190, 130, 1250, 200,
		50, 12, 1, 40,
		0.5, 1, 0.3, 0, 0.7);

	HTuple Pattern3path = "./programParmeter/WZ10-15-1107-141/cam0_2-141-89.sbm";
	double cam0_2_90right_chamfer94begin_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_90right_chamfer94begin_TLineFP, Img2Aug,
		Pattern2path,
		1220, 190, 1480, 190,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_chamfer94end_chamfer87_3begin_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_chamfer94end_chamfer87_3begin_TLineFP, Img2Aug,
		Pattern2path,
		1540, 120, 1750, 120,
		50, 12, 1, 40,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_89left_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_89left_TLineFP, Img2Aug,
		Pattern2path,
		1780, 280, 1780, 340,
		50, 12, 1, 1,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_48_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_48_TLineFP, Img2Aug,
		Pattern2path,
		1780, 340, 1850, 390,
		50, 12, 1, 40,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_86right_91_chamfer87_2_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_86right_91_chamfer87_2_TLineFP, Img2Aug,
		Pattern2path,
		2020, 340, 1940, 400,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);
	cout << "next:12" << endl;
	double cam0_2_86left_chamfer87_1_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_86left_chamfer87_1_TLineFP, Img2_35,
		Pattern2path,
		2090, 440, 2090, 600,
		50, 12, 1, 5,
		0.5, 1, 0.3, 0, 0.7);
	...
	double cam0_2_chamfer9_1begin_35_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_chamfer9_1begin_35_TLineFP, Img2Aug,
		Pattern2path,
		2155, 590, 2263, 590,
		30, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	cam0_2_chamfer9_1begin_35_TLineFP[2] = cam0_2_chamfer9_1begin_35_TLineFP[2] - 2;

	double cam0_2_chamfer9_1end_36_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_chamfer9_1end_36_TLineFP, Img2Aug,
		Pattern3path,
		700, 50, 1000, 150,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_36angle_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_36angle_TLineFP, Img2Aug,
		Pattern3path,
		1400, 310, 1600, 380,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	...
	fault_detect = 3;
	HTuple Pattern4path = "./programParmeter/WZ10-15-1107-141/cam0_3-141-38.sbm";
	double cam0_3_chamfer9_2_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_chamfer9_2_TLineFP, Img3Aug,
		Pattern4path,
		-1140, -80, -700, -80,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_chamfer38_41up_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_chamfer38_41up_TLineFP, Img3Aug,
		Pattern4path,
		-370, -50, -200, -50,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_41down_chamfer43begin_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_41down_chamfer43begin_TLineFP, Img3Aug,
		Pattern4path,
		110, -70, 170, -70,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_chamfer43end_chamfer10begin_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_chamfer43end_chamfer10begin_TLineFP, Img3Aug,
		Pattern4path,
		340, 140, 760, 140,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);
	cout << "ceshi:102:" << cam0_3_chamfer43end_chamfer10begin_TLineFP[0] << "  " << cam0_3_chamfer43end_chamfer10begin_TLineFP[2] << endl;

	double cam0_3_chamfer10end_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_chamfer10end_TLineFP, Img3Aug,
		Pattern4path,
		820, 180, 820, 440,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	cout << "ceshi:103:" << cam0_3_chamfer10end_TLineFP[0] << "  " << cam0_3_chamfer10end_TLineFP[2] << endl;

	double cam0_3_8_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_8_TLineFP, Img3Aug,
		Pattern4path,
		800, 1100, 880, 1180,
		50, 12, 1, 30,
		0.5, 1, 0.3, 0, 0.7);
	cam0_3_8_TLineFP[3] = cam0_3_8_TLineFP[3] - 6;
	cam0_3_8_TLineFP[2] = cam0_3_8_TLineFP[2] - 6;
	cam0_3_8_TLineFP[1] = cam0_3_8_TLineFP[1] + 6;
	cam0_3_8_TLineFP[0] = cam0_3_8_TLineFP[0] + 6;
	cout << "ceshi:101:" << cam0_3_8_TLineFP[0] << "  " << cam0_3_8_TLineFP[2] << endl;
	cout << "ceshi:102:" << cam0_3_chamfer43end_chamfer10begin_TLineFP[0] << "  " << cam0_3_chamfer43end_chamfer10begin_TLineFP[2] << endl;
	cout << "ceshi:103:" << cam0_3_chamfer43end_chamfer10begin_TLineFP[0] << "  " << cam0_3_chamfer43end_chamfer10begin_TLineFP[2] << endl;
	fault_detect = 0;

	//以下程序对特征集中处理，不涉及特征点检测

	/// 下面的写入表格需要单独处理
	double AxialD_141_64_1 = atan((cam0_1_64_TLineFP[2] - cam0_1_64_TLineFP[0]) / (cam0_1_64_TLineFP[3] - cam0_1_64_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_64_1);
	pushback_vectors(resultVectorList, 3, 64.1, 45, -5, +5);
	/// 下面的写入表格需要单独处理
	double AxialD_141_64_2 = ((cam0_1_64_TLineFP[2] - cam0_1_64_TLineFP[0]) + (cam0_1_64_TLineFP[3] - cam0_1_64_TLineFP[1])) / 2;
	resultVectorList.push_back(AxialD_141_64_2 * Calik);
	pushback_vectors(resultVectorList, 4, 64, 0.5, -0.24, +0.24);

	double AxialD_141_63 = atan((cam0_1_63_TLineFP[3] - cam0_1_63_TLineFP[1]) / (cam0_1_63_TLineFP[2] - cam0_1_63_TLineFP[0])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_63);
	pushback_vectors(resultVectorList, 3, 63, 55, -15, +15);


	double AxialD_141_61 = cam0_1_61left_TLineFP[0] - (cam0_1_61right_TLineFP[0] + cam0_1_61right_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_141_61 * Calik);
	pushback_vectors(resultVectorList, 4, 61, 3.8, -0.15, +0.15);

	double AxialD_141_102 = atan((cam0_1_102_100right_chamfer101_2begin_TLineFP[1] - cam0_1_102_100right_chamfer101_2begin_TLineFP[3]) / (cam0_1_102_100right_chamfer101_2begin_TLineFP[2] - cam0_1_102_100right_chamfer101_2begin_TLineFP[0])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_102);
	pushback_vectors(resultVectorList, 3, 102, 45, -15, +15);


	cout << "ceshi:101:" << cam0_1_chamfer101_2end_chamfer101_1begin_TLineFP[2] << "  " << cam0_1_97_100left_chamfer101_1end_TLineFP[0] << endl;
	cout << "ceshi:cam0_3_8_TLineFP" << cam0_3_8_TLineFP << "cam0_3_8_TLineFP[0]:" << cam0_3_8_TLineFP[0] << "cam0_3_8_TLineFP[1]:" << cam0_3_8_TLineFP[1] << endl;
	double Chamfer_141_101_1 = TLineFP_Chamfer_P(Img1Aug,
		cam0_1_chamfer101_2end_chamfer101_1begin_TLineFP[2], 1500, cam0_1_97_100left_chamfer101_1end_TLineFP[0], 2000,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_101_1 * Calik);

	double Chamfer_141_101_2 = TLineFP_Chamfer_P(Img1Aug,
		cam0_1_102_100right_chamfer101_2begin_TLineFP[2] + 10, 1500, cam0_1_chamfer101_2end_chamfer101_1begin_TLineFP[0] + 10, 2000,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_101_2 * Calik);
	pushback_vectors(resultVectorList, 2, 101, 1, -0.24, +0.24);

	double Chamfer_141_62 = TLineFP_Chamfer_P(Img1Aug,
		cam0_1_63_TLineFP[0] - 45, 1500, cam0_1_63_TLineFP[0], 2000,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_62 * Calik);
	pushback_vectors(resultVectorList, 2, 62, 1, -0.23, +0.23);



	/// 下面的写入表格需要单独处理
	double AxialD_141_97_1 = atan((cam0_1_97_100left_chamfer101_1end_TLineFP[2] - cam0_1_97_100left_chamfer101_1end_TLineFP[0]) / (cam0_1_97_100left_chamfer101_1end_TLineFP[3] - cam0_1_97_100left_chamfer101_1end_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_97_1);
	pushback_vectors(resultVectorList, 3, 97.1, 45, -5, +5);
	cout << "fenzi:" << cam0_1_97_100left_chamfer101_1end_TLineFP[2] - cam0_1_97_100left_chamfer101_1end_TLineFP[0] << "fenmu:" << cam0_1_97_100left_chamfer101_1end_TLineFP[3] - cam0_1_97_100left_chamfer101_1end_TLineFP[1] << endl;
	cout << "atan:" << atan((cam0_1_97_100left_chamfer101_1end_TLineFP[2] - cam0_1_97_100left_chamfer101_1end_TLineFP[0]) / (cam0_1_97_100left_chamfer101_1end_TLineFP[3] - cam0_1_97_100left_chamfer101_1end_TLineFP[1])) << endl;
	cout << "*180/pi:" << AxialD_141_97_1 << endl;

	/// 下面的写入表格需要单独处理
	double AxialD_141_97_2 = ((cam0_1_97_100left_chamfer101_1end_TLineFP[2] - cam0_1_97_100left_chamfer101_1end_TLineFP[0]) + (cam0_1_97_100left_chamfer101_1end_TLineFP[3] - cam0_1_97_100left_chamfer101_1end_TLineFP[1])) / 2;
	resultVectorList.push_back(AxialD_141_97_2 * Calik);
	pushback_vectors(resultVectorList, 4, 97, 0.65, -0.1, +0.1);

	double AxialD_141_100 = cam0_1_97_100left_chamfer101_1end_TLineFP[0] - cam0_1_102_100right_chamfer101_2begin_TLineFP[0];
	resultVectorList.push_back(AxialD_141_100 * Calik);
	pushback_vectors(resultVectorList, 4, 100, 4.43, -0.2, +0.2);

	double AxialD_141_99 = cam0_1_99left_chamfer98_TLineFP[2] - cam0_1_97_100left_chamfer101_1end_TLineFP[0];
	resultVectorList.push_back(AxialD_141_99 * Calik);
	pushback_vectors(resultVectorList, 4, 99, 3.84, -0.05, +0.05);

	double Chamfer_141_98 = TLineFP_Chamfer_P(Img1Aug,
		cam0_1_99left_chamfer98_TLineFP[2] + 20, 1500, cam0_1_99left_chamfer98_TLineFP[2] + 100, 2000,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_98 * Calik);
	pushback_vectors(resultVectorList, 2, 98, 1.75, -0.45, +0);


	double AxialD_141_53_1 = atan((cam0_2_53right_chamfer54_2begin_TLineFP[2] - cam0_2_53right_chamfer54_2begin_TLineFP[0]) / (cam0_2_53right_chamfer54_2begin_TLineFP[1] - cam0_2_53right_chamfer54_2begin_TLineFP[3])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_53_1);

	double AxialD_141_53_2 = atan((cam0_2_53_2_chamfer_TLineFP[2] - cam0_2_53_2_chamfer_TLineFP[0]) / (cam0_2_53_2_chamfer_TLineFP[3] - cam0_2_53_2_chamfer_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_53_2);
	pushback_vectors(resultVectorList, 3, 53, 30, -2, +2);

	double AxialD_141_52 = cam0_2_52left_TLineFP[2] - cam0_2_52right_TLineFP[0];
	resultVectorList.push_back(AxialD_141_52 * Calik);
	pushback_vectors(resultVectorList, 4, 52, 13.9, -0.25, +0.25);

	double AxialD_141_50 = atan((cam0_2_47left_50_TLineFP[2] - cam0_2_47left_50_TLineFP[0]) / (cam0_2_47left_50_TLineFP[3] - cam0_2_47left_50_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_50);
	pushback_vectors(resultVectorList, 3, 50, 45, -1, +1);

	double AxialD_141_47 = cam0_2_47left_50_TLineFP[2] - cam0_2_47left_50_TLineFP[0];
	resultVectorList.push_back(AxialD_141_47 * Calik);
	pushback_vectors(resultVectorList, 4, 47, 0.56, -0.05, +0.05);

	double Chamfer_141_54_2 = TLineFP_Chamfer_P(Img2Aug,
		cam0_2_53right_chamfer54_2begin_TLineFP[2], 1500, cam0_2_chamfer54_2end_chamfer54_1begin_TLineFP[0], 2000,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_54_2 * Calik);

	double Chamfer_141_54_1 = TLineFP_Chamfer_P(Img2Aug,
		cam0_2_chamfer54_2end_chamfer54_1begin_TLineFP[2], 1500, cam0_2_47right_53left_chamfer54_1end_TLineFP[2], 2000,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_54_1 * Calik);
	pushback_vectors(resultVectorList, 2, 54, 1.75, -0.45, +0);

	/// 下面的写入表格需要单独处理
	double AxialD_141_48_1 = atan((cam0_2_48_TLineFP[2] - cam0_2_48_TLineFP[0]) / (cam0_2_48_TLineFP[3] - cam0_2_48_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_48_1);
	pushback_vectors(resultVectorList, 3, 48.1, 45, -5, +5);
	/// 下面的写入表格需要单独处理
	double AxialD_141_48_2 = ((cam0_2_48_TLineFP[2] - cam0_2_48_TLineFP[0]) + (cam0_2_48_TLineFP[3] - cam0_2_48_TLineFP[1])) / 2;
	resultVectorList.push_back(AxialD_141_48_2 * Calik);
	pushback_vectors(resultVectorList, 4, 48, 0.58, -0.1, +0.1);

	double AxialD_141_46 = cam0_2_48_TLineFP[0] - cam0_2_52left_TLineFP[2];
	resultVectorList.push_back(AxialD_141_46 * Calik);
	pushback_vectors(resultVectorList, 4, 46, 15.01, -0.25, +0.25);

	double AxialD_141_51 = cam0_2_48_TLineFP[0] - cam0_1_99left_chamfer98_TLineFP[2];
	resultVectorList.push_back(AxialD_141_51 * Calik + pic1To2_moveDistance);
	pushback_vectors(resultVectorList, 4, 51, 69.51, -0.1, +0.1);

	double AxialD_141_89 = (cam0_2_89left_TLineFP[0] + cam0_2_89left_TLineFP[2]) / 2 - cam0_2_47left_50_TLineFP[0];
	resultVectorList.push_back(AxialD_141_89 * Calik);
	pushback_vectors(resultVectorList, 4, 89, 7.08, -0.1, +0.1);

	double AxialD_141_90 = (cam0_2_89left_TLineFP[0] + cam0_2_89left_TLineFP[2]) / 2 - cam0_2_90right_chamfer94begin_TLineFP[2];
	resultVectorList.push_back(AxialD_141_90 * Calik);
	pushback_vectors(resultVectorList, 4, 90, 3.96, -0.05, +0.05);

	double AxialD_141_91 = atan((cam0_2_86right_91_chamfer87_2_TLineFP[3] - cam0_2_86right_91_chamfer87_2_TLineFP[1]) / (cam0_2_86right_91_chamfer87_2_TLineFP[0] - cam0_2_86right_91_chamfer87_2_TLineFP[2])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_91);
	pushback_vectors(resultVectorList, 4, 91, 30, -1, +1);

	double Chamfer_141_94 = TLineFP_Chamfer_P(Img2Aug,
		cam0_2_90right_chamfer94begin_TLineFP[2], 1500, cam0_2_chamfer94end_chamfer87_3begin_TLineFP[0], 2000,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_94 * Calik);
	pushback_vectors(resultVectorList, 2, 94, 1.5, -0.7, +0.7);

	double Chamfer_141_87_3 = TLineFP_Chamfer_P(Img2Aug,
		cam0_2_chamfer94end_chamfer87_3begin_TLineFP[2], 1500, (cam0_2_89left_TLineFP[0] + cam0_2_89left_TLineFP[2]) / 2 - 5, 2000,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_87_3 * Calik);

	double Chamfer_141_87_2 = TLineFP_Chamfer_P(Img2Aug,
		cam0_2_86right_91_chamfer87_2_TLineFP[0]-20, 1900, cam0_2_86right_91_chamfer87_2_TLineFP[0] + 25, 2500,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_87_2 * Calik);

	double Chamfer_141_87_1 = TLineFP_Chamfer_P(Img2Aug,
		(cam0_2_86left_chamfer87_1_TLineFP[0] + cam0_2_86left_chamfer87_1_TLineFP[2]) / 2 - 45, 2000, (cam0_2_86left_chamfer87_1_TLineFP[0] + cam0_2_86left_chamfer87_1_TLineFP[2]) / 2 + 5, 2110,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_87_1 * Calik);
	pushback_vectors(resultVectorList, 2, 87, 0.8, -0.3, +0.3);

	double AxialD_141_83 = (cam0_2_86left_chamfer87_1_TLineFP[0] + cam0_2_86left_chamfer87_1_TLineFP[2]) / 2 - (cam0_2_89left_TLineFP[0] + cam0_2_89left_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_141_83 * Calik);
	pushback_vectors(resultVectorList, 4, 83, 3.88, -3.88, +3.88);

	double AxialD_141_86 = (cam0_2_86left_chamfer87_1_TLineFP[0] + cam0_2_86left_chamfer87_1_TLineFP[2]) / 2 - cam0_2_86right_91_chamfer87_2_TLineFP[2];
	resultVectorList.push_back(AxialD_141_86 * Calik);
	pushback_vectors(resultVectorList, 4, 86, 2.27, -2.27, +0);

	double AxialD_141_36 = atan((cam0_2_36angle_TLineFP[3] - cam0_2_36angle_TLineFP[1]) / (cam0_2_36angle_TLineFP[2] - cam0_2_36angle_TLineFP[0])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_36);
	pushback_vectors(resultVectorList, 3, 36, 20, -0.5, +0.5);

	double AxialD_141_41 = (cam0_3_chamfer38_41up_TLineFP[1] + cam0_3_chamfer38_41up_TLineFP[3]) / 2 - (cam0_3_41down_chamfer43begin_TLineFP[1] + cam0_3_41down_chamfer43begin_TLineFP[3]) / 2;
	resultVectorList.push_back(AxialD_141_41 * Calik);
	pushback_vectors(resultVectorList, 4, 41, 0.3, -0.1, +0.1);

	double Chamfer_141_38 = TLineFP_Chamfer_P(Img3Aug,
		cam0_3_chamfer38_41up_TLineFP[0] - 55, 2300, cam0_3_chamfer38_41up_TLineFP[0] - 5, 3110,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_38 * Calik);
	pushback_vectors(resultVectorList, 2, 38, 1.5, -0.45, +0);

	double Chamfer_141_43 = TLineFP_Chamfer_P(Img3Aug,
		cam0_3_41down_chamfer43begin_TLineFP[0], 2300, cam0_3_chamfer43end_chamfer10begin_TLineFP[0] - 20, 3110,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_43 * Calik);
	pushback_vectors(resultVectorList, 2, 43, 2.8, -0, +0.5);

	double AxialD_141_35 = cam0_2_chamfer9_1begin_35_TLineFP[2] - (cam0_2_86left_chamfer87_1_TLineFP[0] + cam0_2_86left_chamfer87_1_TLineFP[2]) / 2;
	resultVectorList.push_back(AxialD_141_35 * Calik);
	pushback_vectors(resultVectorList, 4, 35, 1.68, -0.12, +0.12);
	/// 下面的写入表格需要单独处理
	double AxialD_141_1_1 = atan((cam0_2_1_TLineFP[2] - cam0_2_1_TLineFP[0]) / (cam0_2_1_TLineFP[3] - cam0_2_1_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_1_1);
	pushback_vectors(resultVectorList, 3, 1.1, 45, -5, +5);
	/// 下面的写入表格需要单独处理
	double AxialD_141_1_2 = sqrt(pow(cam0_2_1_TLineFP[2] - cam0_2_1_TLineFP[0], 2) + pow(cam0_2_1_TLineFP[3] - cam0_2_1_TLineFP[1], 2));
	resultVectorList.push_back(AxialD_141_1_2 * Calik);
	pushback_vectors(resultVectorList, 4, 1, 0.5, -0.2, +0.2);

	double Chamfer_141_10 = TLineFP_Chamfer_P(Img3Aug,
		cam0_3_chamfer43end_chamfer10begin_TLineFP[2], 2300, cam0_3_chamfer10end_TLineFP[0] - 10, 2870,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_10 * Calik);
	pushback_vectors(resultVectorList, 2, 10, 0.8, -0.3, +0.3);
	/// 下面的写入表格需要单独处理
	double AxialD_141_8_1 = atan((cam0_3_8_TLineFP[2] - cam0_3_8_TLineFP[0]) / (cam0_3_8_TLineFP[3] - cam0_3_8_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_141_8_1);
	pushback_vectors(resultVectorList, 3, 8.1, 45, -5, +5);
	/// 下面的写入表格需要单独处理
	double AxialD_141_8_2 = ((cam0_3_8_TLineFP[2] - cam0_3_8_TLineFP[0]) + (cam0_3_8_TLineFP[3] - cam0_3_8_TLineFP[1])) / 2;
	resultVectorList.push_back(AxialD_141_8_2 * Calik);
	pushback_vectors(resultVectorList, 4, 8, 0.665, -0.1, +0.1);

	double Chamfer_141_9_1 = TLineFP_Chamfer_P(Img2Aug,
		cam0_2_chamfer9_1begin_35_TLineFP[2] + 5, 2000, cam0_2_chamfer9_1end_36_TLineFP[0], 2500,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_9_1 * Calik);

	double Chamfer_141_9_2 = TLineFP_Chamfer_P(Img3Aug,
		cam0_3_chamfer9_2_TLineFP[0] - 140, 2500, cam0_3_chamfer9_2_TLineFP[0], 3000,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_141_9_2 * Calik);
	pushback_vectors(resultVectorList, 2, 9, 6.35, -0.25, +0.25);
...
//孔和花键的测量函数
//孔径相机1测孔
double program_3::cam1Picture_holeAlgorithm(string imgPath)//远心相机2测孔图像算法
{
	double diameter1;
	HTuple halconPath = imgPath.c_str();
	...
	double cam0_11_460_152LeftChamferBegin_TLineFP[4];
	{
		...
		Illuminate(ho_ImageEmphasize1, &ho_ImageIlluminate, 170, 110, 0.95);
		...
		AddMetrologyObjectGeneric(hv_MetrologyHandle, "line", hv_Line1.TupleConcat(hv_Line2),
			40, 16, 10, 10, HTuple(), HTuple(), &hv_LineIndices);
		GetMetrologyObjectMeasures(&ho_LineContours, hv_MetrologyHandle, "all", "all",
			&hv_RowLine, &hv_ColumnLine);
...
void program_3::run()
{
	...
	defaultPath = defaultPath + "\\measureData\\WZ10-15-1107-141";
	...
	for (int i = 24; i >= 0; i--)//第二次测量（向下移动）
	{
		emit programProcess(QString("正在对 %1 号特征（%2）进行第2次测量！").arg(lsDiameterResult[i][3]).arg(featureMap[1]), 68 - i * 2);
		...
		case 11:
			break;
		case 12://12-14是49号位置直径
			break;
		case 13:
			break;
		case 14:
			break;
		case 15://15-17是4号位置直径
			break;
		case 16:
			break;
		case 17:
			break;
		case 20://20-22是圆角位置直径
			break;
		case 21:
			break;
		case 22:
	...
	resultVectorList.push_back(lsDiameterResult[0][0]);
	resultVectorList.push_back(lsDiameterResult[0][1]);
	resultVectorList.push_back(lsDiameterResult[2][0]);
	resultVectorList.push_back(lsDiameterResult[2][1]);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[0][3], lsDiameterResult[0][4], lsDiameterResult[0][6], lsDiameterResult[0][5]);
	resultVectorList.push_back(lsDiameterResult[4][0]);
	resultVectorList.push_back(lsDiameterResult[4][1]);
	resultVectorList.push_back(lsDiameterResult[6][0]);
	resultVectorList.push_back(lsDiameterResult[6][1]);
	...
	dataScreenOrg.push_back(lsDiameterResult[20][0]);//滤除42号错误尺寸
	...
	dataScreenOrg.push_back(lsDiameterResult[9][0]);//滤除96号错误尺寸
	...
	dataScreenOrg.push_back(lsDiameterResult[12][0]);//滤除49号错误尺寸
	...
	dataScreenOrg.push_back(lsDiameterResult[15][0]);//滤除44号错误尺寸
	dataScreenOrg.push_back(lsDiameterResult[15][1]);
	dataScreenOrg.push_back(lsDiameterResult[16][0]);
	...
	dataScreening_maxTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[17][3], lsDiameterResult[17][4], lsDiameterResult[17][6], lsDiameterResult[17][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();
	...
	for (int i = 0; i < 1; i++)
	{
		emit programProcess(QString("正在对 %1 号特征（粗糙度） 进行测量 ！").arg(roughnessResult[i][0]), 75);
		cam2_Measure_prepare(i);
	};

	camPtrList[0]->setExposeTime(cam0_exposeTime);//对三个远心相机进行图像采集
	camPtrList[0]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[0]->capturedImg);
	for (int i = 0; i < 3; i++)
	{
		emit programProcess(QString("远心相机正在对 %1 号位置进行轮廓测量！").arg(i + 1), 75 + 4 * i);
	...
	try {
		...
		emit updateDeviceInf(QString("远心相机图像%1处理执行错误").arg(fault_detect));
	};
	...
	emit programTips("即将进行孔检测，请参照图示正确安装机心夹！", 3);
	msleep(200);
	emit fixtureTips(3);
	programConfirm = "doNotContinue";
	do
	...
	/*
	//由于无法对基准G进行有效拟合，对22号跳动特征无法进行有效测量
	for (int i = 3; i >=0; i--)//跳动4-1号位置,以及基准N进行数据采集
	{
		emit programProcess(QString("正在对 %1 号特征（跳动） 进行数据采集！").arg(lsRoundoutResult[i][0]), 90 - i*5);
	...
	*/
	...
	/*
	//孔径特征2号图纸无标记，所以不进行测量
	camPtrList[1]->setExposeTime(cam1_exposeTime);//孔径2号位置测量
	camPtrList[1]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[1]->capturedImg);
	emit programProcess("正在对 孔径2号位置 进行测量！", 94);
	cam1_Measure_prepare(1);
	for (int i = 0; i < cam1PathList.size(); i++)
	{
		try {
			double diameter = cam1Picture_holeAlgorithm(cam1PathList[i]);
			pushback_vectors(diameter, 8, 141.699 + (i + 1) / 10000, 8.9, 0, +0.2);
		}
		catch (...)
		{
			emit updateDeviceInf("孔径位置2图像处理执行错误");
		}
	}
	*/
	//cam1_exposeTime = 1200;
	camPtrList[1]->setExposeTime(cam1_exposeTime);//孔径1号位置测量
	camPtrList[1]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[1]->capturedImg);
	emit programProcess("正在对 孔径1号位置 进行测量！", 95);
	cam1_Measure_prepare(0);
	try {
		for (int i = 0; i < cam1PathList.size(); i++)
		{
			double diameter = cam1Picture_holeAlgorithm(cam1PathList[i]);
			cout << "孔径测量结果" << i + 1 << ": " << diameter << endl;
			resultVectorList.push_back(diameter);
		};
		pushback_vectors(resultVectorList, 8, 81, 4.62, -0.25, +0.15);
	}
	catch (...) {
	...
	//跳动数据处理以及计算
	/*
	double referencePoint_N[3][3];
	for (int i = 0; i < 4; i++)//拟合旋转轴线
	{
		for (int j = 0; j < 3; j++)
		{
			centerCosFit(i, j);
			...
			//cout << "跳动号：" << i << " 点位：" << j << "余弦拟合结果" << " A= " << roundoutDataPtr[i]->cosFit[j][0] << " B= " << roundoutDataPtr[i]->cosFit[j][1] << "C= " << roundoutDataPtr[i]->cosFit[j][2] << endl;
		};
	};
	for (int i = 0; i < 3; i++)
	{
		copy(begin(roundoutDataPtr[0]->axisPoint[i]), end(roundoutDataPtr[0]->axisPoint[i]), begin(referencePoint_N[i]));//拷贝基准N的点位数据至联合基准
	}
	axisLineCalculate(referencePoint_N, 3, directionVector_N, aixsReferancePoint_N);//计算拟合参考轴线
	for (int i = 0; i < 3; i++)
	{
		axisLoaction(1, i, directionVector_N, aixsReferancePoint_N);//分别计算各跳动测量位置2测中的3个子截面的跳动值（相对与基准N）
		roundoutCalculate(0, i,0);
		roundoutCalculate(1, i, 1);
		roundoutCalculate(2, i, 1);
		roundoutCalculate(3, i, 1);
	};
	for (int i = 0; i < 4; i++)//各跳动测量点位，跳动值汇总计算
	{
		...
		pushback_vectors(lsRoundoutResult[i][1], 5, lsRoundoutResult[i][0], 0, 0, lsRoundoutResult[i][2]);
	}
	clearRoundoutData();
	*/
	...
	cout << "2-3:" << pic2To3_moveDistance << endl;
};
