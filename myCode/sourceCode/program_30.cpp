...
program_30::program_30(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
...
bool program_30::stepLengthCompensationCaculation_axis5()
{
	...
	try {
		/*匹配轴向移动点位开始*/
		string Imgpath1_pattern = "./programParmeter/WZ10-11-2006-430/DQ.bmp";
		HObject Img1Aug_pattern = imgAug(Imgpath1_pattern);
...
void program_30::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
void program_30::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime = 50)
{
	//初始化变量
...
void program_30::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_30::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_30::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_30::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_30::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_30::clearRoundoutData()
{
...
//***********************************************************************************************************************************************************************************************************************************************
string program_30::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_30::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_30::cam2_Measure_prepare(int positionNumber)
{
...
void program_30::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_30::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_30::creatDatabaseTable()
{
...
void program_30::writeToDatabase()
{
	/*
...
void program_30::zeroMeasureNub()
{
	allFeatureFlag = "--";
	ngFeatureNum = 0;//NG特征数量
	measurePartsNum_all = 0;//该型号轴测量总数
...
void program_30::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
//远心相机图像检测算法
void program_30::cam0Picture_algorithm()//远心相机1图像算法
{
	...
	string Imgpath1 = imgSavePath[1];
	string Imgpath2 = imgSavePath[2];
	...
	*/
	...
	//大远心相机采集图像路径
	/*大远心相机采集图像路径--调用示例
	string Imgpath1 = "C:/Users/123/Desktop/XD/0.bmp";//需要
	*/
	//string Imgpath1 = "D:\\temporary_sun\\AxisMeasurement\\programParmeter\\WZ10442111-90\\cam0_1.bmp";
	string Imgpath1 = imgSavePath[0]; 
    string Imgpath2 = imgSavePath[1];
...
//	string Imgh1path = "C:/Users/123/Desktop/XD/12.7//H0.bmp";
	...
	/*大远心相机采集图像预处理--调用示例*/
	//HObject Img1Aug = imgAug(Imgpath1);
	HTuple halconPath = Imgpath1.c_str();
	HObject Img1_raw;
	HObject Img2_raw;
	...
	HTuple Pattern1path = R"(D:\temporary_sun\AxisMeasurement\programParmeter\Z911301G-190\cam0_1.sbm)";
	//12#
	/*double cam0_1_Chamfer_15_3[4];
	TemplateMatching_TLineFP_P(cam0_1_Chamfer_15_3, Img1_raw,
		Pattern1path,
		2890, 2458, 3011, 2576,
		50, 12, 1, 50,
		0.4, 1, 0.3, 0, 0.7);*/

	double cam0_1_Chamfer_12_1 = TLineFP_Chamfer_P(Img1_raw,
		2869, 2439, 3001, 2576,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(cam0_1_Chamfer_12_1 * Calik);
	pushback_vectors(resultVectorList, 2, 12, 1, -0.3, +0.3);

	double cam0_1_Chamfer_12_2 = TLineFP_Chamfer_P(Img1_raw,
		3065, 2480,3164, 2576,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(cam0_1_Chamfer_12_2 * Calik);
	pushback_vectors(resultVectorList, 2, 12, 1, -0.3, +0.3);
	//10#
	double cam0_1_Chamfer_10_1 = TLineFP_Chamfer_P(Img1_raw,
		3536, 2477, 3633, 2576,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(cam0_1_Chamfer_10_1 * Calik);
	pushback_vectors(resultVectorList, 2, 10, 1, -0.3, +0.3);

	double cam0_1_Chamfer_10_2 = TLineFP_Chamfer_P(Img1_raw,
		3719, 2468, 3798, 2534,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(cam0_1_Chamfer_10_2 * Calik);
	pushback_vectors(resultVectorList, 2, 10, 1, -0.3, +0.3);
	//9#
	double cam0_1_Chamfer_9_1 = TLineFP_Chamfer_P(Img2_raw,
		1293, 2572, 1325, 2599,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(cam0_1_Chamfer_9_1 * Calik);
	pushback_vectors(resultVectorList, 2, 9, 1, -0.3, +0.3);

	double cam0_1_Chamfer_9_2 = TLineFP_Chamfer_P(Img2_raw,
		757, 2538, 815, 2596,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(cam0_1_Chamfer_9_2* Calik);
	pushback_vectors(resultVectorList, 2, 9, 1, -0.3, +0.3);
...
double program_30::cam0Picture1_algorithmLuowenDown(HObject ho_Image,  HObject ho_Image_L,double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
...
double program_30::cam0Picture1_algorithmLuowenUp(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
	// Local iconic variables
	HObject  ho_ModelContours, ho_LineContours;
	HObject  ho_LineContour;
...
double program_30::cam0Picture1_L_K_b_Down(HObject ho_Image, double* hv_Box)
{
...
double program_30::cam0Picture1_L_K_b_Up(HObject ho_Image, double* hv_Box)
{
...
HObject program_30::imgAug(string imgPath) {
...
//模板匹配找特征点
void program_30::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
...
//根据起始点与终止点求倒角
double program_30::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
...
double program_30::TLineFP_Chamfer_H2(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
double program_30::TLineFP_Chamfer_H2_117up(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
void program_30::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_30::Straight_TLineFP_P137(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour;
...
void program_30::Straight_TLineFP_P116(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_30::Straight_TLineFP_P170(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_30::Straight_TLineFP_P102(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_30::cam0_roundoutMeasure_prepare(int positionNumber)
{
	//trap5轴
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_30::clearScrewRoundOutData()
{
	for (int i = 0; i < 2; i++)
	{
...
double program_30::cam1Picture_holeAlgorithm0(string imgPath)//H0孔图像算法
{
	//需要3.1在下方贴入孔径图像处理代码
	HTuple halconPath = imgPath.c_str();
...
double program_30::cam1Picture_holeAlgorithm1(string imgPath)//H1孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_30::cam1Picture_holeAlgorithm2(string imgPath)//H2孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_30::cam1Picture_holeAlgorithm3(string imgPath)//H3孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
void program_30::run()
{
	...
	defaultPath = defaultPath + "\\measureData\\Z911301G-190";//！需要将文件后夹后缀名改为对应的零件号
	...
	*/
	...
	originalImgPtr = &(camPtrList[0]->capturedImg);
	for (int i = 0; i < 2; i++)
		//远心0-1号采集位置采集图像（将i修改为远心相机的拍照位置总数，循环中的其他语句不需要修改）
	{
		camPtrList[0]->setExposeTime(cam0_exposeTime);
		camPtrList[0]->m_captureMode = "continuous";
		originalImgPtr = &(camPtrList[0]->capturedImg);
