...
program_19::program_19(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
	...
	//跳动数据结构体初始化
	for (int i = 0; i < 4; i++)//如有跳动测量需求将i的循环上限修改为需要进行跳动采集的轴号（!!!!只是为了测试新建的子程序是否添加到主程序中时可以暂时不用修改）
	{
...
bool program_19::stepLengthCompensationCaculation_axis5()
{
...
void program_19::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
void program_19::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime = 50)
{
	//初始化变量
...
void program_19::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_19::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_19::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_19::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_19::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_19::clearRoundoutData()
{
	for (int i = 0; i < 4; i++)//需要将循环体的i变量修改为跳动采集的轴段总数
	{
...
//***********************************************************************************************************************************************************************************************************************************************
string program_19::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_19::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_19::cam2_Measure_prepare(int positionNumber)
{
...
void program_19::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_19::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_19::creatDatabaseTable()
{
...
void program_19::writeToDatabase()
{
	/*
...
void program_19::zeroMeasureNub()
{
	allFeatureFlag = "--";
	ngFeatureNum = 0;//NG特征数量
	measurePartsNum_all = 0;//该型号轴测量总数
...
void program_19::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
//远心相机图像检测算法
void program_19::cam0Picture_algorithm()//远心相机1图像算法
{
	...
	*/
	...
	HTuple Pattern1path = "./programParmeter/KF7153309-230/cam0_0.sbm";
	double cam0_1_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_TLineFP, Img1Aug,
		Pattern1path,
		65, 75, 170, 180,
		50, 12, 1, 30,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_2_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_2_TLineFP, Img1Aug,
		Pattern1path,
		280, 80, 280, 160,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_3_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_3_TLineFP, Img1Aug,
		Pattern1path,
		500, 80, 500, 160,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_4_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_4_TLineFP, Img1Aug,
		Pattern1path,
		780, 170, 870, 260,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_5_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_5_TLineFP, Img1Aug,
		Pattern1path,
		840, 260, 1840, 260,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double Chamfer_38_1 = TLineFP_Chamfer_P(Img1Aug,
		cam0_2_TLineFP[0] - 10, cam0_2_TLineFP[1] - 50, cam0_2_TLineFP[0] + 25, cam0_2_TLineFP[1] + 0,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_38_1 * Calik);
	pushback_vectors(resultVectorList, 2, 38.1, 0.4, -0.2, +0.2);

	double Chamfer_38_2 = TLineFP_Chamfer_P(Img1Aug,
		cam0_3_TLineFP[0] - 50, cam0_3_TLineFP[1] - 50, cam0_3_TLineFP[0] + 20, cam0_3_TLineFP[1] + 20,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_38_2 * Calik);
	pushback_vectors(resultVectorList, 2, 38.2, 0.4, -0.2, +0.2);

	double Chamfer_40 = TLineFP_Chamfer_P(Img1Aug,
		cam0_4_TLineFP[0] - 50, cam0_4_TLineFP[1] - 50, cam0_4_TLineFP[0] + 30, cam0_4_TLineFP[1] + 30,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_40 * Calik);
	pushback_vectors(resultVectorList, 2, 40, 1, -0.3, +0.3);

	double Chamfer_23 = TLineFP_Chamfer_P(Img1Aug,
		cam0_5_TLineFP[2] - 0, cam0_5_TLineFP[3] - 230, cam0_5_TLineFP[2] + 400, cam0_5_TLineFP[3] + 30,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_23 * Calik);
	pushback_vectors(resultVectorList, 2, 23, 6, -1, +1);



	fault_detect = 2;//图像2处理函数开始

	HTuple Pattern2path = "./programParmeter/KF7153309-230/cam0_1.sbm";
	double cam1_1_TLineFP[4];
	TemplateMatching_TLineFP_P(cam1_1_TLineFP, Img2Aug,
		Pattern2path,
		-1050, 170, -150, 170,
		50, 12, 1, 40,
		0.5, 1, 0.3, 0, 0.7);

	double cam1_2_TLineFP[4];
	TemplateMatching_TLineFP_P(cam1_2_TLineFP, Img2Aug,
		Pattern2path,
		-200, 180, -130, 110,
		50, 12, 1, 45,
		0.5, 1, 0.3, 0, 0.7);

	double cam1_3_TLineFP[4];
	TemplateMatching_TLineFP_P(cam1_3_TLineFP, Img2Aug,
		Pattern2path,
		-140, 90, -140, 140,
		50, 12, 1, 70,
		0.5, 1, 0.3, 0, 0.7);

	double cam1_4_TLineFP[4];
	TemplateMatching_TLineFP_P(cam1_4_TLineFP, Img2Aug,
		Pattern2path,
		80, 30, 200, 150,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam1_5_TLineFP[4];
	TemplateMatching_TLineFP_P(cam1_5_TLineFP, Img2Aug,
		Pattern2path,
		150, 150, 2050, 150,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam1_6_TLineFP[4];
	TemplateMatching_TLineFP_P(cam1_6_TLineFP, Img2Aug,
		Pattern2path,
		1900, 150, 2100, -50,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double Chamfer_18 = TLineFP_Chamfer_P(Img2Aug,
		cam1_1_TLineFP[0] - 800, cam1_1_TLineFP[1] - 330, cam1_1_TLineFP[0] + 0, cam1_1_TLineFP[1] + 30,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_18 * Calik);
	pushback_vectors(resultVectorList, 2, 18, 13, -2, +2);

	double Chamfer_52_1 = TLineFP_Chamfer_P(Img2Aug,
		cam1_3_TLineFP[0] - 5, cam1_3_TLineFP[1] - 80, cam1_3_TLineFP[0] + 50, cam1_3_TLineFP[1] + 20,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_52_1 * Calik);
	pushback_vectors(resultVectorList, 2, 52.1, 1, -0.3, +0.3);

	double Chamfer_52_2 = TLineFP_Chamfer_P(Img2Aug,
		cam1_4_TLineFP[0] - 50, cam1_4_TLineFP[1] - 30, cam1_4_TLineFP[0] + 10, cam1_4_TLineFP[1] + 20,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_52_2 * Calik);
	pushback_vectors(resultVectorList, 2, 52.2, 1, -0.3, +0.3);

	double Chamfer_10 = TLineFP_Chamfer_P(Img2Aug,
		cam1_6_TLineFP[2] - 20, cam1_6_TLineFP[3] - 120, cam1_6_TLineFP[2] + 220, cam1_6_TLineFP[3] + 0,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_10 * Calik);
	pushback_vectors(resultVectorList, 2, 10, 5, -1, +1);

	fault_detect = 3;//图像3处理函数开始	

	HTuple Pattern3path = "./programParmeter/KF7153309-230/cam0_2.sbm";
	double cam2_1_TLineFP[4];
	TemplateMatching_TLineFP_P(cam2_1_TLineFP, Img3Aug,
		Pattern3path,
		-2375, 274, -1530, 274,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam2_2_TLineFP[4];
	TemplateMatching_TLineFP_P(cam2_2_TLineFP, Img3Aug,
		Pattern3path,
		-1500, 225, -1560, 285,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam2_3_TLineFP[4];
	TemplateMatching_TLineFP_P(cam2_3_TLineFP, Img3Aug,
		Pattern3path,
		-1330, 170, -1270, 230,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam2_4_TLineFP[4];
	TemplateMatching_TLineFP_P(cam2_4_TLineFP, Img3Aug,
		Pattern3path,
		-190, 160, -270, 240,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam2_5_TLineFP[4];
	TemplateMatching_TLineFP_P(cam2_5_TLineFP, Img3Aug,
		Pattern3path,
		-20, 50, -80, 110,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double Chamfer_11 = TLineFP_Chamfer_P(Img3Aug,
		cam2_1_TLineFP[0] - 350, cam2_1_TLineFP[1] - 200, cam2_1_TLineFP[0] + 0, cam2_1_TLineFP[1] + 0,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_11 * Calik);
	pushback_vectors(resultVectorList, 2, 11, 7.5, -2, +2);

	double Chamfer_58_1 = TLineFP_Chamfer_P(Img3Aug,
		cam2_2_TLineFP[0] - 0, cam2_2_TLineFP[1] - 100, cam2_2_TLineFP[0] + 80, cam2_2_TLineFP[1] - 10,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_58_1 * Calik);
	pushback_vectors(resultVectorList, 2, 58.1, 1, -0.3, +0.3);

	double Chamfer_58_2 = TLineFP_Chamfer_P(Img3Aug,
		cam2_3_TLineFP[0] - 50, cam2_3_TLineFP[1] - 50, cam2_3_TLineFP[0] + 0, cam2_3_TLineFP[1] - 0,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_58_2 * Calik);
	pushback_vectors(resultVectorList, 2, 58.2, 1, -0.3, +0.3);

	double Chamfer_60 = TLineFP_Chamfer_P(Img3Aug,
		cam2_4_TLineFP[0] - 0, cam2_4_TLineFP[1] - 70, cam2_4_TLineFP[0] + 100, cam2_4_TLineFP[1] - 20,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_60 * Calik);
	pushback_vectors(resultVectorList, 2, 60, 0.8, -0.3, +0.3);

	fault_detect = 0;//图像处理函数结束

	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////以下程序对特征集中处理，不涉及特征点检测////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	//PIC0
	double AxialD_37_1 = ((cam0_1_TLineFP[3] - cam0_1_TLineFP[1]) + (cam0_1_TLineFP[2] - cam0_1_TLineFP[0])) / 2;
	resultVectorList.push_back(AxialD_37_1 * Calik);
	pushback_vectors(resultVectorList, 4, 37.1, 1, -0.3, +0.3);

	double AxialD_37_2 = atan((cam0_1_TLineFP[2] - cam0_1_TLineFP[0]) / (cam0_1_TLineFP[3] - cam0_1_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_37_2);
	pushback_vectors(resultVectorList, 3, 37.2, 45, -5, +5);

	double AxialD_6 = (cam0_2_TLineFP[2]+ cam0_2_TLineFP[0])/2 - cam0_1_TLineFP[0];
	resultVectorList.push_back(AxialD_6 * Calik);
	pushback_vectors(resultVectorList, 4, 6, 2.6, -0.05, +0.05);

	double AxialD_39 = (cam0_3_TLineFP[2] + cam0_3_TLineFP[0]) / 2 - (cam0_2_TLineFP[2] + cam0_2_TLineFP[0]) / 2;
	resultVectorList.push_back(AxialD_39 * Calik);
	pushback_vectors(resultVectorList, 4, 39, 2.6, 0, +0.2);

	double AxialD_7 = cam0_5_TLineFP[2] - cam0_5_TLineFP[0];
	resultVectorList.push_back(AxialD_7 * Calik);
	pushback_vectors(resultVectorList, 4, 7, 11.5, 0, +0.2);

	double AxialD_41 = atan((cam0_4_TLineFP[2] - cam0_4_TLineFP[0]) / (cam0_4_TLineFP[3] - cam0_4_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_41);
	pushback_vectors(resultVectorList, 3, 41, 45, -5, +5);

	//PIC1
	double AxialD_8 = cam1_3_TLineFP[0] - cam1_1_TLineFP[0];
	resultVectorList.push_back(AxialD_8 * Calik);
	pushback_vectors(resultVectorList, 4, 8, 10.5, 0, +0.2);

	double AxialD_50_1 = ((cam1_2_TLineFP[2] - cam1_1_TLineFP[2]) + (cam1_1_TLineFP[3] - cam1_2_TLineFP[3])) / 2;
	resultVectorList.push_back(AxialD_50_1 * Calik);
	pushback_vectors(resultVectorList, 4, 50.1, 0.5, -0.2, +0.2);

	double AxialD_50_2 = atan((cam1_1_TLineFP[2] - cam1_2_TLineFP[2]) / (cam1_1_TLineFP[3] - cam1_2_TLineFP[3])) * 180 / PI;
	resultVectorList.push_back(AxialD_37_2);
	pushback_vectors(resultVectorList, 3, 50.2, 45, -5, +5);

	double AxialD_51 = cam1_4_TLineFP[2] - (cam1_3_TLineFP[0]+ cam1_3_TLineFP[2])/2;
	resultVectorList.push_back(AxialD_51 * Calik);
	pushback_vectors(resultVectorList, 4, 51, 4, 0, +0.12);

	double AxialD_53 = atan((cam1_4_TLineFP[2] - cam1_4_TLineFP[0]) / (cam1_4_TLineFP[3] - cam1_4_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_53);
	pushback_vectors(resultVectorList, 3, 53, 45, -5, +5);

	double AxialD_9 = cam1_5_TLineFP[2] - cam1_5_TLineFP[0];
	resultVectorList.push_back(AxialD_9 * Calik);
	pushback_vectors(resultVectorList, 4, 9, 21, -0.2, +0.2);

	double AxialD_61 = atan((cam1_6_TLineFP[2] - cam1_6_TLineFP[0]) / (cam1_6_TLineFP[1] - cam1_6_TLineFP[3])) * 180 / PI;
	resultVectorList.push_back(AxialD_61);
	pushback_vectors(resultVectorList, 3, 61, 45, -5, +5);

	double AxialD_4 = cam1_3_TLineFP[0] - cam0_1_TLineFP[0];
	resultVectorList.push_back(AxialD_4 * Calik + pic0To1_moveDistance);
	pushback_vectors(resultVectorList, 4, 4, 369.3, -0.1, +0.1);

	//PIC2
	double AxialD_13 = cam2_2_TLineFP[0] - cam2_1_TLineFP[0];
	resultVectorList.push_back(AxialD_13 * Calik);
	pushback_vectors(resultVectorList, 4, 13, 9.4, 0, +0.2);

	double AxialD_14 = cam2_4_TLineFP[2] - cam2_2_TLineFP[0];
	resultVectorList.push_back(AxialD_14 * Calik);
	pushback_vectors(resultVectorList, 4, 14, 15.3, -0.2, +0.2);

	double AxialD_59 = cam2_3_TLineFP[2] - cam2_2_TLineFP[0];
	resultVectorList.push_back(AxialD_59 * Calik);
	pushback_vectors(resultVectorList, 4, 59, 3, 0, +0.1);

	double AxialD_57 = ((cam2_2_TLineFP[0] - cam2_2_TLineFP[2]) + (cam2_2_TLineFP[3] - cam2_2_TLineFP[1])) / 2;
	resultVectorList.push_back(AxialD_57 * Calik);
	pushback_vectors(resultVectorList, 4, 57, 0.4, -0.2, +0.2);

	double AxialD_75 = atan((cam2_2_TLineFP[2] - cam2_2_TLineFP[0]) / (cam2_2_TLineFP[1] - cam2_2_TLineFP[3])) * 180 / PI;
	resultVectorList.push_back(AxialD_75);
	pushback_vectors(resultVectorList, 3, 75, 45, -1, +1);

	double AxialD_67 = atan((cam2_3_TLineFP[2] - cam2_3_TLineFP[0]) / (cam2_3_TLineFP[3] - cam2_3_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_67);
	pushback_vectors(resultVectorList, 3, 67, 45, -1, +1);

	double AxialD_61_1 = ((cam2_5_TLineFP[0] - cam2_5_TLineFP[2]) + (cam2_5_TLineFP[3] - cam2_5_TLineFP[1])) / 2;
	resultVectorList.push_back(AxialD_61_1 * Calik);
	pushback_vectors(resultVectorList, 4, 61.1, 0.4, -0.2, +0.2);

	double AxialD_61_2 = atan((cam2_5_TLineFP[2] - cam2_5_TLineFP[0]) / (cam2_5_TLineFP[1] - cam2_5_TLineFP[3])) * 180 / PI;
	resultVectorList.push_back(AxialD_61_2);
	pushback_vectors(resultVectorList, 3, 61.2, 45, -5, +5);

	double AxialD_2 = cam2_2_TLineFP[0] - cam0_1_TLineFP[0];
	resultVectorList.push_back(AxialD_2 * Calik + pic0To2_moveDistance);
	pushback_vectors(resultVectorList, 4, 2, 570.1, -0.2, +0.2);

	double AxialD_1 = cam2_5_TLineFP[0] - cam0_1_TLineFP[0];
	resultVectorList.push_back(AxialD_1 * Calik + pic0To2_moveDistance);
	pushback_vectors(resultVectorList, 4, 1, 588.4, -0.2, +0.2);
...
double program_19::cam0Picture1_algorithmLuowenDown(HObject ho_Image,  HObject ho_Image_L,double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
...
double program_19::cam0Picture1_algorithmLuowenUp(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
	// Local iconic variables
	HObject  ho_ModelContours, ho_LineContours;
	HObject  ho_LineContour;
...
double program_19::cam0Picture1_L_K_b_Down(HObject ho_Image, double* hv_Box)
{
...
double program_19::cam0Picture1_L_K_b_Up(HObject ho_Image, double* hv_Box)
{
...
HObject program_19::imgAug(string imgPath) {
...
//模板匹配找特征点
void program_19::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
...
//根据起始点与终止点求倒角
double program_19::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
...
double program_19::TLineFP_Chamfer_H2(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
double program_19::TLineFP_Chamfer_H2_117up(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
void program_19::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_19::Straight_TLineFP_P137(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour;
...
void program_19::Straight_TLineFP_P116(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_19::Straight_TLineFP_P170(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_19::Straight_TLineFP_P102(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_19::cam0_roundoutMeasure_prepare(int positionNumber)
{
	//trap5轴
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_19::clearScrewRoundOutData()
{
	for (int i = 0; i < 2; i++)
	{
...
double program_19::cam1Picture_holeAlgorithm0(string imgPath)//H0孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_19::cam1Picture_holeAlgorithm1(string imgPath)//H1孔图像算法
{
	//在下方贴入孔径图像处理代码
...
double program_19::cam1Picture_holeAlgorithm2(string imgPath)//H2孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_19::cam1Picture_holeAlgorithm3(string imgPath)//H3孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_19::cam1Picture_holeAlgorithm4(string imgPath)//H4孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
void program_19::run()
{
	...
	defaultPath = defaultPath + "\\measureData\\KF7153009-230";//！需要将文件后夹后缀名改为对应的零件号
	...
	*/
	...
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
	for (int i = 0; i < 34; i++)//第一次测量（向上移动）（需要将i修改为直径采集的总次数）
	{
	...
	for (int i = 33; i >= 0; i--)//第二次测量（向下移动）（需要将i修改为直径采集的总次数-1）
	{
		...
		case 0://24号直径特征（后续取最大值）
			break;
		case 1:
			break;
		case 2:
			break;
		case 3://43号直径特征（后续取最小值）
			break;
		case 4:
			break;
		case 5:
			break;
		case 6://42号直径特征（后续取最大值）
			break;
		case 7:
			break;
		case 8:
			break;
		case 9://22号直径特征（后续取最大值）
			break;
		case 10:
			break;
		case 11:
			break;
		case 12://20号直径特征（后续取最大值）
			break;
		case 13:
			break;
		case 14:
			break;
		case 15://55号直径特征（后续取最小值）
			break;
		case 16:
			break;
		case 17:
			break;
		case 18://54号直径特征（后续取最大值）
			break;
		case 19:
			break;
		case 20:
			break;
		case 22://16号直径特征（后续取最大值）
			break;
		case 23:
			break;
		case 24:
			break;
		case 25://66号直径特征（后续取最小值）
			break;
		case 26:
			break;
		case 27:
			break;
		case 28://64号直径特征（后续取最大值）
			break;
		case 29:
			break;
		case 30:
			break;
		case 31://62号直径特征（后续取最大值）
			break;
		case 32:
			break;
		case 33:
	...
	//直径数据处理取最大值
	dataScreenOrg.push_back(lsDiameterResult[0][0]);//24
	...
	dataScreenOrg.push_back(lsDiameterResult[6][0]);//42
	dataScreenOrg.push_back(lsDiameterResult[6][1]);
	dataScreenOrg.push_back(lsDiameterResult[7][0]);
	...
	dataScreening_maxTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[8][3], lsDiameterResult[8][4], lsDiameterResult[8][6], lsDiameterResult[8][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();

	dataScreenOrg.push_back(lsDiameterResult[9][0]);//22
	...
	dataScreenOrg.push_back(lsDiameterResult[12][0]);//20
	...
	dataScreenOrg.push_back(lsDiameterResult[18][0]);//54
	...
	dataScreening_maxTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[20][3], lsDiameterResult[20][4], lsDiameterResult[20][6], lsDiameterResult[20][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();

	dataScreenOrg.push_back(lsDiameterResult[22][0]);//16
	dataScreenOrg.push_back(lsDiameterResult[22][1]);
	dataScreenOrg.push_back(lsDiameterResult[23][0]);
	...
	dataScreening_maxTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[24][3], lsDiameterResult[24][4], lsDiameterResult[24][6], lsDiameterResult[24][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();

	dataScreenOrg.push_back(lsDiameterResult[28][0]);//64
	...
	dataScreening_maxTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[30][3], lsDiameterResult[30][4], lsDiameterResult[30][6], lsDiameterResult[30][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();

	dataScreenOrg.push_back(lsDiameterResult[31][0]);//62
	...
	dataScreening_maxTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[31][3], lsDiameterResult[31][4], lsDiameterResult[31][6], lsDiameterResult[31][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();

	//直径选最小值输出
	dataScreenOrg.push_back(lsDiameterResult[3][0]);//43
	...
	dataScreenOrg.push_back(lsDiameterResult[15][0]);//55
	...
	dataScreenOrg.push_back(lsDiameterResult[25][0]);//66
	...
	dataScreening_minTwo(dataScreenOrg, resultVectorList);
	pushback_vectors(resultVectorList, 1, lsDiameterResult[27][3], lsDiameterResult[27][4], lsDiameterResult[27][6], lsDiameterResult[27][5]);
	dataScreenOrg.clear();
	dataScreenOrg.shrink_to_fit();

	
	//需要将直径实际测量结果传递给粗糙度用于补偿粗糙度轴移动位置
	cam2_radius[0][1] = lsDiameterResult[13][9];//传递旋转半径d4给粗糙度0号采集位置（如有新增复制本句代码）进行相应修改即可
	cam2_radius[1][1] = lsDiameterResult[21][9];
	/*
	新增参考代码
	...
	*/
	...
	for (int i = 0; i < 2; i++)//需要修改i为粗糙度测量点位的总数量
	{
		switch (i)
		{
		case 0://设置粗糙度测量点位Ra0的曝光
			cam2_exposeTime = 550;
			break;
		//如需要对其他点位如Ra1进行曝光设置，请参照如下代码
		/*
		//Ra1参考代码
		case 1:
			cam2_exposeTime = 550;
	...
	*/
	try {
		cam0Picture_algorithm();//远心相机的图像处理函数（视觉处理函数调试完成后需要将代码贴入该函数）
	}
	catch (...) {
...
/*
	...
	lsSensorMeasure_cylindricity(0);*///圆柱度测量函数，此处表明对为圆柱度测量位置C0进行圆柱度测量,如有多个位置有圆柱度测量要求可以参照下面代码新加，！！！！如果不需要测量圆柱度请删除圆柱度测量的全部相关代码
	/*
	...
	*/
	...
	//跳动测量*****************************************************************
	//第一步采各跳动测量轴段的基本数据
	for (int i = 3; i >= 0; i--)//只需要修改i值为跳动数据采集的轴段的总数-1（该循环中的其他部分不需要修改）
	{
	...
	for (int i = 0; i <4; i++)//（只需要修改i值为跳动数据采集的轴段的总数）
	{
		for (int j = 0; j < 3; j++)
		{
			centerCosFit(i, j);
	...
	////第三步选择跳动基准面的圆心数据并计算拟合的基准轴线（如不需要拟合基准轴线可以直接注释掉第三步的全部代码）
	//double referencePoint[6][3];//需要数组第一个长度=基准数量*3
	//int loopTime = 0;
	//for (int j = 0; j < 3; j++)
	//{
	//	//提取第一个基准上三个数据采集截面的圆心数据，
	//	copy(begin(roundoutDataPtr[0]->axisPoint[j]), end(roundoutDataPtr[0]->axisPoint[j]), begin(referencePoint[loopTime]));//提取第一个基准上三个数据采集截面的圆心数据，这里表示第一个基准是RT0（需要根据基准的位置更改roundoutDataPtr[0]）
	//	loopTime++;
	//	//提取第2个基准上三个数据采集截面的圆心数据，
	//	copy(begin(roundoutDataPtr[5]->axisPoint[j]), end(roundoutDataPtr[5]->axisPoint[j]), begin(referencePoint[loopTime]));//提取第二个基准截面上的圆心数据，这里表示第二个基准为RT5（需要根据基准的位置更改roundoutDataPtr[0]）
	//	loopTime++;
	//}
...
//cam1_exposeTime = 550;//孔径相机测量曝光设置（可能需要按照实际进行修改）其他没有注释的语句不需要修改
...
//int looptime = 0;
//cam1_Measure_prepare(0);//对孔径拍摄位置H0进行测量（这个函数参数为0）如需要对H1孔径测量需要将（0）改为1
//for (int i = 0; i < cam1PathList.size(); i++)
//{
//	try {
//		double diameter = cam1Picture_holeAlgorithm0(cam1PathList[i]);//孔径相机拍摄位置H0的图像处理函数（注意函数名字，孔径处理算法调试完成后，粘贴进该函数中），如需要对H1孔径测量需要将cam1Picture_holeAlgorithm0改为cam1Picture_holeAlgorithm1，并在其中粘贴孔径测量H1的相关算法
//
...
//{
//	pushback_vectors(resultVectorList, 8, 111, 8.9, -0, +0.2);//含义为将多次孔径测量的结果显示到软件上，需要修改pushback_vector参数的特征号，公称值等信息
//
//};

/*
如需要添加其他孔径位置如H1测量可参考此部分代码
	cam1_exposeTime = 450;//孔径相机测量曝光设置（可能需要按照实际进行修改）其他没有注释的语句不需要修改
camPtrList[1]->setExposeTime(cam1_exposeTime);
camPtrList[1]->m_captureMode = "continuous";
originalImgPtr = &(camPtrList[1]->capturedImg);
emit programProcess("正在对 孔径 进行测量！", 68);
cam1_Measure_prepare(1);//如需要对H1孔径测量需要将（0）改为1
for (int i = 0; i < cam1PathList.size(); i++)
{
	try {
		double diameter = cam1Picture_holeAlgorithm1(cam1PathList[i]);//对H1孔径测量需要将cam1Picture_holeAlgorithm0改为cam1Picture_holeAlgorithm1，并在其中粘贴孔径测量H1的相关算法
