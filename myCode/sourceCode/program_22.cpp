...
program_22::program_22(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
...
bool program_22::stepLengthCompensationCaculation_axis5()
{
...
void program_22::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
void program_22::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime = 50)
{
	//初始化变量
...
void program_22::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_22::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_22::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_22::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_22::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_22::clearRoundoutData()
{
...
//***********************************************************************************************************************************************************************************************************************************************
string program_22::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_22::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_22::cam2_Measure_prepare(int positionNumber)
{
...
void program_22::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_22::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_22::creatDatabaseTable()
{
...
void program_22::writeToDatabase()
{
	/*
...
void program_22::zeroMeasureNub()
{
	allFeatureFlag = "--";
	ngFeatureNum = 0;//NG特征数量
	measurePartsNum_all = 0;//该型号轴测量总数
...
void program_22::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
//远心相机图像检测算法
void program_22::cam0Picture_algorithm()//远心相机1图像算法
{
...
//螺纹中径计算函数
double program_22::cam0Picture1_algorithmLuowen(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru, vector <double> luoju_P, vector <double> yadi_R, vector <double> yaxingbanjiao, vector <double> xiaojing_d)//测螺纹
{
...
double program_22::cam0Picture1_L_K_b(HObject ho_Image, double* hv_Box)
{
...
HObject program_22::imgAug(string imgPath) {
...
//模板匹配找特征点
void program_22::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
...
//根据起始点与终止点求倒角
double program_22::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
...
double program_22::TLineFP_Chamfer_H2(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
double program_22::TLineFP_Chamfer_H2_117up(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
void program_22::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_22::Straight_TLineFP_P137(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour;
...
void program_22::Straight_TLineFP_P116(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_22::Straight_TLineFP_P170(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_22::Straight_TLineFP_P102(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_22::cam0_roundoutMeasure_prepare(int positionNumber)
{
	//trap5轴
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_22::clearScrewRoundOutData()
{
...
double program_22::cam1Picture_holeAlgorithm0(string imgPath)//H0孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_22::cam1Picture_holeAlgorithm1(string imgPath)//H1孔图像算法
{
	//在下方贴入孔径图像处理代码
...
double program_22::cam1Picture_holeAlgorithm2(string imgPath)//H2孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_22::cam1Picture_holeAlgorithm3(string imgPath)//H3孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_22::cam1Picture_holeAlgorithm4(string imgPath)//H4孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
void program_22::run()
{
	...
	defaultPath = defaultPath + "\\measureData\\WJ10H131035A-cucaodu";//！需要将文件后夹后缀名改为对应的零件号
	...
	*/
	...
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
	for (int i = 0; i < 1; i++)//第一次测量（向上移动）（将i修改为直径采集的总次数）
	{
	...
	for (int i = 0; i >= 0; i--)//第二次测量（向下移动）（将i修改为直径采集的总次数-1）
	{
	...
	*/
	...
	cam2_radius[0][1] = lsDiameterResult[0][9];
  //传递旋转半径d4给粗糙度0号采集位置（如有新增复制本句代码）进行相应修改即可
	/*
	新增参考代码
