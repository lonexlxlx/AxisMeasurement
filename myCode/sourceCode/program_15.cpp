...
#include "program_15.h"

program_15::program_15(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
...
bool program_15::stepLengthCompensationCaculation_axis5()
{
...
void program_15::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
void program_15::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime = 50)
{
	//初始化变量
...
void program_15::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_15::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_15::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_15::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_15::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_15::clearRoundoutData()
{
	for (int i = 0; i < 7; i++)//需要将循环体的i变量修改为跳动采集的轴段总数
	{
...
//***********************************************************************************************************************************************************************************************************************************************
string program_15::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_15::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_15::cam2_Measure_prepare(int positionNumber)
{
...
void program_15::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_15::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_15::creatDatabaseTable()
{
...
void program_15::writeToDatabase()
{
	/*
...
void program_15::zeroMeasureNub()
{
	allFeatureFlag = "--";
	ngFeatureNum = 0;//NG特征数量
	measurePartsNum_all = 0;//该型号轴测量总数
...
void program_15::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
//远心相机图像检测算法
void program_15::cam0Picture_algorithm()//远心相机1图像算法
{
	...
	*/
	...
	double pic0To4_moveDistance;
	pic0To4_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[4]) - axis5_compensation(cam0_arriveOrgEncode_axis5[0]);//计算拍照位置0和拍照位置4之间的距离(减法代表拍照位置2-拍照位置1)
	double pic1To3_moveDistance;
	pic1To3_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[3]) - axis5_compensation(cam0_arriveOrgEncode_axis5[1]);//计算拍照位置1和拍照位置3之间的距离(减法代表拍照位置2-拍照位置1)
	double pic1To2_moveDistance;
	pic1To2_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[2]) - axis5_compensation(cam0_arriveOrgEncode_axis5[1]);//计算拍照位置1和拍照位置1之间的距离(减法代表拍照位置2-拍照位置1)
	//需要从以下复制
	//大远心相机采集图像路径
	/*大远心相机采集图像路径--调用示例
...
//	string Imgh1path = "C:/Users/123/Desktop/XD/12.7//H0.bmp";
	...
	*/
	...
	HTuple Pattern1path = "D:/temporary_sun/AxisMeasurement/programParmeter/WZ10451080-370-4/cam0_0.sbm";
	double cam0_1_105XIA_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_105XIA_TLineFP, Img1_raw,
		Pattern1path,//下行粘贴HAC调试坐标
		-55, -120, -55, -55,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	fault_detect = 2;//图像2处理函数开始
	HTuple Pattern2path = "D:/temporary_sun/AxisMeasurement/programParmeter/WZ10451080-370-4/cam0_1.sbm";
	double cam0_1_43_XIA_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_43_XIA_TLineFP, Img2Aug,
		Pattern2path,
		-400, -820, -400, -140,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_74x_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_74x_TLineFP, Img2Aug,
		Pattern2path,
		10, -830, 10, -330,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_74s_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_74s_TLineFP, Img2Aug,
		Pattern2path,
		610, -1940, 610, -1530,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	double cam0_1_Chamfer_73 = TLineFP_Chamfer_P(Img2Aug,
		859, 1877, 1150, 2400,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(cam0_1_Chamfer_73 * Calik);
	pushback_vectors(resultVectorList, 2, 73, 4.3, 0, 0.5);

	double cam0_1_Chamfer_172 = TLineFP_Chamfer_P(Img2Aug,
		1139, 3568, 1204, 3914,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(cam0_1_Chamfer_172 * Calik);
	pushback_vectors(resultVectorList, 2, 172, 0.65, -0.05, 0.05);

	double cam0_1_176_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_176_TLineFP, Img2Aug,
		Pattern2path,
		30, -80, -140, 110,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	...
	HTuple Pattern3path = "D:/temporary_sun/AxisMeasurement/programParmeter/WZ10451080-370-4/cam0_2.sbm";

	double cam0_1_84s_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_84s_TLineFP, Img3_raw,
		Pattern3path,
		-600, 0, 200, 0,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	fault_detect = 4;//图像处理函数结束
	HTuple Pattern4path = "D:/temporary_sun/AxisMeasurement/programParmeter/WZ10451080-370-4/cam0_3.sbm";

	double cam0_1_82s_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_82s_TLineFP, Img4_raw,
		Pattern4path,
		10, 0, 510, 0,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);

	fault_detect = 5;//图像处理函数结束
	HTuple Pattern5path = "D:/temporary_sun/AxisMeasurement/programParmeter/WZ10451080-370-4/cam0_4.sbm";
	double cam0_1_105s_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_105s_TLineFP, Img5_raw,
		Pattern5path,
		10, -20, 10, -80,
		50, 12, 1, 50,
		0.5, 1, 0.3, 0, 0.7);
	...
	double AxialD_105 = (cam0_1_105s_TLineFP[0] - cam0_1_105XIA_TLineFP[0]);
	resultVectorList.push_back(AxialD_105 * Calik + pic0To4_moveDistance);
	pushback_vectors(resultVectorList, 4, 105, 971.6, -0.5, 0.5);

	double AxialD_43 = (cam0_1_74x_TLineFP[0] - cam0_1_43_XIA_TLineFP[0]);
	resultVectorList.push_back(AxialD_43 * Calik);
	pushback_vectors(resultVectorList, 4, 43, 4.9, -0.1, 0);

	double AxialD_74 = (cam0_1_74s_TLineFP[0] - cam0_1_74x_TLineFP[0]);
	resultVectorList.push_back(AxialD_74 * Calik);
	pushback_vectors(resultVectorList, 4, 74, 6.87, -0.18, 0.18);

	double AxialD_176jd = atan((cam0_1_176_TLineFP[0] - cam0_1_176_TLineFP[2]) / (cam0_1_176_TLineFP[3] - cam0_1_176_TLineFP[1])) * 180 / PI;
	resultVectorList.push_back(AxialD_176jd);
	pushback_vectors(resultVectorList, 3, 176.1, 45, -5, 5);

	double AxialD_176 = ((cam0_1_176_TLineFP[3] - cam0_1_176_TLineFP[1]) + (cam0_1_176_TLineFP[0] - cam0_1_176_TLineFP[2])) / 2;
	resultVectorList.push_back(AxialD_176 * Calik);
	pushback_vectors(resultVectorList, 4, 176, 1.5, 0, 0.3);

	double AxialD_84 = cam0_1_84s_TLineFP[0] - cam0_1_74x_TLineFP[0];
	resultVectorList.push_back(AxialD_84 * Calik+ pic1To2_moveDistance);
	pushback_vectors(resultVectorList, 4, 84, 114.5, -0.5, 0.5);

	double AxialD_82 = cam0_1_82s_TLineFP[0] - cam0_1_74x_TLineFP[0];
	resultVectorList.push_back(AxialD_82 * Calik+ pic1To3_moveDistance);
	pushback_vectors(resultVectorList, 4, 82, 415, -0.5, 0.5);
...
double program_15::cam0Picture1_algorithmLuowenDown(HObject ho_Image,  HObject ho_Image_L,double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
...
double program_15::cam0Picture1_algorithmLuowenUp(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
	// Local iconic variables
	HObject  ho_ModelContours, ho_LineContours;
	HObject  ho_LineContour;
...
double program_15::cam0Picture1_L_K_b_Down(HObject ho_Image, double* hv_Box)
{
...
double program_15::cam0Picture1_L_K_b_Up(HObject ho_Image, double* hv_Box)
{
...
HObject program_15::imgAug(string imgPath) {
...
//模板匹配找特征点
void program_15::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
...
//根据起始点与终止点求倒角
double program_15::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
...
double program_15::TLineFP_Chamfer_H2(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
double program_15::TLineFP_Chamfer_H2_117up(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
void program_15::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_15::Straight_TLineFP_P137(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour;
...
void program_15::Straight_TLineFP_P116(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_15::Straight_TLineFP_P170(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_15::Straight_TLineFP_P102(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_15::cam0_roundoutMeasure_prepare(int positionNumber)
{
	//trap5轴
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_15::clearScrewRoundOutData()
{
	for (int i = 0; i < 2; i++)
	{
...
double program_15::cam1Picture_holeAlgorithm0(string imgPath)//H0孔图像算法
{
	//需要3.1在下方贴入孔径图像处理代码
	HTuple halconPath = imgPath.c_str();
...
double program_15::cam1Picture_holeAlgorithm1(string imgPath)//H1孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_15::cam1Picture_holeAlgorithm2(string imgPath)//H2孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_15::cam1Picture_holeAlgorithm3(string imgPath)//H3孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
void program_15::run()
{
	...
	defaultPath = defaultPath + "\\measureData\\WZ10451080-4";//！需要将文件后夹后缀名改为对应的零件号
	...
	*/
	...
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
	for (int i = 0; i < 8; i++)//第一次测量（向上移动）（需要将i修改为直径采集的总次数）
	{
	...
	for (int i = 7; i >= 0; i--)//第二次测量（向下移动）（需要将i修改为直径采集的总次数-1）
	{
	...
	*/
	...
	for (int i = 0; i < 3; i++)//需要修改i为粗糙度测量点位的总数量
	{
		switch (i)
		{
	...
	camPtrList[0]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[0]->capturedImg);
	for (int i = 0; i < 5; i++)//远心0-1号采集位置采集图像（将i修改为远心相机的拍照位置总数，循环中的其他语句不需要修改）
	{
		camPtrList[0]->setExposeTime(cam0_exposeTime);
		camPtrList[0]->m_captureMode = "continuous";
		originalImgPtr = &(camPtrList[0]->capturedImg);
	...
	double pic0To4_moveDistance;
	pic0To4_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[4]) - axis5_compensation(cam0_arriveOrgEncode_axis5[0]);//计算拍照位置0和拍照位置1之间的距离(减法代表拍照位置2-拍照位置1)
	double pic1To3_moveDistance;
	pic1To3_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[3]) - axis5_compensation(cam0_arriveOrgEncode_axis5[1]);//计算拍照位置0和拍照位置1之间的距离(减法代表拍照位置2-拍照位置1)
	double pic1To2_moveDistance;
	pic1To2_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[2]) - axis5_compensation(cam0_arriveOrgEncode_axis5[1]);//计算拍照位置0和拍照位置1之间的距离(减法代表拍照位置2-拍照位置1)
	...
	*/
	...
	looptime = 0;
	cam1_Measure_prepare(0);//对孔径拍摄位置H0进行测量（这个函数参数为0）如需要对H1孔径测量需要将（0）改为1，并复制
	
	for (int i = 0; i < cam1PathList.size(); i++)
	{
		try {
			double diameter = cam1Picture_holeAlgorithm0(cam1PathList[i]);//孔径相机拍摄位置H0的图像处理函数（注意函数名字，孔径处理算法调试完成后，粘贴进该函数中），如需要3.1对H1孔径测量需要将cam1Picture_holeAlgorithm0改为cam1Picture_holeAlgorithm1，并在其中粘贴孔径测量H1的相关算法
	...
	{
		pushback_vectors(resultVectorList, 8, 111, 8.9, -0, +0.2);//需要 3.4.3.3含义为将多次孔径测量的结果显示到软件上，需要修改pushback_vector参数的特征号，公称值等信息
	...
	*/
	...
	//跳动测量*****************************************************************
	//第一步采各跳动测量轴段的基本数据
	for (int i = 6; i >= 0; i--)//只需要修改i值为跳动数据采集的轴段的总数-1（该循环中的其他部分不需要修改）
	{
	...
	for (int i = 0; i < 7; i++)//（只需要修改i值为跳动数据采集的轴段的总数）
	{
		for (int j = 0; j < 3; j++)
		{
			centerCosFit(i, j);
	...
	for (int j = 0; j < 3; j++)
	{
		//提取第一个基准上三个数据采集截面的圆心数据，需要修改roundoutDataPtr[0]
		...
		loopTime++;
		//提取第2个基准上三个数据采集截面的圆心数据，需要修改roundoutDataPtr[5]
	...
	for (int i = 0; i < 7; i++)//需要修改i值为跳动数据采集的轴段的总数
	{
