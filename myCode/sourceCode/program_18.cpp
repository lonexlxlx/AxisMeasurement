...
program_18::program_18(cam_device* cam0, cam_device* cam1, cam_device* cam2, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr)
{
	//指针初始化
	camPtrList[0] = cam0;
	camPtrList[1] = cam1;
	...
	//跳动数据结构体初始化
	for (int i = 0; i < 1; i++)//如有跳动测量需求将i的循环上限修改为需要进行跳动采集的轴号（!!!!只是为了测试新建的子程序是否添加到主程序中时可以暂时不用修改）
	{
...
bool program_18::stepLengthCompensationCaculation_axis5()
{
...
void program_18::lsSensorMeasure_diameter(int positionNumber, int time)
{
	//使用光幕传感器进行直径测量
...
void program_18::lsSensorMeasure_cylindricity(int positionNumber, float intervalTime = 50)
{
	//初始化变量
...
void program_18::lsSensorMeasure_roundOutPrepare(int startLocation, int endLocation, int positionNumber, int intervalTime = 10)//使用光幕传感器进行跳动测量;startLocation特征测量开始采样位置的索引；endLocation采样结束时的位置索引
{
	//初始化变量
	/*
...
void program_18::axisLineCalculate(double pointsData[][3], int pointsNum, double* directionVector, double* aixsReferancePoint)//计算拟合轴线
{
...
void program_18::centerCosFit(int positionNumber, int positionIndex)//拟合对应跳动位置的余弦曲线
{
...
void program_18::axisLoaction(int positionNumber, int positionIndex, double* directionVector, double* aixsReferancePoint)//计算各跳动位置实际回转轴心
{
...
*/
void program_18::roundoutCalculate(int positionNumber, int positionIndex, int calculateMode = 0)
{
	int dataLength;
	//按照拟合的方式进行跳动计算
	if (calculateMode == 0)
...
void program_18::clearRoundoutData()
{
	for (int i = 0; i < 1; i++)//需要将循环体的i变量修改为跳动采集的轴段总数
	{
...
//***********************************************************************************************************************************************************************************************************************************************
string program_18::cam0_Measure_prepare(int positionNumber)
{
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_18::cam1_Measure_prepare(int positionNumber)
{
	cam1PathList.clear();
...
void program_18::cam2_Measure_prepare(int positionNumber)
{
...
void program_18::saveAsExcel()//将主界面上的表格保存为excel文件
{
...
bool program_18::mergeCells(QString start, QString end, QString value)
{
	if (worksheet == NULL)
		return false;
...
void program_18::creatDatabaseTable()
{
...
void program_18::writeToDatabase()
{
	/*
...
void program_18::zeroMeasureNub()
{
	allFeatureFlag = "--";
	ngFeatureNum = 0;//NG特征数量
	measurePartsNum_all = 0;//该型号轴测量总数
...
void program_18::pushback_vectors(vector<double>& fResult, int fType, double fIndex, double fNominalsize, double fLowerTolerance, double fUpperTolerance)
{
	//将对应测量结果写入动态数组储存
...
//远心相机图像检测算法
void program_18::cam0Picture_algorithm()//远心相机1图像算法
{
	...
	*/
	...
	//大远心相机采集图像路径
	/*大远心相机采集图像路径--调用示例
	string Img1path = "C:/Users/123/Desktop/XD/0.bmp";
	*/
	...
	*/
	...
	HTuple Pattern1path = "D:/temporary_sun/AxisMeasurement/programParmeter/Z82292121270-HJC/cam0_0.sbm";
	double cam0_0_1Start_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_0_1Start_TLineFP, Img1Aug,
		Pattern1path,
		-20, -10, -20, 275,
		50, 12, 1, 40,
		0.5, 1, 0.3, 0, 0.7);

	double Chamfer_2 = TLineFP_Chamfer_P(Img1_raw,
		cam0_0_1Start_TLineFP[0] - 120, cam0_0_1Start_TLineFP[1] - 300, cam0_0_1Start_TLineFP[0] + 150, cam0_0_1Start_TLineFP[1] - 40,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_2 * Calik);
	pushback_vectors(resultVectorList, 2, 2, 2, -0.2, +0.2);

	fault_detect = 2;//图像2处理函数开始
	HTuple Pattern2path = "D:/temporary_sun/AxisMeasurement/programParmeter/Z82292121270-HJC/cam0_1.sbm";
	double cam0_1_8_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_8_TLineFP, Img2Aug,
		Pattern2path,
		-680, 160, 50, 160,
		50, 12, 1, 30,
		0.5, 1, 0.3, 0, 0.7);

	double Chamfer_3 = TLineFP_Chamfer_P(Img2_raw,
		cam0_1_8_TLineFP[0] - 100, cam0_1_8_TLineFP[1] - 300, cam0_1_8_TLineFP[0] + 20, cam0_1_8_TLineFP[1] - 220,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_3 * Calik);
	pushback_vectors(resultVectorList, 2, 3, 1.2, -0.2, +0.2);

	double Chamfer_9 = TLineFP_Chamfer_P(Img2_raw,
		cam0_1_8_TLineFP[2] - 20, cam0_1_8_TLineFP[3] - 300, cam0_1_8_TLineFP[2] + 100, cam0_1_8_TLineFP[3] - 210,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_9 * Calik);
	pushback_vectors(resultVectorList, 2, 3, 1.2, -0.2, +0.2);

	fault_detect = 3;//图像3处理函数开始
	HTuple Pattern3path = "D:/temporary_sun/AxisMeasurement/programParmeter/Z82292121270-HJC/cam0_2.sbm";
	double cam0_1_12_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_12_TLineFP, Img3Aug,
		Pattern3path,
		-710, 200, -1, 200,
		50, 12, 1, 30,
		0.5, 1, 0.3, 0, 0.7);

	double Chamfer_13 = TLineFP_Chamfer_P(Img3_raw,
		cam0_1_12_TLineFP[0] - 100, cam0_1_12_TLineFP[1] - 300, cam0_1_12_TLineFP[0] + 20, cam0_1_12_TLineFP[1] - 230,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_13 * Calik);
	pushback_vectors(resultVectorList, 2, 13, 1.2, -0.2, +0.2);

	double Chamfer_17 = TLineFP_Chamfer_P(Img3_raw,
		cam0_1_12_TLineFP[2] - 20, cam0_1_12_TLineFP[3] - 300, cam0_1_12_TLineFP[2] + 100, cam0_1_12_TLineFP[3] - 230,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
		-1, 0, 0, 3, 2);
	resultVectorList.push_back(Chamfer_17 * Calik);
	pushback_vectors(resultVectorList, 2, 17, 1.2, -0.2, +0.2);

	fault_detect = 4;//图像4处理函数开始
	HTuple Pattern4path = "D:/temporary_sun/AxisMeasurement/programParmeter/Z82292121270-HJC/cam0_3.sbm";
	double cam0_1_20_TLineFP[4];
	TemplateMatching_TLineFP_P(cam0_1_20_TLineFP, Img4Aug,
		Pattern4path,
		21, 200, 1710, 200,
		50, 12, 1, 30,
		0.5, 1, 0.3, 0, 0.7);

	double Chamfer_18 = TLineFP_Chamfer_P(Img4Aug,
		cam0_1_12_TLineFP[0] - 120, cam0_1_12_TLineFP[1] - 400, cam0_1_12_TLineFP[0] + 100, cam0_1_12_TLineFP[1] - 230,
		2, 20, 60,
		true, 5, 4, 2,
		20, -0.5, 0.5,
		90, 1,
	...
	pushback_vectors(resultVectorList, 2, 18, 1.2, -0.2, +0.2);

	fault_detect = 0;//图像处理函数结束
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////以下程序对特征集中处理，不涉及特征点检测////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	double pic0To1_moveDistance;
	pic0To1_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[1]) - axis5_compensation(cam0_arriveOrgEncode_axis5[0]);
	...
	double pic0To3_moveDistance;
	pic0To3_moveDistance = axis5_compensation(cam0_arriveOrgEncode_axis5[3]) - axis5_compensation(cam0_arriveOrgEncode_axis5[0]);


	double AxialD_1 = cam0_1_8_TLineFP[0] - cam0_0_1Start_TLineFP[2];
	resultVectorList.push_back(AxialD_1 * Calik + pic0To1_moveDistance);
	pushback_vectors(resultVectorList, 4, 1, 68, -0.1, +0.1);

	double AxialD_5 = cam0_1_20_TLineFP[2] - cam0_0_1Start_TLineFP[2];
	resultVectorList.push_back(AxialD_5 * Calik + pic0To3_moveDistance);
	pushback_vectors(resultVectorList, 4, 5, 509.4, -0.1, +0.1);

	double AxialD_8 = cam0_1_8_TLineFP[2] - cam0_1_8_TLineFP[0];
	resultVectorList.push_back(AxialD_8 * Calik);
	pushback_vectors(resultVectorList, 4, 8, 8, -0.2, +0.2);

	double AxialD_12 = cam0_1_12_TLineFP[2] - cam0_1_12_TLineFP[0];
	resultVectorList.push_back(AxialD_12 * Calik);
	pushback_vectors(resultVectorList, 4, 12, 8, -0.2, +0.2);

	double AxialD_15 = cam0_1_20_TLineFP[2] - cam0_1_12_TLineFP[2];
	resultVectorList.push_back(AxialD_15 * Calik + pic0To3_moveDistance - pic0To2_moveDistance);
	pushback_vectors(resultVectorList, 4, 15, 80.3, -0.1, +0.1);

	double AxialD_20 = cam0_1_20_TLineFP[2] - cam0_1_20_TLineFP[0];
	resultVectorList.push_back(AxialD_20 * Calik);
	pushback_vectors(resultVectorList, 4, 20, 20, -1, +1);


};


double program_18::cam0Picture1_algorithmLuowenDown(HObject ho_Image,  HObject ho_Image_L,double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
...
double program_18::cam0Picture1_algorithmLuowenUp(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru)//测螺纹
{
	// Local iconic variables
	HObject  ho_ModelContours, ho_LineContours;
	HObject  ho_LineContour;
...
double program_18::cam0Picture1_L_K_b_Down(HObject ho_Image, double* hv_Box)
{
...
double program_18::cam0Picture1_L_K_b_Up(HObject ho_Image, double* hv_Box)
{
...
HObject program_18::imgAug(string imgPath) {
...
//模板匹配找特征点
void program_18::TemplateMatching_TLineFP_P(double* TLineFP_result, HObject ho_Image, HTuple hv_ModelFile, int MatchingLine1, int MatchingLine2, int MatchingLine3, int MatchingLine4,
...
//根据起始点与终止点求倒角
double program_18::TLineFP_Chamfer_P(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
...
double program_18::TLineFP_Chamfer_H2(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
double program_18::TLineFP_Chamfer_H2_117up(HObject ho_Image, double GenRect1, double GenRect2, double GenRect3, double GenRect4,
	int EdgesSubP1, int EdgesSubP2, int EdgesSubP3,
	bool Segment, int SegmentContour1, int SegmentContour2, int SegmentContour3,
	double SelectContour1, double SelectContour2, double SelectContour3,
	int UnionContour1, int UnionContour2,
...
void program_18::Straight_TLineFP_P(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_18::Straight_TLineFP_P137(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour;
...
void program_18::Straight_TLineFP_P116(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
...
void program_18::Straight_TLineFP_P170(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_18::Straight_TLineFP_P102(double* TLineFP_result, HObject ho_Image, int Line1, int Line2, int Line3, int Line4,
	int Metrology1, int Metrology2, int Metrology3, int Metrology4)
{
	// Local iconic variables
	HObject  ho_LineContours, ho_LineContour, ho_ImageMean1, ho_ImageEmphasize1, ho_ImageIlluminate, ho_ImageEquHisto;
...
void program_18::cam0_roundoutMeasure_prepare(int positionNumber)
{
	//trap5轴
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
...
void program_18::clearScrewRoundOutData()
{
	for (int i = 0; i < 2; i++)
	{
...
double program_18::cam1Picture_holeAlgorithm0(string imgPath)//H0孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_18::cam1Picture_holeAlgorithm1(string imgPath)//H1孔图像算法
{
	//在下方贴入孔径图像处理代码
...
double program_18::cam1Picture_holeAlgorithm2(string imgPath)//H2孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_18::cam1Picture_holeAlgorithm3(string imgPath)//H3孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
double program_18::cam1Picture_holeAlgorithm4(string imgPath)//H4孔图像处理算法
{
	//在下方贴入孔径图像处理代码
...
void program_18::run()
{
	...
	defaultPath = defaultPath + "\\measureData\\Z82292121270-HJC";//！需要将文件后夹后缀名改为对应的零件号
	...
	*/
	...
	moveControlPtr->setCurrentAxis(5);
	moveControlPtr->setMoveMode("Trap");
	for (int i = 0; i < 6; i++)//第一次测量（向上移动）（需要将i修改为直径采集的总次数）
	{
	...
	for (int i = 5; i >= 0; i--)//第二次测量（向下移动）（需要将i修改为直径采集的总次数-1）
	{
	...
	//dataScreenOrg.push_back(lsDiameterResult[0][0]);
	...
	cam2_radius[0][1] = lsDiameterResult[2][9];//传递旋转半径d4给粗糙度0号采集位置（如有新增复制本句代码）进行相应修改即可
	/*
	新增参考代码
	...
	*/
	...
	for (int i = 0; i < 1; i++)//需要修改i为粗糙度测量点位的总数量
	{
		switch (i)
		{
	...
	camPtrList[0]->m_captureMode = "continuous";
	originalImgPtr = &(camPtrList[0]->capturedImg);
	for (int i = 0; i < 4; i++)//远心0-1号采集位置采集图像（将i修改为远心相机的拍照位置总数，循环中的其他语句不需要修改）
	{
		camPtrList[0]->setExposeTime(cam0_exposeTime);
		camPtrList[0]->m_captureMode = "continuous";
		originalImgPtr = &(camPtrList[0]->capturedImg);
	...
	*/
	...
	//跳动测量*****************************************************************
	//第一步采各跳动测量轴段的基本数据
	for (int i = 0; i >= 0; i--)//只需要修改i值为跳动数据采集的轴段的总数-1（该循环中的其他部分不需要修改）
	{
	...
	for (int i = 0; i < 1; i++)//（只需要修改i值为跳动数据采集的轴段的总数）
	{
		for (int j = 0; j < 3; j++)
		{
			centerCosFit(i, j);
	...
	for (int i = 0; i < 1; i++)//需要修改i值为跳动数据采集的轴段的总数
	{
		//对于基准采集位置不进行跳动计算，所以使用case跳过
		switch (i) {
		//case 0://跳动计算轴段RT0（需要跳过的基准轴段）
		//	break;
		//case 1://跳动计算轴段RT1（以A-B为基准）
