...
class program_17 :public QThread//需要将所有 program_17替换为新的子程序号
{
	Q_OBJECT;
public:
	...
	program_17(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_17();

	QString partName = "转自连接轴";//需要修改为零件名称
	QString partNub="WZ10151107-HJC";//需要修改为零件图号
	...
	//光幕传感器测量对应的移动位置
	long ls_StepLength_axis5[4] = {//需要将数组长度修改为直径测量位置的总数，并将点位信息填入
		700143,724000,776000,831586};


	//光幕传感器直径测量结果存储（//需要将数组第一维长度修改为直径测量位置的总数），并将直径信息填入
	double lsDiameterResult[4][10] = { //0号是一次测量值，1号为2次测量值，2号是两次均值，3号是工序卡序号,4为标准值，5为上限，6为下限，7第一次旋转距离（补偿），8第二次旋转距离（补偿），9旋转距离均值
		{ 0, 0, 0,9 ,45.06 ,0 ,-0.01 , 0, 0, 0},{ 0, 0, 0,14 ,47.24 ,0 ,-0.1 , 0, 0, 0},{ 0, 0, 0, 58 ,40 ,0 ,-0.12 , 0, 0, 0},{ 0, 0, 0,12 ,41.87 ,0 ,-0.01 , 0, 0, 0} };
	...
	long lsRoundoutSteplength_axis5[2][3] = {//第一组是A基准,第二组是基准B也是137号待测圆柱面
		{699643,700143,700643},{831086,831586,832086} };
	...
	long cam0_StepLength_axis5[2] = {413110,565110};//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
	...
	long cam2_parpare[2][3] = {//0号元素五轴移动位置；1号1轴移动到位置（参考）；2号1轴退回位置(其中前三组位置是在凸台下方)；
		...
		{-42700, -184352,120000},{9300, -195193,120000}
