...
class program_14 :public QThread
{
	Q_OBJECT;
public:
	...
	program_14(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_14();

	QString partName = "动力涡轮传动轴";//需要修改为零件名称
	QString partNub="XD-WZ10451080-370";//需要修改为零件图号
	...
	//光幕传感器测量对应的移动位置
	long ls_StepLength_axis5[8] = {//需要将数组长度修改为直径测量位置的总数，并将点位信息填入
		803436,846541,954290,2430080,2429980,2430180,2348860,2561995};


	//光幕传感器直径测量结果存储（//需要将数组第一维长度修改为直径测量位置的总数），并将直径信息填入
	double lsDiameterResult[8][10] = { //0号是一次测量值，1号为2次测量值，2号是两次均值，3号是工序卡序号,4为标准值，5为上限，6为下限，7第一次旋转距离（补偿），8第二次旋转距离（补偿），9旋转距离均值
		{ 0, 0, 0,7,40.055,0,-0.011, 0, 0, 0},{ 0, 0, 0,10,40.053,0.013,0.005, 0, 0, 0},{ 0, 0, 0,76,85.65,0.225,0.0215, 0, 0, 0},{ 0, 0, 0,207,34.8,0,-0.1,0, 0, 0},
         { 0, 0, 0,207,34.8,0,-0.1,0, 0, 0}, { 0, 0, 0,207,34.8,0,-0.1,0, 0, 0},{ 0, 0, 0,148,35.14,0,-0.011, 0, 0, 0},{ 0, 0, 0,168,29.3,0,-0.2, 0, 0, 0}
	};


	//光幕传感器圆柱度测量参数设定（需要将数组长度修改为圆柱度测量位置的总数，并将点位信息填入）
	long lsCylindricitySteplength_axis5[2][3] = {//0号是第一个截面5轴对应的位置，1号是2截面，2号是3截面
		{800721,804648,808576},{2344950,2348860,2352770} };
	...
	long lsRoundoutSteplength_axis5[7][3] = {//第一组是A基准,第二组是基准B也是137号待测圆柱面
		{710260,719637,729014},{798400,803436,808472},{832650,832697,832744},{951457,9542895,957122},{1196580,1207530,1218480},{2344950,2348860,2352770},{2560470,2561995,2563520} };
		//（需要将数组长度修改为跳动采集轴段位置的总数，并将跳动图纸信息填入，如果采集轴段为基准可以默认填入{0，0,0}）
	float lsRoundoutResult[7][3] = {//0号工序卡序号，1号是测量值，2号是上限
		{0,0,0}, {24,0,0.1},{25,0,0.1},{71,0,0.012},{86, 0, 0.03},{0,0,0},{154,0,0.05 }};
	//光幕传感器跳动测量数据（需要将数组长度修改为跳动采集轴段位置的总数）
	roundoutData* roundoutDataPtr[6];//第一组是A基准,第2组是24号跳动尺寸,是第三组诗25号；,是第4组诗71号,是第4组诗86号,第6组是基准b;
	...
	string imgSavePath[5];//远心相机采集图像地址（需要将数组长度修改为远心拍照位置的总数）
	long cam0_StepLength_axis5[5] = {350583,707195,1179830,1507730, 2563520,};//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
	...
	vector<string> cam1PathList;//孔径相机拍照储存位置（须要多次拍照）
	double CalikKong = 0.00691842;//测孔花键相机标定系数   从0.686改
	//将修改数组第一维长度为孔径相机拍摄的位置总数，并将孔径相机的点位写入
	long cam1_prepare[1][5] = {//0号元素是5轴位置，1号元素是2轴拍照位置，2号元素是2轴退回位置,3号元素为特征的均布个数,4号是图像的绘制模式（参考主程序中的AxisMeasurement::displayImg说明）//需要2.6，改绘图模式（程序号）
		{1522150 ,-294386,-220000,3,16}
	};
	...
	double cam1Picture_holeAlgorithm0(string imgPath);
	double cam1Picture_holeAlgorithm1(string imgPath);
	double cam1Picture_holeAlgorithm2(string imgPath);
