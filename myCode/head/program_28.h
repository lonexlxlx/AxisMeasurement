...
class program_28 :public QThread//需要将所有 program_28替换为新的子程序号
{
	Q_OBJECT;
public:
	...
	program_28(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_28();

	QString partName = "长拉紧螺栓";//需要修改为零件名称
	QString partNub="J644123B-210";//需要修改为零件图号
	...
	//光幕传感器测量对应的移动位置
	long ls_StepLength_axis5[3] = { 710060,1110915,1132366 };
	...
	double lsDiameterResult[3][10] = { {0,0,0,1,25,0,-.15,0,0,0},{0,0,0,5,12.15,0,-.05,0,0,0},{0,0,0,4,11.1,0,-.02,0,0,0} };
	...
	long cam0_StepLength_axis5[1] = { 849641 };//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
