...
class program_6 :public QThread
{
	Q_OBJECT;
public:
	...
	program_6(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_6();

	void MyHalconExceptionHandler(const HException& except);
	QString partName = "涡轮连接轴";
	QString partNub="WZ10-44-1112-155";
	...
	long ls_StepLength_axis5[6] = {
		640000,690000,710000,748000,800500,827000};
	//光幕传感器直径测量结果存储
	double lsDiameterResult[6][10] = { //0号是一次测量值，1号为2次测量值，2号是两次均值，3号是工序卡序号,4为标准值，5为上限，6为下限，7第一次旋转距离（补偿），8第二次旋转距离（补偿），9旋转距离均值
		{ 0,0,0,42,49.8,-0.015,-0.135,0,0,0},{ 0,0,0,52,49.8,-0.02,-0.05,0,0,0}, { 0,0,0,53,50.27,0,-0.05,0,0,0},{ 0,0,0,55,64.72,0,-0.1,0,0,0},{ 0,0,0,11,75.5,0,-0.05,0,0,0}, 
		{ 0,0,0,10,76.5,0,-0.05,0,0,0}, };
	...
	*/
	...
	long lsRoundoutSteplength_axis5[4][3] = {//第二组是跳动基准E
		{654000,655000,656000},{704000,710000,716000},{800000,800500,801000},{826000,827000,828000}
        };                                                                            
	float lsRoundoutResult[4][3] = {//0号工序卡序号，1号是测量值，2号是上限
		{999.1, 0, 0.03}, {0, 0, 0}, {999.2, 0, 0.03}, {999.3, 0, 0.03}};
	struct roundoutData {
	...
	//光幕传感器跳动测量数据
	roundoutData*  roundoutDataPtr[4];//第二个指针用于储存基准E的相关数据
	...
	long cam0_StepLength_axis5[2] = {386000,500000};//远心相机测量时轴5对应的点位
	...
	long cam1_prepare[1][5] = {//0号元素是5轴位置，1号元素是2轴拍照位置，2号元素是2轴退回位置,3号元素为特征的均布个数,4号是图像的绘制模式（参考主程序中的AxisMeasurement::displayImg说明）
		{-85000 - 600,-255000,-220000,10,8}};
	int cam1_exposeTime=700;//测圆孔相机曝光
