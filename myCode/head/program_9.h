...
class program_9 :public QThread
{
	Q_OBJECT;
public:
	...
	program_9(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_9();

	QString partName = "输出轴";
	QString partNub="WZ10-11-2006-430";
	...
	long stepLengthCompensation_cam0Place= 366600;//五轴补偿计算初始拍照位置
	...
	//光幕传感器测量对应的移动位置
	long ls_StepLength_axis5[34] = {//
		623600,624000,624400,640000,651400,651750,652100,660000,667200,667500,
	    667800,703000,753000,767000,815000,865200,873600,882836,883136,883436,
	    898514,898814,899114,905000,973500,973900,974300,977600,978000,978400,
	    993000,999200,999600,1000000};


	//光幕传感器直径测量结果存储
	double lsDiameterResult[34][10] = { //0号是一次测量值，1号为2次测量值，2号是两次均值，3号是工序卡序号,4为标准值，5为上限，6为下限，7第一次旋转距离（补偿），8第二次旋转距离（补偿），9旋转距离均值
		{ 0, 0, 0,30 ,58 ,0 ,-0.3 , 0, 0, 0},{ 0, 0, 0,30 ,58 ,0 ,-0.3 , 0, 0, 0},{ 0, 0, 0,30 ,58 ,0 ,-0.3 , 0, 0, 0},{ 0, 0, 0, 15,60.7, - 0.035,-0.048 , 0, 0, 0},{ 0, 0, 0, 18,58.5 ,0 ,-0.3 , 0, 0, 0},
		{ 0, 0, 0, 18,58.5 ,0 ,-0.3, 0, 0, 0},{ 0, 0, 0, 18,58.5 ,0 ,-0.3 , 0, 0, 0},{ 0, 0, 0, 16, 60.66,0 ,-0.3 , 0, 0, 0},{ 0, 0, 0, 18,58.5 ,0 ,-0.3 , 0, 0, 0},{ 0, 0, 0,18,58.5 ,0 ,-0.3 , 0, 0, 0},
		{ 0, 0, 0, 18,58.5 ,0 ,-0.3 , 0, 0, 0},{ 0, 0, 0, 17,60.2 ,0 ,-0.3 , 0, 0, 0},{ 0, 0, 0,54 ,60.8 ,0.038 ,0.025 , 0, 0, 0},{ 0, 0, 0, 40,60.2 ,0 ,-0.3 , 0, 0, 0},{ 0, 0, 0,39 ,61 ,0.033 ,0.026 , 0, 0, 0},
		{ 0, 0, 0, 115,69.3 ,0 ,-0.3 , 0, 0, 0},{ 0, 0, 0, 86, 40, 0.1,-0.1 , 0, 0, 0},{ 0, 0, 0, 94,45.5 ,0 ,-0.25 , 0, 0, 0},{ 0, 0, 0, 94,45.5 ,0 ,-0.25  , 0, 0, 0},{ 0, 0, 0, 94,45.5 ,0 ,-0.25 , 0, 0, 0},
		{ 0, 0, 0, 94,45.5 ,0 ,-0.25 , 0, 0, 0},{ 0, 0, 0,94,45.5 ,0 ,-0.25  , 0, 0, 0},{ 0, 0, 0, 94,45.5 ,0 ,-0.25  , 0, 0, 0},{ 0, 0, 0, 157,42 ,0 ,-0.16 , 0, 0, 0},{ 0, 0, 0,168 ,43.5 ,0.2 ,0 , 0, 0, 0},
		{ 0, 0, 0,168 ,43.5 ,0.2 ,0  , 0, 0, 0},{ 0, 0, 0,168 ,43.5 ,0.2 ,0 , 0, 0, 0},{ 0, 0, 0, 155,38.8,0,-0.25, 0, 0, 0},{ 0, 0, 0,155,38.8,0,-0.25, 0, 0, 0},{ 0, 0, 0,155,38.8,0,-0.25 , 0, 0, 0},
		{ 0, 0, 0,148 ,39.88 ,0.02 ,0.008 , 0, 0, 0},{ 0, 0, 0, 147,37.8 ,0 ,-0.25 , 0, 0, 0},{ 0, 0, 0,147,37.8 ,0 ,-0.25, 0, 0, 0},{ 0, 0, 0, 147,37.8 ,0 ,-0.25 , 0, 0, 0} };


	//光幕传感器圆柱度测量参数设定
	long lsCylindricitySteplength_axis5[2][3] = {//0号是第一个截面5轴对应的位置，1号是2截面，2号是3截面
		{785000,815000,850000},  {984000,993000,994000} };
	//光幕传感器圆柱度测量结果
	double lsCylindricityResult[2][3] = {//0号是工序卡序号，1号是测量值，2号是上限
		{36, 0, 0.003}, {136, 0, 0.003} };
	...
	long lsRoundoutSteplength_axis5[2][3] = {//第一组是A基准,第二组是基准B也是137号待测圆柱面
		{785000,815000,850000},{984000,993000,994000}, };
	float lsRoundoutResult[2][3] = {//0号工序卡序号，1号是测量值，2号是上限
		{0,0,0}, {137, 0, 0.005}};
	//光幕传感器跳动测量数据
	roundoutData* roundoutDataPtr[2];//第一组是A基准,第二组是基准B也是137号待测圆柱面
	...
	string imgSavePath[4];//远心相机采集图像地址
	long cam0_StepLength_axis5[4] = { 366600,482000,625000,693000};//远心相机测量时轴5对应的点位
	double cam0_arriveOrgEncode_axis5[4] = { 0, 0, 0, 0 };//远心相机到位对应的光栅尺原始读数（用于补偿粗糙度测量移动位置）
	...
	double pic1To2_moveDistance = 99;//相机从位置1到2位移距离
	double pic2To3_moveDistance = 99;//相机从位置2到3位移距离
	double pic3To4_moveDistance = 99;//相机从位置3到4位移距离
	...
	long cam1_prepare[5][5] = {//0号元素是5轴位置，1号元素是2轴拍照位置，2号元素是2轴退回位置,3号元素为特征的均布个数,4号是图像的绘制模式（参考主程序中的AxisMeasurement::displayImg说明）
		{125000 - 600,-263500,-220000,4,0},{143000 - 600,-273000,-220000,3,11},{166000 - 600,-276000,-220000,9,12},{200000 - 600,-277500,-220000,3,13},{222000 - 600,-278000,-220000,4,0}
	};
	int cam1_exposeTime=550;//测圆孔相机曝光
	double cam1_axis5Place[5] = {0,0,0,0,0};
	//孔直径相机需要传递的参数
	double cam1_h2_118down[4];
	double cam1_h2_118down_0[3];
	double cam1_h2_118down_2[3];
	double pichole2to4_moveDistance;
	//粗糙度相机测量参数设定***********************************************************************************************************************************************************************************


	string roughnessPicPath[5];
	long cam2_parpare[4][3] = {//0号元素五轴移动位置；1号1轴移动到位置（参考）；2号1轴退回位置(其中前三组位置是在凸台下方)；

		{-126700, -142500-1000,120000}, {-63700, -143910 - 1000, 120000}, {48300, -141720 - 1000,120000}, {226300, -194600 - 1000, 120000}
	};
	float cam2_radius[4][2] = {//0号元素参考回转半径值；1号元素实际回转半径值
		{30.331, 0},{30.0444, 0},{30.51665, 0},{19.9529, 0}
	};
	double roughnessResult[4][3] = {//0号粗糙度编号，1号粗糙度上限；2号测量值
		{13, 0.4, 99},{55, 1.6, 99},{48, 1.6, 99},{139, 0.8, 99}
	};
	...
	void cam0Picture_algorithm();//远心相机1图像算法
	...
	double cam1Picture_holeAlgorithm1(string imgPath);
	void cam1Picture_holeAlgorithm2(string imgPath, int n, vector <double>& container_116,vector <double>& container_117, vector <double>& container_119);
	void cam1Picture_holeAlgorithm3(string imgPath);
	void cam1Picture_holeAlgorithm4(string imgPath,int n, vector <double>& container_117, vector <double>& container_118, vector <double>& container_170);
	double cam1Picture_holeAlgorithm5(string imgPath);
