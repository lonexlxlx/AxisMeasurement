...
class program_27 :public QThread//需要将所有 program_27替换为新的子程序号
{
	Q_OBJECT;
public:
	...
	program_27(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_27();

	QString partName = "中心拉杆";//需要修改为零件名称
	QString partNub="S100153028";//需要修改为零件图号
	...
	//光幕传感器测量对应的移动位置
	long ls_StepLength_axis5[5] = {//需要将数组长度修改为直径测量位置的总数，并将点位信息填入
		960480,976466,989109,1084340,1208670};


	//光幕传感器直径测量结果存储（//需要将数组第一维长度修改为直径测量位置的总数），并将直径信息填入
	double lsDiameterResult[3][10] = { //0号是一次测量值，1号为2次测量值，2号是两次均值，3号是工序卡序号,4为标准值，5为上限，6为下限，7第一次旋转距离（补偿），8第二次旋转距离（补偿），9旋转距离均值
		{ 0, 0, 0,26 ,86.2 ,0 ,-0.05 , 0, 0, 0},{ 0, 0, 0,39 ,47.24 ,0 ,-0.1 , 0, 0, 0},{ 0, 0, 0,27 ,29.6 ,0 ,-0.1 , 0, 0, 0}};
	...
	string imgSavePath[2];//远心相机采集图像地址（需要将数组长度修改为远心拍照位置的总数）
	long cam0_StepLength_axis5[2] = { 678502,1527680};//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
	double cam0_arriveOrgEncode_axis5[2] = { 0, 0};//远心相机到位对应的光栅尺原始读数（用于补偿粗糙度测量移动位置）（需要将数组长度修改为远心拍照位置的总数，默认填入0）
	...
	int screwPrepare[2][3] = { 
		{968331,1208900,4}, {357691,1208900,4} };
