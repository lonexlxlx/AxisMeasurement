...
class program_13 :public QThread
{
	Q_OBJECT;
public:
	...
	program_13(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_13();

	QString partName = "动力涡轮传动轴";//需要修改为零件名称
	QString partNub="WZ10-45-1080-370-3";//需要修改为零件图号
	...
	long ls_StepLength_axis5[6] = {//需要将数组长度修改为直径测量位置的总数，并将点位信息填入
		803436,846541,954290,2430080,2429980,2430180 };
	...
	double lsDiameterResult[6][10] = { //0号是一次测量值，1号为2次测量值，2号是两次均值，3号是工序卡序号,4为标准值，5为上限，6为下限，7第一次旋转距离（补偿），8第二次旋转距离（补偿），9旋转距离均值
		{ 0, 0, 0,7 ,40.055 ,0 ,-0.011 , 0, 0, 0},{ 0, 0, 0,10 ,40.053 ,0.013 ,0.005 , 0, 0, 0},{ 0, 0, 0,76 ,85.65 ,0.225 ,0.0215 , 0, 0, 0},
        { 0, 0, 0,207 ,34.8 ,0 ,-0.1 , 0, 0, 0},{ 0, 0, 0,207 ,34.8 ,0 ,-0.1 , 0, 0, 0},{ 0, 0, 0,207 ,34.8 ,0 ,-0.1 , 0, 0, 0}
	};


	//光幕传感器圆柱度测量参数设定（需要将数组长度修改为圆柱度测量位置的总数，并将点位信息填入）
	long lsCylindricitySteplength_axis5[2][3] = {//0号是第一个截面5轴对应的位置，1号是2截面，2号是3截面
		{832650,846541,860432},{2344950,2348860,2352770} };
	...
	long lsRoundoutSteplength_axis5[6][3] = {//第一组是A基准,第二组是基准B也是137号待测圆柱面
		{710260,719637,729014},{798400,803436,808472},{832650,832697,832744},{951457,954290,957122},{1196580,1207530,1218480},{2344950,2348860,2352770} };
	//（需要将数组长度修改为跳动采集轴段位置的总数，并将跳动图纸信息填入，如果采集轴段为基准可以默认填入{0，0,0}）
	float lsRoundoutResult[6][3] = {//0号工序卡序号，1号是测量值，2号是上限
		{0,0,0}, {24,0,0.1},{25,0,0.1},{71,0,0.012},{86, 0, 0.03},{0,0,0} };
	...
	long cam0_StepLength_axis5[3] = { 707195,1179830,1507730};//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
