...
class program_18 :public QThread//需要将所有 program_18替换为新的子程序号
{
	Q_OBJECT;
public:
	...
	program_18(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_18();

	QString partName = "参照轴";//需要修改为零件名称
	QString partNub="Z82292121270-HJC";//需要修改为零件图号
	...
	long ls_StepLength_axis5[6] = {//需要将数组长度修改为直径测量位置的总数，并将点位信息填入
		670000,757676,1100000,1469200,1530000,1620000};
	...
	double lsDiameterResult[6][10] = { //0号是一次测量值，1号为2次测量值，2号是两次均值，3号是工序卡序号,4为标准值，5为上限，6为下限，7第一次旋转距离（补偿），8第二次旋转距离（补偿），9旋转距离均值
		{ 0, 0, 0,19 ,57 ,0.1 ,-0.1 , 0, 0, 0},{ 0, 0, 0,11 ,64 ,0 ,-0.2 , 0, 0, 0},{ 0, 0, 0,19.1 ,57 ,0.1 ,-0.1 , 0, 0, 0} ,{ 0, 0, 0,14 ,65 ,0 ,-0.2 , 0, 0, 0} ,{ 0, 0, 0,19.2 ,57 ,0.1 ,-0.1 , 0, 0, 0},{ 0, 0, 0,21 ,65 ,0 ,-0.2 , 0, 0, 0} };


	//光幕传感器圆柱度测量参数设定（需要将数组长度修改为圆柱度测量位置的总数，并将点位信息填入）
	long lsCylindricitySteplength_axis5[1][3] = {//0号是第一个截面5轴对应的位置，1号是2截面，2号是3截面
		{757176,757676,758176} };
	...
	long lsRoundoutSteplength_axis5[1][3] = {//第一组是A基准,第二组是基准B也是137号待测圆柱面
		{1090000,1100000,1110000}  };
	//（需要将数组长度修改为跳动采集轴段位置的总数，并将跳动图纸信息填入，如果采集轴段为基准可以默认填入{0，0,0}）
	float lsRoundoutResult[1][3] = {//0号工序卡序号，1号是测量值，2号是上限
		{10, 0, 0.1}  };
	//光幕传感器跳动测量数据（需要将数组长度修改为跳动采集轴段位置的总数）
	roundoutData* roundoutDataPtr[1];//第一组是A基准,第2组是100号跳动尺寸,是第三组诗99号；第四组是基准b;
	...
	string imgSavePath[4];//远心相机采集图像地址（需要将数组长度修改为远心拍照位置的总数）
	long cam0_StepLength_axis5[4] = {339630,500000,1200000,1330000	};//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
	double cam0_arriveOrgEncode_axis5[4] = {0, 0, 0, 0};//远心相机到位对应的光栅尺原始读数（用于补偿粗糙度测量移动位置）（需要将数组长度修改为远心拍照位置的总数，默认填入0）
