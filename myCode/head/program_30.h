...
class program_30 :public QThread//需要将所有 program_30替换为新的子程序号
{
	Q_OBJECT;
public:
	...
	program_30(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_30();

	QString partName = "输出轴";//需要修改为零件名称
	QString partNub="Z911301G-190";//需要修改为零件图号
	...
	//光幕传感器测量对应的移动位置
	long ls_StepLength_axis5[7] = { 608716,631589,663648,673669,721378,760001,773706};

	//光幕传感器直径测量结果存储（//需要将数组第一维长度修改为直径测量位置的总数），并将直径信息填入
	double lsDiameterResult[7][10] = { {0,0,0,15,60.4,0,-.05,0,0,0},{0,0,0,16,57.7,0,-.25,0,0,0},{0,0,0,17,61,0,-.05,0,0,0},{0,0,0,18,58.5,0,-.3,0,0,0},{0,0,0,19,60,0,-.1,0,0,0},{0,0,0,20,61,0,-.1,0,0,0},{0,0,0,21,60.6,0,-.1,0,0,0} };
	...
	long cam0_StepLength_axis5[2] = { 368491,517708 };//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
	double cam0_arriveOrgEncode_axis5[2] = { 0,0 };
	//远心相机到位对应的光栅尺原始读数（用于补偿粗糙度测量移动位置）（需要将数组长度修改为远心拍照位置的总数，默认填入0）
