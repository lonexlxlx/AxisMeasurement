...
class program_37 :public QThread//需要将所有 program_37替换为新的子程序号
{
	Q_OBJECT;
public:
	...
	program_37(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_37();

	QString partName = "薄壁传动轴";//需要修改为零件名称
	QString partNub="J6D66508-660";//需要修改为零件图号
	...
	//光幕传感器测量对应的移动位置
	long ls_StepLength_axis5[1] = {897603};

	//光幕传感器直径测量结果存储（//需要将数组第一维长度修改为直径测量位置的总数），并将直径信息填入
	double lsDiameterResult[1][10] = { {0,0,0,1,69.75,0,-.1,0,0,0} };
	...
	long cam0_StepLength_axis5[1] = { 874395 };//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
