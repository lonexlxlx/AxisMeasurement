...
class program_2 :public QThread
{
	Q_OBJECT;
public:
	...
	program_2(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_2();

	QString partName = "转子连接轴";
	QString partNub="WZ10-15-1107-340";
	...
	long ls_StepLength_axis5[22] = {
		584000,596000,613800,630000,678800,679100,679400,682300,682450,682600,
	    682750,682900,686150,686450,686750,692300,699500,710750,783000,832000,
	    839650,863000};
	//光幕传感器直径测量结果存储
	double lsDiameterResult[22][10] = { //0号是一次测量值，1号为2次测量值，2号是两次均值，3号是工序卡序号,4为标准值，5为上限，6为下限，7第一次旋转距离（补偿），8第二次旋转距离（补偿），9旋转距离均值
		{ 0,0,0,119,91.69,0,-0.25,0,0,0},{ 0,0,0,118,66.42,0,-0.38,0,0,0},{ 0,0,0,26,61.727,0,-0.01,0,0,0},{ 0,0,0,76,61.2,0.2,-0.2,0,0,0},{ 0,0,0,59,54.74,0.12,-0.12,0,0,0},
		{ 0,0,0,59,54.74,0.12,-0.12,0,0,0},{ 0,0,0,59,54.74,0.12,-0.12,0,0,0}, { 0,0,0,58,49.0,0.2,-0.2,0,0,0},{ 0,0,0,58,49.0,0.2,-0.2,0,0,0},{ 0,0,0,58,49.0,0.2,-0.2,0,0,0},
		{ 0,0,0,58,49.0,0.2,-0.2,0,0,0},{ 0,0,0,58,49.0,0.2,-0.2,0,0,0},{ 0,0,0,50,49.74,0,-0.01,0,0,0},{ 0,0,0,50,49.74,0,-0.01,0,0,0},{ 0,0,0,50,49.74,0,-0.01,0,0,0},
		{ 0,0,0,52,43.58,0.12,-0.12,0,0,0},{ 0,0,0,51,44.993,0,-0.009,0,0,0},{ 0,0,0,53,40,0.12,-0.12,0,0,0},{ 0,0,0,130,40,0.12,-0.12,0,0,0},{ 0,0,0,131,41.818,0,-0.009,0,0,0},
		{ 0,0,0,132,39.11,0,-0.24,0,0,0},{ 0,0,0,135,39.11,0,-0.24,0,0,0}};
	...
	*/
	...
	// 光幕传感器跳动初始化参数 * *******************************************************************************************************************************************************
	 ...
	 //该零件无跳动测量要求
	long lsRoundoutSteplength_axis5[3][3] = {//第一组是待测点，第二组为基准I，第三组为基准K
		{685300,686300,687300},{698500,699500,700500},{831500,833000,834500},
        };                                                                                   
	float lsRoundoutResult[3][3] = {//0号工序卡序号，1号是测量值，2号是上限
		{6, 0, 0.05}, {0, 0, 0}, {0, 0, 0}};
	struct roundoutData {
	...
	//光幕传感器跳动测量数据
	roundoutData*  roundoutDataPtr[3];//第一组是待测点，第二组为基准I，第三组为基准K
	...
	//远心相机测量参数初始化****************************************************************************************************************************************************************************
	string imgSavePath[3];//远心相机采集图像地址
	long cam0_StepLength_axis5[3] = {358000,428000,560000};//远心相机测量时轴5对应的点位
	...
	double pic2To3_moveDistance = 0;//相机从位置1到2位移距离
	...
	long cam2_parpare[3][3] = {//0号元素五轴移动位置；1号1轴移动到位置（参考）；2号1轴退回位置(；                     
		{-67200,-181600-1000 ,120000 }, {16300,-194160 - 1000 ,120000 }, {65300,-189660 - 1000 ,120000 }};
	float cam2_radius[3][2] = {//0号元素参考回转半径值；1号元素实际回转半径值
		{22.496, 0},{20.004, 0}, {20.911, 0}};
	double roughnessResult[3][3] = {//0号粗糙度编号，1号粗糙度上限；2号测量值
		{138,0.8 , 99},{148,1.6 , 99}, {48,0.8 , 99}};
