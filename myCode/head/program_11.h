...
class program_11 :public QThread
{
	Q_OBJECT;
public:
	...
	program_11(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_11();

	QString partName = "动力涡轮传动轴";//需要修改为零件名称
	QString partNub="WZ10451080-1";//需要修改为零件图号
	...
	//光幕传感器测量对应的移动位置
	long ls_StepLength_axis5[6] = {//需要将数组长度修改为直径测量位置的总数，并将点位信息填入
		797351,842089,953948,2429880,2430080,2430280};


	//光幕传感器直径测量结果存储（//需要将数组第一维长度修改为直径测量位置的总数），并将直径信息填入
	double lsDiameterResult[6][10] = { //0号是一次测量值，1号为2次测量值，2号是两次均值，3号是工序卡序号,4为标准值，5为上限，6为下限，7第一次旋转距离（补偿），8第二次旋转距离（补偿），9旋转距离均值
		{ 0, 0, 0,7 ,40.055 ,0 ,-0.011 , 0, 0, 0},{ 0, 0, 0,10 ,40.053 ,0.013 ,0.005 , 0, 0, 0},{ 0, 0, 0,76 ,85.65 ,0.225 ,0.215 , 0, 0, 0},{ 0, 0, 0,207 ,34.8 ,0 ,-0.1 , 0, 0, 0}
	   ,{ 0, 0, 0,207 ,34.8 ,0 ,-0.1 , 0, 0, 0} ,{ 0, 0, 0,207 ,34.8 ,0 ,-0.1 , 0, 0, 0} };


	//光幕传感器圆柱度测量参数设定（需要将数组长度修改为圆柱度测量位置的总数，并将点位信息填入）
	long lsCylindricitySteplength_axis5[2][3] = {//0号是第一个截面5轴对应的位置，1号是2截面，2号是3截面
		{796851,797351,797851},{2347680,2348680,2349680} };
	...
	long lsRoundoutSteplength_axis5[6][3] = {//a,a-b,a-b,-,-,b
		{719825,720325,720825},{796851,797351,797851}, {841589,842089,842589}, {953748,953948,954148}, {1207165,1207665,1208165}, {2347680,2348680,2349680} };
	//（需要将数组长度修改为跳动采集轴段位置的总数，并将跳动图纸信息填入，如果采集轴段为基准可以默认填入{0，0,0}）
	float lsRoundoutResult[6][3] = {//0号工序卡序号，1号是测量值，2号是上限
		{0,0,0}, {24,0,0.01},{25,0,0.01},{71,0,0.012},{86, 0, 0.03},{0,0,0} };
	//光幕传感器跳动测量数据（需要将数组长度修改为跳动采集轴段位置的总数）
	roundoutData* roundoutDataPtr[6];//a,a-b,a-b,-,-,b
	...
	long cam0_StepLength_axis5[3] = { 686141,923522,1514230};//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
	...
	double cam1Picture_holeAlgorithm2(string imgPath, int n, vector <double>& container_116,vector <double>& container_117, vector <double>& container_119);
	double cam1Picture_holeAlgorithm3(string imgPath);
	double cam1Picture_holeAlgorithm4(string imgPath,int n, vector <double>& container_117, vector <double>& container_118, vector <double>& container_170);
