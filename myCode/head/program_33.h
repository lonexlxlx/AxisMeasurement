...
class program_33 :public QThread//需要将所有 program_33替换为新的子程序号
{
	Q_OBJECT;
public:
	...
	program_33(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_33();

	QString partName = "短拉紧螺栓";//需要修改为零件名称
	QString partNub="J644123B-260";//需要修改为零件图号
	...
	//光幕传感器测量对应的移动位置
	long ls_StepLength_axis5[2] = { 845853,1109389 };

	//光幕传感器直径测量结果存储（//需要将数组第一维长度修改为直径测量位置的总数），并将直径信息填入
	double lsDiameterResult[2][10] = { {0,0,0,5,14.04,0,-.02,0,0,0},{0,0,0,4,12.1,-.06,-.08,0,0,0} };
