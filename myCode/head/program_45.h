...
#include "rtwtypes.h"//matlab files									
#include <cstddef>									
#include <cstdlib>									
...
class program_45 :public QThread//需要将所有 program_45替换为新的子程序号									
{
	Q_OBJECT;
public:
	...
	program_45(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_45();

	QString partName = " 粗糙轴 ";
	QString partNub = " KT033145221-217GX";
	...
	//光幕传感器测量对应的移动位置								
	long ls_StepLength_axis5[2] = { 888061,888079 };
	...
	double lsDiameterResult[2][10] = { {0,0,0,298,40.0147,0.1,-0.1,0,0,0},{0,0,0,297,40.0147,0.1,-0.1,0,0,0} };
	...
	long lsRoundoutSteplength_axis5[1][3] = { {344126,344133,344139} };
	...
	roundoutData* roundoutDataPtr[1];
	...
	string imgSavePath[3];
	long cam0_StepLength_axis5[3] = { 413234,614899,614899 };
	double cam0_arriveOrgEncode_axis5[3] = { 0,0,0 };
	...
	long cam2_parpare[2][3] = { {123068,-195333,120000},{123068,-195333,120000} };



	//需要修改数组为粗糙度点位总数，并写入半径信息								
	float cam2_radius[2][2] = { {20.00735,0},{20.00735,0} };


	//需要修改数组为粗糙度点位总数，并写入粗糙度尺寸信息								
	double roughnessResult[2][3] = { {18,1.4,99},{22,0.8,99} };


	int cam2_exposeTime = 400;
