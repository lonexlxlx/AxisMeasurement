...
class program_36 :public QThread//需要将所有 program_36替换为新的子程序号
{
	Q_OBJECT;
public:
	...
	program_36(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_36();

	QString partName = "短拉紧螺栓";//需要修改为零件名称
	QString partNub="WJ10H131035-XL";//需要修改为零件图号
	...
	//光幕传感器测量对应的移动位置
	long ls_StepLength_axis5[1] = {1692041};

	//光幕传感器直径测量结果存储（//需要将数组第一维长度修改为直径测量位置的总数），并将直径信息填入
	double lsDiameterResult[1][10] = { {0,0,0,2,79.12,0.6,-.8,0,0,0} };
	...
	double roughnessResult[1][3] = {//0号粗糙度特征号，1号粗糙度上限；2号测量值
		{1, 3, 99}
	};
