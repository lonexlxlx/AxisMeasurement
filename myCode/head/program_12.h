...
class program_12 :public QThread
{
	Q_OBJECT;
public:
	...
	program_12(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_12();

	QString partName = "动力涡轮传动轴";//需要修改为零件名称
	QString partNub="WZ10451080-2";//需要修改为零件图号
	...
	long ls_StepLength_axis5[6] = {//需要将数组长度修改为直径测量位置的总数，并将点位信息填入
		800943,843387,953948,2429880,2430080,2430280 };
	...
	long lsCylindricitySteplength_axis5[2][3] = {//0号是第一个截面5轴对应的位置，1号是2截面，2号是3截面
		{800443,800943,801443},{2347680,2348680,2349680} };
	...
	long lsRoundoutSteplength_axis5[6][3] = {//第一组是A基准,第二组是基准B也是137号待测圆柱面
		{719825,720325,720825},{800443,800943,801443}, {842887,843387,843887}, {953748,953948,954148}, {1207165,1207665,1208165}, {2347680,2348680,2349680} };
	...
	roundoutData* roundoutDataPtr[6];//A;D0;D1;D2;_;B;
	...
	long cam0_StepLength_axis5[3] = {681204,927879,1519920};//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
	...
	long cam2_parpare[2][3] = {//0号元素五轴移动位置；1号1轴移动到位置（参考）；2号1轴退回位置(其中前三组位置是在凸台下方)；
		...
		{76687, -195190,120000},{187248,-80702,120000}
