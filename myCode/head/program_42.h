...
class program_42 :public QThread//需要将所有 program_42替换为新的子程序号
{
	Q_OBJECT;
public:
	...
	program_42(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_42();

	QString partName = "动力涡轮传动轴";//需要修改为零件名称
	QString partNub = "WZ10451080-4";//需要修改为零件图号
	...
	long lsCylindricitySteplength_axis5[2][3] = {//0号是第一个截面5轴对应的位置，1号是2截面，2号是3截面
		{832650,796853,860432 },{2344950,2348860,2352770}
	};
	...
	long cam0_StepLength_axis5[5] = { 350583,707195,930589,1507730,2278920 };//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
	double cam0_arriveOrgEncode_axis5[5] = { 0, 0,0,0,0 };//远心相机到位对应的光栅尺原始读数（用于补偿粗糙度测量移动位置）（需要将数组长度修改为远心拍照位置的总数，默认填入0）
