...
class program_1 :public QThread
{
	Q_OBJECT;
public:
	...
	program_1(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_1();
	...
	*/
	...
	//远心相机测量参数初始化****************************************************************************************************************************************************************************
	string imgSavePath[5];//远心相机采集图像地址
	long cam0_StepLength_axis5[5] = {458000,580000,700000,820000,944000};//远心相机测量时轴5对应的点位
	double cam0_arriveOrgEncode_axis5[5] = { 0, 0,0,0,0};//远心相机到位对应的光栅尺原始读数（用于补偿粗糙度测量移动位置）
	...
	double pic2To3_moveDistance = 0;
	double pic3To4_moveDistance = 0;
	double pic4To5_moveDistance = 0;
	...
	*/
    ...
    //pair<double, double> program_1::cam0Picture1_algorithmLuowen(HObject ho_Image, HObject ho_Image_L, double L, float OUT2, int m, double Pix_zhoujing_chuanru);//用远心相机0的第一张图像测螺纹
