...
class program_16 :public QThread//需要将所有 program_16替换为新的子程序号
{
	Q_OBJECT;
public:
	...
	program_16(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_16();

	QString partName = "转子连接轴";//需要修改为零件名称
	QString partNub="WZ1015117-160-1";//需要修改为零件图号
	...
	long ls_StepLength_axis5[8] = {//需要将数组长度修改为直径测量位置的总数，并将点位信息填入
		697489,699225,700961,726207,771783,828875,831231,833586};
	...
	double lsDiameterResult[8][10] = { //0号是一次测量值，1号为2次测量值，2号是两次均值，3号是工序卡序号,4为标准值，5为上限，6为下限，7第一次旋转距离（补偿），8第二次旋转距离（补偿），9旋转距离均值
		{ 0, 0, 0,9 ,45.06 ,0 ,-0.01 , 0, 0, 0},{ 0, 0, 0,9 ,45.06 ,0 ,-0.01 , 0, 0, 0},{ 0, 0, 0,9 ,45.06 ,0 ,-0.01 , 0, 0, 0},{ 0, 0, 0,103 ,44.069 ,0 ,-0.127 , 0, 0, 0},{ 0, 0, 0,58 ,40 ,0.12 ,-0.12 , 0, 0, 0},
	    { 0, 0, 0,12 ,41.67 ,0 ,-0.01 , 0, 0, 0},{ 0, 0, 0,12 ,41.67 ,0 ,-0.01 , 0, 0, 0},{ 0, 0, 0,12 ,41.67 ,0 ,-0.01 , 0, 0, 0} };
	...
	long cam0_StepLength_axis5[2] = { 408109,562014};//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
	...
	long cam2_parpare[2][3] = {//0号元素五轴移动位置；1号1轴移动到位置（参考）；2号1轴退回位置(其中前三组位置是在凸台下方)；
		...
		{-40493,-184368,120000},{64531, -190210,120000}
