...
class program_32 :public QThread//需要将所有 program_32替换为新的子程序号
{
	Q_OBJECT;
public:
	...
	program_32(cam_device* cam1, cam_device* cam2, cam_device* cam3, moveControl* cradDevicePtr, ls_device* lsDevicePtr, QTableWidget* tablePtr, QSqlDatabase* DbPtr, roughnessFun* roughnessObjPtr);
	~program_32();

	QString partName = "长螺栓";//需要修改为零件名称
	QString partNub="WZ10442111-100";//需要修改为零件图号
	...
	//光幕传感器测量对应的移动位置
	long ls_StepLength_axis5[4] = { 731734,790396,839837,886659};

	//光幕传感器直径测量结果存储（//需要将数组第一维长度修改为直径测量位置的总数），并将直径信息填入
	double lsDiameterResult[4][10] = { {0,0,0,12,7.65,0,-.1,0,0,0},{0,0,0,11,6.45,0,-.1,0,0,0},{0,0,0,9,7.65,0,-.1,0,0,0},{0,0,0,8,6.45,0,-.1,0,0,0}};
	...
	long cam0_StepLength_axis5[2] = { 467244,593062 };//远心相机测量时轴5对应的点位（需要将数组长度修改为远心拍照位置的总数，并将拍摄的点位写入）
	...
	long cam2_parpare[1][3] = { {89345,-276323,120000} };
	//需要修改数组为粗糙度点位总数，并写入半径信息
	float cam2_radius[1][2] = {//0号元素参考回转半径值；1号元素实际回转半径值
		{7.6195,0}
	};
	//需要修改数组为粗糙度点位总数，并写入粗糙度尺寸信息
	double roughnessResult[1][3] = { {1,1.6,99} };

	int cam2_exposeTime=300;//粗糙度镜头曝光
